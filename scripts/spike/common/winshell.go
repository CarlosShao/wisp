...
// COM object at `this`.
func comCall(this uintptr, slot uintptr, args ...uintptr) (uintptr, uintptr, error) {
	vt := *(*uintptr)(unsafe.Pointer(this))
	fn := *(*uintptr)(unsafe.Pointer(vt + slot*8))
	r1, r2, err := syscall.SyscallN(fn, append([]uintptr{this}, args...)...)
	return r1, r2, err
}
...
func CreateD2DWindow(title string) (*D2DWindow, error) {
	fmt.Fprintln(os.Stderr, "TRACE CreateD2DWindow begin")
	pCoInitializeEx.Call(0, coinitApartmentMT)
	fmt.Fprintln(os.Stderr, "TRACE coinit done")
	...
	fmt.Fprintln(os.Stderr, "TRACE show+alpha done")
	w.RenderFrame("Wisp S0 spike")
	fmt.Fprintln(os.Stderr, "TRACE render done")
	pumpMessages(6)
	fmt.Fprintln(os.Stderr, "TRACE pump done")
	return w, nil
}
...
func AttachToKillOnCloseJob() (windows.Handle, error) {
	...
	if r, _, err := procSetInformationJobObject.Call(job, 9, uintptr(unsafe.Pointer(&li)), uintptr(size)); r == 0 {
		return 0, err
	}
	if err := windows.AssignProcessToJobObject(jobHandle, windows.CurrentProcess()); err != nil {
		return 0, err
	}
