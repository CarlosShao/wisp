...
- **[A30] ⚠ 一次全量证据自查：我自己盖的三张"D22 clean"是假的，一张票在 0/8 框的情况下被我改了 `-done`** —
  ...
  - **⑤ 元教训：门必须自报工作量，否则"没报问题"和"没看"在输出上长得一模一样。** 本轮挖出的同类空仪器：
    ...
    （R16 裁定 3 禁止回退），但**另外两条还没有归属**——建议新立一张"每个门自报工作量"的小票收口
    （我先不建票面，等票 70 的全仓格式化落地，避免撞车）。
  **归属**：①我今天就地改回（已完成）；②④ 归**证据更正代理**（只追加、不覆写原文，每条带可复现命令）；
  ⑤ 归票 70 之后的新票；②的根因修复（正确调用姿势）已在 A22/票 67 落地。
