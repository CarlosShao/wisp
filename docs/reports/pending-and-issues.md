...
| **Q-55**（**09-26 00:1x 新出现，未答；这是登记、不是催**）代码里有 **4 枚 `DEFERRED` 标记（`D28-1` 三枚／`D11-3` 一枚），`SPEC-12 §5` 的登记表却没有对应行** ⇒ `AGENTS.md §1.1` 那条"1:1 **双向**对得上"今天不成立，而**没有任何仪器在扫它**（`tools/d22scan` 那七项 ban 不含它；说不出 run id＋step 就当门不存在）。三条出路**没有一条能避开冻结面**：**甲**＝标记写错了（`D28-1` 这种"D 号＋子号"在 `docs/specs/**`＋`PLAN.md` 里**从未被定义过**，全仓只有 `D45-1`/`D45-2` 两个先例），该改标记 ⇒ 动 4 处注释；**乙**＝登记表缺行，补 `D28-1`／`D11-3` 两行 ⇒ **动 `docs/specs/**`＝契约文本**；**丙**＝`AGENTS §1.1` 那句里 `D-xx` 从来不含票内子号，是这句话射程写宽了 ⇒ **重述契约句**（也是人工批准那一档）。⇒ **我的推荐＝甲，随后再上一枚对账检（双向差集）**。**不按推荐来的坏处**：走乙最省事，但会把一个**没有文本定义的编号**固化进规格，下一位照 §5 反查代码时仍然查不到语义；走丙等于承认规矩空转，那 4 处欠账继续"代码里有、表上查无此人"。**人话后果（一句）**：不修的话，规格里写着"推迟的事要两边都数得着"，而代码里四处推迟的事在规格里**查不到**——下一位按规格去核，会合理地下结论"这些欠账不存在"。详见票 150（立为 `blocked`，等这一枚）。 |
