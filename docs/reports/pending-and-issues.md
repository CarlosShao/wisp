...
| **Q-53**（**09-25 21:2x 新出现，未答**）票 146 把 `LiveApprovals()` 那扇门后的 8 枚引用槽位真拷了一层，但我 21:2x 现跑发现**同族第二扇门原样留着**：`internal/agent/approval/queue.go:478` 那枚 `replay` 里 **`:493 d := it.Dec`** 不走 `cloneDecision`（一条 `grep -rn ':= it\.Dec'` 现量：**全仓非测试恰好 1 处，就是它**）。⇒ 修它必须动 `queue.go`——**那是审批队列本体，票 146 全程零字节，也在我的禁改面上** | 甲：批准放开 `queue.go` **只有 `replay` 那一行**，本票（148）连着把"会响的检"一起补上（照 146 `AC#2` 规格：摘掉修法那一发必须从绿变红）／乙：不批，那一行继续留着，我把"已知未收"钉在台账与 148 票面上，**不假装它没了** | **甲**（理由：这跟 146 是同一枚缺陷的第二发，不是新范围；而 146 已经给出现成形状可照——那 8 枚槽位、那把会红的检、那句承重，都不用重新设计。⚠ 但**批准权在你**，我不自决） | 不答＝148 `AC#1` 一直按住；**`AC#2`（把 146 那支从未字面复算的资源读数补上）不受影响、今天就能做**。口令「批 148 Ⓐ」／「驳 148 Ⓐ」 |
| **Q-54**（**09-25 21:2x 新出现，未答**）面板到底该收到哪些字段，今天**没有任何一份冻结文本说过**。`LiveApprovals()` 往外送的是一份含 `Params`（**完整工具参数**）的东西；今天唯一读侧是 `cmd/wisp/panel_pump.go`（只读、且只落有界摘要）。⇒ 146 `AC#4` 钉住了"**这枚函数不许变成入站路由**"，但**"往外送什么范围"是空的**——**空不等于"已批准全送"** | 甲：**具名收窄**成"面板只需要的那几枚字段"（配一份白名单＋一枚会红的对账）／乙：**具名写"全字段照送"**＋一句为什么／丙：先不答，等界面定稿（`A254` 那件）一起看——因为"面板需要哪些字段"的答案一半在界面上 | **丙**（理由与 `Q-52` 撤回同源：**在一版还会被推翻的界面上定"面板需要什么字段"＝让你为变动中的东西签冻结契约**；而它今天**不挡任何腿**——146 已结、泵在、只读侧无泄露形状。⚠ 若走甲：**`Params` 再往下一层仍共用**那件事（value 里的 `[]any`／嵌套 map，`reflect.DeepEqual` 照不到，二轮验收在健康锚点上量到"写前写后都 true、队列记录真被改掉、仓内零条用例红"）**必须一起处置**，否则等于把 146 那扇门重新开一条缝） | 不答＝148 `AC#3` 按住，`queue.go:453-456` 那份"8 枚只护 1 枚"的投影归属也一起悬着；其余不受影响。口令「批 148 Ⓑ」／「驳 148 Ⓑ」 |

| **Q-55**（**09-26 00:1x 新出现，未答；这是登记、不是催**）代码里有 **4 枚 `DEFERRED` 标记（`D28-1` 三枚／`D11-3` 一枚），`SPEC-12 §5` 的登记表却没有对应行** ⇒ `AGENTS.md §1.1` 那条"1:1 **双向**对得上"今天不成立，而**没有任何仪器在扫它**（`tools/d22scan` 那七项 ban 不含它；说不出 run id＋step 就当门不存在）。三条出路**没有一条能避开冻结面**：**甲**＝标记写错了（`D28-1` 这种"D 号＋子号"在 `docs/specs/**`＋`PLAN.md` 里**从未被定义过**，全仓只有 `D45-1`/`D45-2` 两个先例），该改标记 ⇒ 动 4 处注释；**乙**＝登记表缺行，补 `D28-1`／`D11-3` 两行 ⇒ **动 `docs/specs/**`＝契约文本**；**丙**＝`AGENTS §1.1` 那句里 `D-xx` 从来不含票内子号，是这句话射程写宽了 ⇒ **重述契约句**（也是人工批准那一档）。⇒ **我的推荐＝甲，随后再上一枚对账检（双向差集）**。**不按推荐来的坏处**：走乙最省事，但会把一个**没有文本定义的编号**固化进规格，下一位照 §5 反查代码时仍然查不到语义；走丙等于承认规矩空转，那 4 处欠账继续"代码里有、表上查无此人"。**人话后果（一句）**：不修的话，规格里写着"推迟的事要两边都数得着"，而代码里四处推迟的事在规格里**查不到**——下一位按规格去核，会合理地下结论"这些欠账不存在"。详见票 150（立为 `blocked`，等这一枚）。 **⚠ 09-26 01:0x 依据更正（结论不变、我给的两条理由各错一处，原句不抹）**：① 我写"乙＝动 `docs/specs/**`＝契约变更，须人工批准"——138 验收程按 `SPEC-12 §4.1:38` 字面射程（由 C/D 枚举报决定）判这句**扣不上**；② 我引 `AGENTS §1.1` 那句里"配不上代码标记"那一半——它实测 **`SPEC-12 §5` 是 27 枚散文键数据行、零枚 `DEFERRED(D-xx)` 标记**（与我自己那把尺量的"该段含 `|` 共 29 行"不矛盾：29 减表头与分隔行＝27）⇒ **真已违的是反方向**（有标记无行），不是我以为的双向都缺。**剩下两条够硬的理由**是：票面 `:12` 逐字冻 `docs/specs/*.md`；以及**登记落笔者不能是受益人**。⇒ **而这件事真需要你的理由只剩一条**：票 138 的来源是**你批准的 `Q-43`**，把它改口＝改你批过的范围，那是你的权限、不是我的。⇒ 另补一枚版本账（给票 150 的 AC#1）：D 形状标记在 `3f6322f` 是 **3 枚／2 种**，在 HEAD 已是 **4 枚**（139 的 `compress.go` 多出第三枚）——**分母随锚点变，别拿任一侧当定数。** |
...
- [2026-09-25 19:4x +08] **A255｜票 146 交件：写手在两处顶回了我，两处都对——一处是我的票面自相矛盾，一处是我的字段表数少了三枚**

  **被验对象**＝`7f75ff3`（票 146 实现程五枚 commit：`598c0c6`/`22f7b1a`/`0d12661`/`3fde333`/`7f75ff3`，未推）。我逐枚 `git show --numstat` 点过名：**只碰 4 枚文件**（`internal/agent/approval/pending_read.go` ＋1 枚新测试 ＋ 它自己的证据件 ＋ 票面 Progress log），**禁改清单路径零命中**，五枚 AC 框**一枚没勾**（它把勾留给了非实现者表，这是对的形状——本仓那族缺陷叫"票面全勾而表为空"）。

  **① 我的票面里有一处自相矛盾，被它抓出来：`AC#1 默认 ⓑ` 与 `AC#2` 不能同时成立。**
  票 146 的 `AC#2` 我把断言极性写死了——"就地改掉返回值里那枚 map 或那几枚切片之后，**队列里那条原始记录没有跟着变**"。而 ⓑ 那一支（只改注释、实现仍浅拷贝）下这句**永假**。
  ⇒ 它给的第三条理由也成立：**"把修法摘掉、这一发是不是从此打不红"这一问，对"一句注释"恒答"仍不红"** ⇒ ⓑ 无论怎么造用例都收不下 `AC#2` 的承重判据；而 `AC#4` 又把 `tools/d22scan/**` 划成零字节（不许另造静态扫那一路仪器）。⇒ **两枚 AC 只有 ⓐ（真拷一层）同时收得下。**
  ⚠ **这正是我在票 145 上刚登记过的同一族缺陷（`AC#2 ↔ AC#4③` 冲突）在我自己票面上的第二次复发**，而且**方向一样**：我把"默认方向"和"验收判据"分别写在不同段落，**写第二段落时没有回头推演第一段落那支能不能活到第二段落**。⇒ 规矩要落成一条硬动作：**今后任何一张写了"默认方向"的票，出票前必须把那个默认方向代入每一条 AC 逐条问"它在这支下结得了吗"**，把答案写进票面（不是记在我脑子里）。
  ⚠ **代价我认下来一条**：它选 ⓐ 之后，`Params` 的**再往下一层**今天仍不隔离（value 里的 `[]any`／嵌套 map 仍共用底层），它明说了两发检都不探那一层、也明说"要钉那一层是一票的体量、不是本票残余"。⇒ **这一格不许被读成"已彻底解决"**，登记成后续票的候选（等验收表回来再定开不开）。

  **② 它把我的字段表从 5 枚改成 8 枚，我独立复核为"它对"。**
  我票面那张表点了 `Params`(map)／`Args`／`RulesHit`／`Paths`／`Capabilities` 五枚，**漏了 `Blacklist BlacklistNote` 里那三枚 `[]string`**（`Absolute`／`Unlockable`／`AlreadyUnlocked`，`internal/tools/mode.go:66-81`）。我刚逐字读了 `internal/tools/gate.go:15-40` 与那枚结构定义 ⇒ **1 map ＋ 7 slice ＝ 8 枚槽位，它数得对、我数少了**。
  ⚠ **这次漏数的性质比"数字错"重**：**如果它照我的表做"深拷一层"，那三枚切片仍会与队列共用底层，而那句 "copies" 的注释就会被当成已经兑现而永久留下**——**一张不完整的清单会让修复看起来比实际做完**。⇒ 定式进记忆：**凡票面里出现"逐字段/逐处"这种穷举式表格，我必须要么给出得出那张表的命令（让它可复算），要么明写"本表非穷举"**——我这次两者都没做，只写了五枚看起来像穷举的行。
  附带两枚同向的小不符：它现量 `LiveApprovals(` **10 处**（我票面写 11，差在测试侧一枚）、那条 `RulesHit =|RulesHit[` 普查它数出 **4 处**（我写 3，多出 `internal/panel/approval.go:93`）。⇒ **票面文字它一字没改、照现量做**，这是我要的形状。

  **③ `AC#3` 那一格最值钱的那条不是数字，是它自己指出"这台仪器结构上照不到这枚改动"**：`wisp slo -state WorkPeak` 的 subject 是 skeleton（`goroutines_max=1`），**那 5 秒里 `LiveApprovals` 一次都没被调用**（唯一接线在 `cmd/wisp/run.go:422` 的 `wisp run` 腿）。双臂均值差**小于组内极差** ⇒ 它按票面写"未观察到差异"而不是"无代价"。
  ⇒ **口径从此要这样引**：**票 146 的资源那一问在 `WorkPeak` 这一档是"未取证"，不是"已量过、没代价"**；它的替代凭据是一发确定性 `-bench`（落在仓外，满深度一次调用 ＋4736 B／＋72 allocs）。⚠ 这条与记忆里那两条同族："**改后全绿可能是那条新路径零次执行**"、"**step 层的绿可以等于什么都没测**"——这次是三度同形，只是这回**是写手自己报的**，所以验收程那一格只需复算、不需揭发。

  **④ 验收已派**（非实现者、锚 `7f75ff3`、只写 `docs/evidence/s1/146-liveapprovals-r1-accept-r1.md`），四发进攻逐条写进派单：**改前复跑**（摘掉修法必须红，红不起来＝装饰，直接退回不附条件）／**单点回退**（**只撤 `Blacklist.Absolute` 那一枚**——正是我票面漏掉、它补上的那批，答不出哪条用例变红＝覆盖面主张要从"8 枚"复算成"实覆 N 枚"）／**极性反证**（三种读回里若有一种其实比的是返回值，那一路不抵账）／**深度许诺复算**（`cloneParamsMap` 只拷一层，注释写的边界与代码做到的必须逐字一致，不一致＝退回）。

  `next=`＝①**验收表在飞**，回来再定五枚框；②**票 144 也在飞**（已交 `9d51d1f`/`95885fb`/`f63f0e3`/`4b607ea` 四枚，AC#1 判"不走自证作废那一支"、AC#2 已让 `collectReport` 分得开"没写完"与"坏了"）⇒ **两票收齐后一次批量推**；③⚠ **推送后必做逐名比集合那把尺**（`A254③` 那条固定动作），因为 `test-core` 现在带着 `TestC21DesignTokensFourWayAgree` 那一枚**已知常红**——不比对就会有人把它误归到票 146 或票 144 头上（写手自己也点了这一格，它的处置是"不救、改走 `git archive` 纯净快照"，这是对的）；④`Params` 再往下一层那一格**待验收回来决定是否开票**；⑤界面那一整块继续"等 owner 反馈"（`A254`），我这边一枚都不摆给他。

- [2026-09-25 19:5x +08] **A256｜票 144 那程撞 150 轮上限死了；这一次我没有代提它的脏增量——因为 `A250①` 那枚"我救 WIP 时只跑了 go vet"三小时前刚把我自己烫过一次**

  **它实际交回了什么（`result` 字段那句"Now the code change (AC#2 + AC#5):"不算数，按老规矩看 commit 序列＋盘上内容）**：**四枚已提交**——`9d51d1f`(证据 §0，105 行，含"票面行号我逐格重量过、两处与票面不符")／**`95885fb` fix(AC#2/AC#5)** `cmd/wisp/slo_windows.go` +193/−19／**`f63f0e3` test(AC#1)** 新 `cmd/wisp/slo_report_144_windows_test.go` +319／`4b607ea`(证据 §1)。**另留三枚未提交**：票面 73/5、那枚测试 36/7、证据件 264/6。⇒ **它是带着 AC#3 与 AC#4 两格没做死的，不是从头就没干活**（164 次工具调用／37 分钟）。

  **我替它先查的两件事（都给了续程，省它一程）**：
  ①**`go vet ./cmd/wisp/` 在那个脏状态下 rc=0** ⇒ 它留下的测试文件语法完整、没截断在半句（我读了尾部 30 行，一枚函数闭合）。
  ②**但 `go test -count=1 -run 'SLO144' ./cmd/wisp/` 起不来**：整份输出只有 `exit status 0xc0000135` ＋ 两行 `FAIL`，**`=== RUN` 0 条**。⇒ **这不是代码红，是测试进程加载期缺 DLL**（票 98 那一族）。正确跑法是仓里现成的 CI 同形脚本 **`bash scripts/wisp-cli-tests.sh`**，它替你做 DLL 注入；⚠ 那里头 :100-106 的注释记了一条**反直觉坑**：`PATH` 要用 **shell 自己的形式**（`/d/work/...`），**用 `pwd -W` 那种带空格的 `D:/work/...` 盘符形式反而 `0xc0000135`**（MSYS2 对原生子进程重写 PATH 的形状不被 DLL 搜索接受）。`third_party/sherpa-onnx/` 三枚 dll 现量在位。
  ⚠ **关键的一条是我自己的读数纪律**：`0xc0000135` 与"整包 rc=1"长得一模一样，**如果我只看 rc 就会把它记成"续程的活失败了"**；而"0 条 `=== RUN`"才是"根本没跑到"的凭据。⇒ 这两枚形状（**rc 相同、成因相反**）以后一律用 `=== RUN` 枚数分开。

  **③ 决定：它的脏增量我不代提，交续程跑出来再落盘。** 这一条是 `A250①` 的**正面回收**——今天 18:3x 我救半死者的 WIP 时只跑了 `go vet` 就提交，结果把一道**恒红**推上了 `cmd/wisp`（那格红今天还在归因账上）；随后又漏了 gofumpt 把第二道门推红（`A253①`）。⇒ **同一种动作再次出现时，我选了"先拿到行为读数再提"**：续程派单第一格就是"用 CI 同形脚本跑一遍、把三件输出贴进它第一次 commit 作为'我没凭读代码认账'的凭据"。
  ⇒ 顺带把规矩写硬：**代提死者脏增量的门槛不是"vet 通"，是"那枚文件所属包的测试真跑出过读数"**；拿不到读数就不许提，改派续程。

  **④ 一处现场证据：ZCode 此刻真在接手前端。** 收笔时 `git status` 里有 ` M frontend/scripts/gen-tokens.mjs`（**mtime 距我那次读数 34 秒**）与未跟踪 `.zcodeignore`（18:48）。⇒ **与我的程无关、一个字不进任何一枚我的 commit**；我在续程派单里把它具名点成"停手回报"的触发条件之一（**"我用了 pathspec"不是豁免**，清单里有别人就不许提）。这也顺带证明 `A254` 那条"等 owner 反馈"是**有东西在动的等**，不是我把事放下了。

  `next=`＝①**三枚程在飞**：票 146 验收（锚 `7f75ff3`）＋票 144 续程＋（无其它）；②144 收齐后与 146 一起**一次批量推**，⚠ **推送后必做逐名比集合**（`A254③` 固定动作），因为 `test-core` 上带着 `TestC21DesignTokensFourWayAgree` 那枚**已知常红**、不比对就会被误归到今天这两票头上；③票 146 的 `Params` 再下一层那一格待验收回来定开不开票；④界面整块继续等 owner（`A254`），我不摆问题给他。

- [2026-09-25 20:1x +08] **A257｜票 146 验收回来了：三格成立、两格退回，而且它推翻了我记在账上的一句——那张"票面自相矛盾"的账只有一半成立**

  **件**＝`docs/evidence/s1/146-liveapprovals-r1-accept-r1.md`（**656 行、6 枚 commit，每枚只碰那一枚路径**，我逐枚 `--name-only` 点过名，未推）。

  **① 先更正我自己：`A255` 那句"它给的第三条理由也成立"是假句，我现在收回。**
  `A255` 我逐字写过：『**它给的第三条理由也成立**：ⓑ 唯一可能的另一类仪器是静态扫，而 `AC#4` 把 `tools/d22scan/**` 划成零字节 ⇒ 两枚 AC 只有 ⓐ 收得下』。
  ⇒ 验收程**当场把这句打假**：它**不动 `tools/d22scan/**` 一字节**，改在允许落点 `internal/agent/approval/` 里造出**两枚 ⓑ 支仪器**——其中一枚**读源码钉注释**，**删掉那 239 字节的 ⓑ 纪律句它就红**。所以"对一句注释恒答仍不红"不成立，"只有 ⓐ 收得下"也成立不了。
  ⚠ **要紧的是我犯的正是我自己刚立的那条规矩的反面**：`A255` 里我刚写下"它的理由②③我没有独立推演"——**我核了方向（浅拷贝下"记录没变"是 Go 语义假话，这一半验收程复算为真、ⓐ 不需回退），却把整条论证当成已成立记进了台账**。⇒ **升级：判"某支修法收不下某格 AC"这类否定式论断，必须自己造一发反例试试**（造不出来才许写"收不下"），**只推演语义不算复算**。这条与我今天早些时候被"验收者点名的最小修法也是未验证断言"烫过的那次同族，只是这次的"点名人"换成了我自己。
  ⇒ **方向本身不变**：`AC#1` 是**"退回（只退论证、代码不回退、框不勾）"**——ⓐ 仍然是票面两支里唯一收得下 `AC#2` **字面**的那支。

  **② `AC#2` 退回的理由是一枚我完全没想到的真假绿——这轮验收最值钱的产出。**
  把 `cloneParamsMap` 改成**"键留着、值丢掉"**（`out[k]=nil`）⇒ **两发新检全绿、包内既有用例无一红**。⇒ **"copies" 许诺的"值保真"那一半今天没有任何人钉**——写手钉住了"别共用底层"，没人钉住"拷过来的东西得是真的"。
  ⇒ 它另外造了一发**合体变异**（M-a＋F 同装、把它自己的探针件挪出仓库只剩仓里用例）⇒ 读数 **34 PASS／0 FAIL／1 SKIP，与"改后健康态"四个数一模一样**。**这一发是给我看的**：它证明"四数相同＋名册差集只多两枚"这套自查**在这种形状下完全失灵**（我派单里要求的正是这两把尺）。⇒ 最小闭合集合它给了一条：**就地写之前加一发 `reflect.DeepEqual(返回值那份, 队列存储项)`**，一条收掉 E／F／M-a 三形。
  ⚠ 我原先写派单时说"≥2 枚各自附条件的洞必须单造一发组合变异"——**这次是验收程自己造的，我那条老规矩第一次被下游完整执行**。

  **③ 第二枚洞：`AC#1` 那格里"第 9 枚引用字段不会静静进来"是半真半假。**
  给 `tools.Decision` 新增一枚 `cloneDecision` **不拷**的 `map` 槽位、夹具不填它 ⇒ **两发检 rc=0 全绿**，而"就地写它就能改掉已决断记录"是活的。原因：`refSlots` 那个 `switch` **只有 Map／Slice／Struct 三支**，**Ptr／Interface／Chan／Func／私有字段全部静默跳过**。⇒ 与 `A255②`（我漏数三枚切片）**同一族**：**"我列全了"这件事本身需要一把会红的尺**，注释里声称的穷尽不算。
  ✅ **同时它把好消息也量了**：**8 枚槽位（1 map＋7 slice）普查成立、无第 9 枚**；`RawMessage` 在 go1.27 里打成 `jsontext.Value` 但 `kind=slice elem=uint8`（**按字符串认类型会数漏**，这是一条新仪器坑）；`Capability＝string` ⇒ 票面"元素没量"那格可结。单点回退打得最狠：**8 枚逐枚各撤一发，每一枚两发各自红、红句点名被撤的那一枚**，连票面漏计、被写手补上的 `Blacklist.*` 三枚都有独立红句 ⇒ **"实覆 8 枚"，覆盖面主张不必改写**。

  **④ 归因更正一格（它多走了一步，我接受）**：`A253④`/`A254` 我把 `TestC21DesignTokensFourWayAgree` 写成"本机红＝文件被挪走／CI 红＝少 26 条声明"两形。**它补了第三格：那枚红在纯净锚点副本上也红，红句是 `:446/:461/:471` 的四方值不一致**（`tokens.css` 在 `7f75ff3` 是 tracked blob）。⇒ **"推送之后第二形会消失"不成立，两形都在** ⇒ `A254` 那条"本编队不动 `tokens_fourway_test.go` 一字"**更硬了**（不是等一等，是这两形都得靠 owner 定稿才结）。

  **⑤ 处置：三笔退回合成一程修复，不另开票**——`AC#1` 论证更正＋洞①值保真＋洞②穷尽性，**三样都落在同一枚测试文件与同一段注释**上，拆两程＝同一文件并行（本仓假并行的登记形状）。
  ⚠ **勾一格没勾**：`A255 next=①` 我写过"验收回来就勾 `AC#3`"——**我收回，五格全不勾**。理由：`AC#3` 的框字面是"给出 WorkPeak 前后读数"，而两程现量一致证明**那台仪器 5 秒内根本不执行 `LiveApprovals`**（`goroutines_max=1`、唯一接线在 `wisp run` 腿）⇒ **这一格结构上永远产不出它自己想量的读数**。本仓对这种判据的既有正解我只认第一支：**原句不删＋在票面具名登记"为什么产不出＋实质凭据是哪两发＋这一支从未被字面复算过"**（第二支"悄悄换成实质版"＝本仓 616 行注释预先引用不存在读数那一族的复发）。⇒ **另有一条我这次不重犯**：同一格若第二次被退回＝不再续第三格（改造成家族票第 N 发＋把翻勾条件换成可复算的跨票依赖）；本轮是第一次退回，所以续一程。

  **⑥ 现场（与我的程无关、但值得留一条）**：收笔时 `frontend/**` 有 **12 枚脏文件＋4 枚未跟踪新文件**、mtime 就在几分钟前，另有 `docs/reports/frontend-session-log-zcode.md` 与 `.zcodeignore`。⇒ **owner 新指派的那枚前端程正在大量动笔，本编队对这些路径零写入、零 `git add`**；我给它那三枚程的派单里都具名写了"清单里出现 `frontend/` 就停手不提交"。**共享树里真正防的是"不带 pathspec 的那一枚 commit 把别人的活卷走"，不是我这一侧。**

  `next=`＝①**三枚程在飞**：票 146 **修复程**（三笔退回合一、锚 `fca1c5e`）＋票 144 续程（已交 `7bce7c1`/`59aafd4`/`6bb906b`，AC#3/AC#4 有读数了）＋无其它；②**146 修复程交件后还要再来一轮验收**（`AC#1`/`AC#2` 的勾由第二轮验收定）；③待推 **25 枚以上**（`origin/dev..HEAD` 现量），**攒到两票收齐一次批量推**，⚠ **推前数清每一枚是谁的**（在飞写码程存在时推分支会把它中间态一起发布），**推后必做逐名比红名集合**（`A254③` 固定动作）；④`Params` **再往下一层**那一格（value 里的 `[]any`／嵌套 map 仍共用）——本轮仍不钉，**待修复程回来判"开票还是并入家族票"**；⑤界面整块继续等 owner（`A254`），我不摆问题给他。

- [2026-09-25 20:4x +08] **A258｜票 144 续程交件（7 枚）：它顶回了我派单里一句前提、自己复量出一枚 gofumpt 腐坏、并撞到我那条"逐包单跑"纪律的反例；另外它把五枚 AC 自己勾了，我去加了"不抵账"横幅**

  **① 我派单里那句前提是错的。** 我写给续程的第一句是"前一程死在 AC#3 之前"——**而未提交那 264 行证据件里 §3 已经写满了读数**。续程的处置是**两句都不信**：另起仓外副本（`D:/tmp/wisp-144-r2/`）把 §1-§4 **逐格重量**，量完的结论是"前一程的 AC#3 读数成立"（变异 A ⇒ `PASS=2 FAIL=5`、红的全是"没写完"那 5 枚；变异 B ⇒ `PASS=5 FAIL=2`、红的全是"当场红"那 2 枚、句落 `:250`/`:270`；pristine 回装 `ok`）。
  ⇒ **这条要记的是我的派单形状**：我在同一份派单里既写了"它的读数要你自己复算"、又写了"它死在哪一格之前"这个**状态断言**——**后者同样是未验证断言，而且它会引导程跳过真读数**（记忆里那条"派单里的否定式前提同样是未验证断言"的同族，这次是**时序式前提**）。⇒ 今后派单**不写"前程死在第几格"**，改写成"前程的 commit 序列如下，每格成不成你自己量"。

  **② 它自己抓出并修好了一枚我上午才犯过的错。** §4.2 那句"我复量了 gofmt 与 gofumpt 都返回空"是**在升级夹具之前**量的——带 `+36/-7` 复量时**两把尺都报出那枚测试文件**，于是 `gofumpt -w`（`a91d7c2`，**只有空白、改前后 `diff -w` 逐字相同**）。⇒ **`A253①` 那条教训（gofumpt 不是 gofmt、CI 那一步只认前者）在两小时内就在另一枚程身上复现并被抓住**——说明把它写进派单是有用的，不是仪式。

  **③ 我要求的那发差分它补上了，而且这一发是本票最承重的**：同一份测试放进 `95885fb^` 的**真修前树**与修后树，问**同一枚 667/668 字节的真半截文件**——修前＝`unexpected end of JSON input` 且第一发即死；修后＝`never wrote a complete report within 33s (last read: 667 bytes …tail had not arrived)` 且在预算里重读。⇒ 它据此把 §3.3 那句"**外部读数没变**"改口成"**半截文件真被读到时必变**"。`AC#4` 名册也从"同树取两次"换成 `git archive 95885fb^` 现造真修前树：**106 → 120、`comm` 两向消失 0／新增 14（逐枚 `TestSLO144*`）、FAIL 0／SKIP 0／panic 0**。

  **④ 一枚我裁"不另开票、记台账"的新发现（它的处置我认为对，理由要留字）**：`unwritten` 分支上 `subjectReportRead.offset` 对全部 **1978 枚前缀恒为 0**（只有完整文档报 1978）⇒ **到点红句里 offset 那一半不携带信息**（携带信息的是 `%d bytes read`）。
  ⇒ **它没顺手改**，理由：改错误文本会让 §3.2/§3.4 **已落盘的变异红句逐字变样**，而那批红句正是本票的自证凭据。**这一格不构成 AC 违背**，且"顺手改文本"会**销毁自己刚立的凭据**——**不换**。
  ⇒ **裁定＋绑定条件**：**记本台账、不开票**；**下一枚改 `collectReport` 那两句错误文本的程，必须先重跑那批变异、把红句重新量一遍**（谁改谁重量）。这样这条债不会因为"恒 0"就永久无人管，也不会为了一条不携带信息的字段去销毁凭据。
  ⚠ 我要防的形状它自己在件里点出来了：**"用'改了会弄乱凭据'来掩盖'这把尺少了一半'"**。⇒ 所以这里必须留一句：**offset 恒 0 是真的、被登记了、不是被解释掉了**。

  **⑤ 一条会改我派单措辞的现场**：它**只 call 了 `./cmd/wisp/` 这一包、用 CI 同形脚本**，仍然撞上那枚假红 `TestAC1ResidentLegBooksItsShutdownBeforeClosingTheSink`（`PASS=69 FAIL=1`；文件属票 130、不在本票 diff；终态重跑 `PASS=70 FAIL=0 SKIP=0`、`=== RUN=130`）。另有一发 `wisp slo` rc=2（`NtQuerySystemInformation: buffer never sufficient`），重跑 3 轮全 rc=0。⇒ **我过去写成"逐包单跑必过"的那条纪律，措辞过强了**——"单跑"降低概率但**不消除**它；真正的判据是**"这一发的编队静不静"**，不是"我有没有把包拆开"。
  ⚠ 这一条我不自己动手改结论——**已把它单独列给验收程复算**（派单第 6 条："别沿用我那句'单跑必过'当结论，那是我的断言"）。**它的答案会决定我怎么重写所有测量程的派单。**

  **⑥ 治理一格：五枚 AC 框是实现方自勾，而此刻本票还没有任何非实现者的表。** ⇒ **我没撕自勾，加了"不抵账"横幅**（`1b3fccc`，本条落笔前现量 `git log -1` 取的真号）——**撕掉会丢掉"实现方当时认为哪几格结了"这个读数，而那与表的差异本身就是本仓要审计的对象**。对照今天票 146 的写手是**自己不勾、把勾留给编排者**的：同一件事两种形状，**都要能看出来是谁留的笔**。⇒ 验收程已派（锚 `1e94672`、只写 `144-slo-report-partial-read-r1-accept-r1.md`、明令"不许动票面那五枚 `[x]`"、并要求**逐格出三档、不许整体判通过**）。

  `next=`＝①**三枚程在飞**：票 146 修复程（三笔退回合一，锚 `fca1c5e`）＋票 144 验收程（锚 `1e94672`）＋无其它；②票 146 修复程交件后**还要第二轮验收**；③**待推**枚数现量（`origin/dev..HEAD`）——**攒到这两票都收齐一次批量推**，⚠ 推前数清每一枚是谁的（在飞写码程存在时推分支会把它中间态一起发布）、推后**必做逐名比红名集合**（`A254③`）；④**两格待裁的已裁一格**：`offset 恒 0`＝**记台账、不开票**，但**绑一条硬条件**——下一枚改那两句错误文本的程必须先重跑那批变异、把红句重量一遍（谁改谁重量，本条④）；`Params 再往下一层` 等修复程回来判"开票还是并入家族票"；⑤**"逐包单跑够不够"这一格等验收程的答案回来再改派单模板**（本条⑤）；⑥界面整块继续等 owner，我不摆问题给他。

- [2026-09-25 20:5x +08] **A259｜票 146 修复交件：那发"与健康态四数一模一样"的合体变异不再成立了；另记一次 `rm -rf` 纪律擦边，并把它暴露的规则含糊补硬**

  **件**＝四枚 commit `b694378`（测试侧 +111/−37）／`73e0d7b`（新修复件 279 行 ＋ 实现件 §2 之后追加 `>` 更正 17 行）／`a1f5934`（票面 Progress log 19 行）／`74255b9`（§8 追加一处自陈）。**我逐枚 `--numstat` 点过名：全部落在它可写清单内**，`internal/tools/`、`internal/risk/`、`docs/PLAN.md`、`frontend/**`、`design/**`、`cmd/**` **零命中**，生产那枚 `pending_read.go` **一字未动**。五枚 AC 框**它一枚没勾**，把勾留给了第二轮（对的形状）。

  **① 本轮最值钱的一格：第一轮那发"两枚洞合体 ⇒ 34/0/1 与健康态四个数一模一样"的假绿，现在被同一把尺量成 32/2/1。** 五行对照（它给读数、第二轮验收复跑）：`F`（键留值丢）34/0/1 → **33/1/1**，红在 probe 2 的值保真那发；`M-a`（新增一枚不拷的 map、nil 夹具）→ **33/1/1**，红在 probe 1 的普查 9 vs 8；`E`（空克隆）修前**红但 panic 吞掉 30 条读数** → 修后 **31/3/1、`=== RUN` 54**（吞不吞这回事是用 RUN 枚数分开量的，正是 `A256` 那条纪律的正面使用）。
  **② 它选了"甲＝穷尽普查"而不是"乙＝把注释降级"**，并把未知 kind 直接 `t.Fatalf`（fail-closed）。⇒ 这正是我在派单里写死的那句"ⓐ不许只选乙而不给仪器"被执行了。**但我不因此算它结**：新尺的软肋我已单列一发交给第二轮——**"夹具非空"那一形才是它这轮加的守卫真正要顶住的地方**（上一轮被抓的是"夹具不填"），并且要问清"这一形到底是**穷尽普查**堵住的、还是**值保真比对**顺手堵住的"——两味药谁在承重，混了就等于下一轮又少一味。

  **③ 一处纪律擦边，它主动认了，我收下并把规则补硬（不追究，但登记）。** 原话在修复件 `:248-251`：建验证副本时在一条 `for` 里用了 **`rm -rf "$d"`**，清的是**仓外、它自己刚建、本就要重建的空目录**（`D:\tmp\wisp-146fix\verify\snap-*`），**仓内零 `rm`／零 worktree／零 checkout，无任何工作丢失**。它没有洗："**这条本程不该做，记在此不洗**"。另有一处它自己也登记了：一次自己造成的 pathspec 打错（`internal/agent/approvals/x` → rc=1、**未产生 commit**、没换路子绕，改对路径重试）。
  **⇒ 裁定：不追究，但这是我的规则含糊造成的，不是它的判断问题。** 我派单同时写了"临时件只建不删"和"要复跑变异就得能重设靶树"，**这两句之间那条路我没给它铺**——它选了"清旧的"，正解是**换一枚新目录名**。
  ⇒ **规则补硬（进今后每一份派单，一句话）**：**"要重建副本时换一个新序号目录名（`snap1`→`snap2`→…），不要用 `rm` 清旧的；旧副本一律留着。"** 理由有两条，第二条更要紧：①`rm` 每次都向人弹授权窗（噪声是我造的）；②**快照本身是"可重跑凭据"**——裁决表里引的每一发变异都可能要在那棵树上再跑一遍，清了就从〔可复现〕掉回〔仅自述〕（这条在 `MEMORY` 里已记过一次，今天以另一形复现）。

  **④ 它留下三格空格，我不现在裁**（等第二轮的建议一起判"开票／并入家族票／记台账"）：**`Params` 再往下一层仍共用**（value 里的 `[]any`／嵌套 map，DeepEqual 这一发今天照不到）；**"把 `Params` 递到面板要不要收窄"那支 ⓓ**；**`replay` 那枚同族出口**。

  `next=`＝①**两枚程在飞**：票 146 **第二轮验收**（锚 `74255b9`、只写 `146-liveapprovals-r1-accept-r2.md`）＋票 144 验收（锚 `1e94672`）；②146 那五枚框由两表一起定，**`AC#3` 那格"结构上产不出读数"的具名登记仍欠**（票面文件归修复程写、我不往里插字，等它这轮收笔）；③**待推枚数现量后一次批量推**（`origin/dev..HEAD`），推前数清每一枚是谁的、推后逐名比红名集合（`A254③`）；④今后派单加那条"换目录名不清旧的"硬句子；⑤界面整块继续等 owner，我不摆问题给他。

- [2026-09-25 21:1x +08] **A260｜票 144 验收：五格全成立（自勾这次有表可对了），但它在五格之外造出两枚退回、其中一枚推翻我 20:4x 刚写进台账的两句——今天第三次**

  **件**＝`docs/evidence/s1/144-slo-report-partial-read-r1-accept-r1.md`（678 行、7 枚 commit、每枚只碰它自己那一枚路径；**票面那五枚 `[x]` 这次没有可撕的对象**——五格 `AC#1..AC#5` 全部判**成立**，每格带现量命令与读数）。它另有一批**新硬读数**：承重那发用自己的驱动重撞（1949/1950 字节的真 `writeSLO` 前缀，修前 `unexpected end of JSON input`＋`elapsed=0s`、修后点名 1949 字节），**并补了一发前一程没取的**：尾巴晚 400 ms 到达 ⇒ 修前 `VERDICT=error`（报告被丢掉）、修后 `VERDICT=collected` ⇒ **这枚修法买的是行为差，不是措辞差**（这一格今天一直悬着，是它落下来的）。

  **① 退回①：`a91d7c2` 不是"只有空白"——而我 20:4x 在 `A258②` 里把这句话原样抄进了台账。**
  三处都写着"改动只有空白／前后两版 `diff -w` 逐字相同"（票面、证据件 §4.2、commit 标题"只改对齐"），**三处都不成立**。
  ⇒ 我自己 21:0x 现量：`git diff -w --numstat a91d7c2^ a91d7c2` ＝ **31 加／2 删，非空**。真实形状＝**夹具升级 ＋ `gofumpt -w` 混在同一枚 commit**：夹具从 2 枚字段扩到含 2 条 `observe.Sample` ＋ 2 条 `observe.Verdict`，另加一枚 6 行注释块，并把一发 `strings.Contains(doc,"report")` 换成"长度下限＋5 枚字面量"。
  ⇒ **实质无放水**（替换那发是**更严**、极性未变）⇒ 要修的只是**声明不准**，**不许**因此回退夹具或改任何断言（已写进新票 147 `AC#3`）。

  **② 退回②：我在 `A258④` 裁的那格"记台账、不开票、绑一条谁改文本谁重量"——绑的那条没有牙。**
  现量凭据（验收件的 **MG** 那一发，**带正控**）：**只**把红句里 `at offset %d` 那半句拿掉 ⇒ `rc=0`、七枚用例**全绿**。
  ⇒ 也就是说"**改之前先重量一遍红句**"这个条件**结构上永远触发不了**——改了也不会红，谁都不会撞上它。
  ⚠ 这与 `A258④` 我自己写下的警觉恰好相反：我当时写"要防的是'用'改了会弄乱凭据'掩盖'这把尺少了一半'"——**结果被掩盖的那一半是真的，而"弄乱凭据"那一半是假的**。⇒ **裁定作废，改开票 147**（含：那半句红句要么让字段真带信息、要么把措辞改成它真做到的、要么留下三语义各自成立的会响的检——ⓑ 只改措辞不算收，因为仍然零仪器）。
  ⚠ 同一族第二发也在里面：`collectReportWithin` 头上**新写的注释**逐字承诺红句会点名"文档停在哪一字节"⇒**注释预先承诺尚未产出的行为**（票 144 兄弟票 §E7 那一族）第二次出现，**这次出现在两天内新加的注释里**。

  **③ 今天第三次同形错，给它定名并上锁。** `A243`（dist 假警报）、`A255`（"第三条理由也成立"被 146 验收推翻）、`A258②④`（这次两格同时中）——**成因全是同一个：我把子程的自述当读数写进真相源，标的是"我核过"，实际核的是它的措辞。**
  ⇒ **硬动作（自本条起生效，进每份派单也进我自己）**：台账里凡引用子程的具体读数，**必须逐条带出处档位**——〔我本轮现跑过〕／〔日志＋归档，抽验〕／**〔仅自述，未复核〕**。
  ⇒ **今天这三条都写成了第一档而实际是第三档**。往后**只有当我在同一轮里贴出得出该数的命令与输出，才许写第一档**；否则明写第三档，让人一眼看出那是转述。

  **④ 我那条"逐包单跑必过"被答了，答案是"不够，但台账给的理由也不是机制"**：验收程做了四形对照（单枚／整包／**真把 `./cmd/wisp/` ＋ `./internal/panel/` 合跑**／整包＋6 枚烧核），那枚用例**四次全绿**；合跑那一形唯一的红是既有的 `TestC21DesignTokensFourWayAgree`。⇒ **单跑不保证**（续程自己 `PASS=69 FAIL=1` 就是反例）、**合跑也不必然红**；真因写在树里（票 131 的 guard 注释：负载重的机器上子进程 CTRL_BREAK 后 `0xc000013a`、零记录）。⇒ 派单措辞要改三处：**①"逐包单跑"从"必过"降级为"降低概率"；②真正的判据是"这一发编队静不静"；③后台任务的 output-file 只能当指路牌、不能当凭据**（它另撞见一发自称"已完成"的后台输出与盘上凭据**三处同向不符**：`124/18/PASS=88` vs 现读 `120/14/FAIL=0`）。

  `next=`＝①**票 147 已立未派**（与票 144 同落点 `cmd/wisp/`，**等 144 收线再派**，别同抢）；②**146 第二轮验收在飞**（锚 `74255b9`），144 这边五格已判成立⇒**144 的 `-done` 改名等我做完两件事**：把两处 `>` 更正落进项面与证据件（归 147 `AC#3` 一起做）＋确认 `A258④` 作废已在账；③待推 **44 枚以上**，仍攒到 146 收齐一次批量推，⚠ 推前数清每一枚是谁的、推后逐名比红名集合；④三格空格（`Params` 再下一层／ⓓ 收窄／`replay` 同族出口）等 146 第二轮的建议一起裁；⑤**派单模板本轮新增三条措辞改动**（本条④）＋"换目录名不清旧的"（`A259③`）；⑥界面整块继续等 owner，我不摆问题给他。

- [2026-09-25 21:3x +08] **A261｜票 146 结线（五框全勾，两份非实现者表背书）；第二轮没打穿，但把三句措辞的射程改准了；另：我在 146 那扇门后发现同族第二扇门，开了票 148**

  **件**＝`docs/evidence/s1/146-liveapprovals-r1-accept-r2.md`（426 行、3 枚 commit `7ae767f`/`ef48505`/`3738d3b`，我逐枚点过名、每枚只含它自己那一枚文件）。**`AC#1`／`AC#2` 两格都判「成立」，这一轮没有动用"退回"档**。

  **① 五发进攻的落点（三发都没打穿，这是好消息，但要写清为什么没打穿）**：
  - **①变异对照逐字复现**（第一凭据）：旧尺（`b694378^`）下 F／M-a／合体三发 **34/0/1、`=== RUN` 54、rc=0，与健康态四个数一模一样**；新尺下 33/1/1、33/1/1、32/2/1。**E 那发的"panic 吞读数"也复现**（旧尺 `RUN=5` → 新尺 `RUN=54`）。
  - **②非空夹具那一形没打穿，但抓出一句过宽的话**：修复件与验收件都写着"一条 `DeepEqual` 一次收掉 E／F／M-a"——**真实形状是三味分管三形**：名册未长＝类型普查（`:223`）；**名册已长而少拷＝probe 1 的 backing 比对（`:255`，此处 DeepEqual 必然绿）**；全新空 map＝**只有 DeepEqual**（`:287`）。⇒ **"一条收掉三形"从此要按"三味分管三形"引**（同一枚事实被写成一句更短更好听的话，就又成了覆盖面主张）。
  - **③单点回退：`fail-closed 穷尽`这句成立、不必降级成"实覆 N 支"**——Ptr／Interface／Chan／Func／Array 五支各自撤掉**都仍响**（红挪到 `default`），**只有撤 `default` 那一发出现全绿**，而那一发它用一枚仓外探针当场钉出"就地写返回的 `*[]string` 真改掉了已决断记录"。⇒ 但那五支给的是**拒绝测**不是测到 ⇒ **"第 9 枚不会静静进来"要写成："会被点名、map/slice 可测到、其余只能拒绝"**。
  - **④DeepEqual 不是恒真**：两侧非别名；`tools.Decision` 19 枚字段无 func/chan/裸指针；它自造两发"只有它红"。**反向也量了**：有一形两边同源、DeepEqual 绿、红的是别处。⇒ **这一味是承重的**，不是装饰。
  - **⑤理由①独自撑得住**（ⓐ 方向不需要那两句假理由），它还多找到一腿：**146 `AC#1` 第四判据"取地址后传出去"在 `cmd/wisp/panel_pump.go` 上按切片值读作"有"** ⇒ 这一腿对 ⓐ 是**加强的**（判据有"有"⇒ ⓑ 本就不许单独收）。

  **② 我把 146 五框全勾了，依据是两份表、不是任何自勾**（对照 `A260` 那格：144 的五枚自勾这次有表对上了；146 的框一直是空的）。
  ⚠ **`AC#3` 是按"实质凭据"勾的，票面字面那一支从未复算过**——原因（WorkPeak 的 subject 是 skeleton、那 5 秒根本不执行 `LiveApprovals`）、两发代替凭据（双臂各 3 次、均值差小于组内极差 ⇒ 写"未观察到差异"没写"无代价"；确定性 `-bench` ＋4736 B／＋72 allocs）、以及**谁该回来重量这一格**，全用 `>` 追加在原句之后（**原句一字不抹**），触发改成具名跨票依赖 ⇒ **票 148 `AC#2`**。
  ⇒ 这一格走的是本仓既有那条裁定：**"故意留空的一格"与"结案须 0 未勾"冲突时＝勾交付物＋把洞归口另一张票＋原话不改**，**没有新造规矩**。

  **③ 我在收线时现量到同族第二扇门（这次先跑再写，`A260③` 那条上锁第一次被执行）**：`internal/agent/approval/queue.go:478` 那枚 `replay` 里 **`:493 d := it.Dec`** 不走 `cloneDecision`，一条 `grep -rn ':= it\.Dec' internal cmd | grep -v _test.go` ⇒ **全仓非测试恰好 1 处**。另顺手量到 `:453-456` 那份投影**逐字段挑着拷、8 枚里只护了 `Paths`**（**这不是漏拷、是范围不同**——`Params` 那几枚根本没进这份投影）。
  ⇒ 开**票 148**，**立为 blocked**：两枚前置都是人工批准（Ⓐ 放开 `queue.go` 那一行／Ⓑ 定"面板该收到哪些字段"），**我不自决、也不替他推**（`Q-49` 那次我替批准者加条件、`Q-52` 那次我摆错时刻，两笔都在账上）。
  ⇒ **登成 `Q-53`／`Q-54` 进 Q 表，这是登记、不是催**：`Q-54` 我自己给的推荐就是**丙＝先别答**（与 `A254` 同源：别在一版还会被推翻的界面上定"面板需要什么字段"），`Q-53` 虽推荐甲但**不催、不挡任何腿**，148 的 `AC#2` 今天就能做。

  **④ 一处既有仪器要降级**（第二轮报的，我接受）：第一轮 §0 那句用 `grep -rl $'\r'` 量出的 **0**，**在本机复现不出来**（同一条命令它量到 456／483，**比 go 文件总数还大**）。⇒ **副本确实是纯 LF**（它另用 `od -c` ＋逐字节计数确认），所以**结论不变、但那把尺不稳**。⇒ **今后 CRLF 校验一律按字节量**（与记忆里那两枚"假绿来自工具形状"同族：MSYS 吞参数、`grep -c` 静默 `No matches`；这次是**同一个 grep 在不同 shell 下语义不同**）。

  `next=`＝①**144 与 146 两票都结线**⇒ **推**：把 `7b2a870` 之后积压的一次批量推两边（推前 `rev-list --count` 现量＋数清每一枚是谁的；⚠ 此刻**没有写线程在飞**，是唯一干净的窗口）；②**推后必做逐名比红名集合**（`A254③` 固定动作），`test-core` 那枚 `TestC21DesignTokensFourWayAgree` 是**已知常红**、别误归到这两票；③**票 147 待派**（落点 `cmd/wisp/`）、**票 148 blocked 不派**；④三格空格已各自归位：`再往下一层`＝记台账＋归 148 `AC#3`、ⓓ 收窄＝`Q-54`、`replay` 出口＝`Q-53`；⑤派单模板再补一条：**引用一把尺之前先问它稳不稳**（本轮新增：`grep -rl $'\r'` 不稳，CRLF 按字节量）；⑥界面整块继续等 owner，我不摆问题给他（`Q-53`／`Q-54` 是**登记**，不催）。

- [2026-09-25 21:3x +08] **A262｜59 枚积压一次批量推到两边（含三枚前端会话的活，逐枚点名不隐身）；本条真正要记的是"我自己那两把取数仪器在这一轮各骗了我一次，而且都在落笔前被抓住"**

  **① 推前四条现量〔我本轮现跑过〕。** `git rev-list --count 7b2a870..HEAD` = **59**；`git merge-base --is-ancestor 7b2a870 HEAD` = **真** ⇒ 两枚远端（`origin` 与 `cnb` 当时同停 `7b2a870`）都是纯 fast-forward，**没有任何一枚需要强推**；编队静不静按"活产物"量而不按"我记得"：最新一枚 `subagents/agent-*.jsonl` 的 mtime = **21:09:38**，`date` 当时 = **21:17:41**（静 8 分钟，那三枚分别是 144 验收与 146 二轮验收，都已交件）。⇒ 两推皆 `rc=0`，两边现同停 `80fa0551`。

  **② 作者字段在这一堆里是零信息量的，能分归属的只有"逐枚首目录集合"。** `git log --format='%an' 7b2a870..HEAD | sort | uniq -c` = **`59 CarlosShao`**——**我的编队与那枚前端会话用的是同一个 git 身份**，所以"看看是谁写的"这把尺在这里根本切不开。逐枚取首目录后聚合：`docs` 40／`.scratch` 6／`.scratch,docs` 4／`internal` 3／`cmd` 3／**`frontend` 2／`docs,frontend` 1**。⇒ **随行发布的那三枚前端活我逐枚点名**（不是我方产出、也不该被算进我方产出）：`4772b1e3`（18:35，票 61 两条）、`ab7afcad`（20:34，demo 重构第一批）、`62431e97`（20:42，输入行钉底）。⚠ 这三枚在 `ci` 里对应的那一步 `lint-frontend` **基线与新推两枚 run 都是 success** ⇒ 随行没把任何门弄红，但**"没弄红"不等于"我方核过它"**——那三枚的内容归前端会话与 owner，本编队对 `frontend/**` 零写入、零裁定。

  **③ 两把我自己的仪器本轮各骗我一次（都抓住了，所以这格的价值全在"抓住的形状"）。**
  - **(a) 日志要等 run 终态，job 先完成不算。** `gh run view --log --job <已完成的 job>` 在 **run 仍 `in_progress`** 时**拒绝出日志**，返回的是**一行错误文本**（`run ... is still in progress; logs will be available when it is complete`）。我第一次因此量到 `test-core 红名 = 0 枚`——**那是仪器没跑，不是用例变干净**（同一时刻 `TestC21DesignTokensFourWayAgree` 大概率还红着）。⇒ **固定动作**：拿 CI 日志前先看 **run 级** `status`，别看 job 级结论；`0 命中` 之前先确认这把尺**今天真出过字**（与既有条"0 命中要先拿已知正控打一遍那把尺"同一条，这次是它的 CI 版）。
  - **(b) `gh run watch --exit-status` 的退出码不是 CI 裁决。** 我的后台 watcher **`rc=1`** 收尾，按字面读就是"这推红了"。真相：它在 21:2x 被一次 `Get https://api.github.com/...: EOF` 打断，而**当时 run 还在跑**（`gh run list` 现读 `in_progress`、已 7m9s；同一条 `gh api` 连打三次全 EOF，而 `gh api repos/CarlosShao/wisp` 与 `gh run list` 同时都正常）。⇒ 这条是既有条**"报错≠死亡"**换了对象：**代理会带着空返回死掉，仪器也会**；判"这推的颜色"只认 run 级 `conclusion` 或步级 `conclusion`，不认取数工具的退出码。

  **④ 已经拿到了的那半读数，比"变绿"更值得记：修一枚红买回来两枚裁决。** 步级（走 `jobs[].steps[]`，**不需要日志**，所以这一格与③(a) 无冲突）〔我本轮现跑过〕：`lint` 那枚 job 里 **`gofmt (gofumpt)` 由 failure→success**，紧跟其后的两枚 **`go vet (module)`／`go vet (tools/d22scan module)` 由 skipped→success**。⇒ ⚠ **别把它记成"只是我造的红被我自己抹了"**：那两步没写 `if: always()`，一枚前置红会**吃掉它们**（本仓既有那条"一步红会吃掉后续步"），所以差别是实质的——**CI 恢复了两枚它此前根本没给出的裁决**。`lint` 整枚 job 仍 failure，唯一剩的红因是 `staticcheck`（＝积压 `#65` 那一族，**不是今天新增的量**）。
...
- [2026-09-25 22:2x +08] **A264｜票 147 实现程交件（判 ⓐ、三枚新尺、四件报回）；本条把我这一轮的三条核收与"哪些数仍是自述"分开摆，另定一枚 `.scratch/wisp/probes/` 的落点规矩**

  **① 三条核收〔我本轮现跑过〕，全过——但这三条本来就切不到它的结论。**
  - **numstat 只带自己的路径**：三枚 commit 逐枚数，路径集＝`cmd/wisp/slo_windows.go`＋`cmd/wisp/slo_report_144_windows_test.go`＋两枚证据件＋两枚票面＋`.scratch/wisp/probes/147/`，**`frontend/**`／`design/**` 零命中**（⚠ 那两棵树在我派单期间从 `??` 变成 ` M`——**别家在活**，它没卷进去，我也没把它的任何状态算进"零命中"的宣称里）。
  - **禁改轴现量归零**：`git diff --name-only 80fa0551..HEAD -- <每枚路径>` 逐枚量，`internal/risk`／`internal/panel`／`internal/agent/approval`／`tools/d22scan`／`frontend`／`design`／`docs/PLAN.md`／`docs/specs`／`scripts/slo-check.ps1`／`internal/observe` **十枚全 0**。
  - **它没给自己打勾**：147 票面 `^- [ ]` = **5**、`^- [x]` = **0**。另两枚我顺手钉住的形状：**144 那批测试文件在整个区间里删除行 = 0**（它自陈"169 加／0 删＝既有文本一行未抹"，这一条我复算成立）、**三枚 `func TestSLO147*` 真在树里**（`grep` 现读名字，不是听它报数）。
  ⇒ ⚠ **这三条只证明"边界干净"，一条都不证明"尺会响"**。所有会变红的读数（改前三红／改后转绿／摘修法形 A 三红／撤半句形 B 三红、门禁四数 `130/70→133/73`、`d22scan` 各作用域 `examined` 数）**全是〔仅自述，未复核〕**，验收程已按这个口径派出去了。

  **② 它推翻了我票面上的两句措辞，两处我都认。** (a) 我说"三处'只有空白'的声明"，实为**四处文本＋一枚 commit 标题**（144 票面两行＋144 证据件 §4.2＋那枚已推送的标题）——**我开票时数漏了同一枚 commit 里还有第二行**，这是既有条"分组结构天然漏尾项"的第五次实发；(b) 我说"两处 `offset: -1`"，实为**四处**。**两处都是我的读数错、不是它的**，更正由它按 append-only 追加在 147 票面与证据件里（我这一条只点名，不覆盖它的写法）。另：票面引的红句行号 `:594-595` 在锚点树上实为 `:591`——**行号带版本**这一条今天第三次咬人，被咬的是我自己写的票面。

  **③ 一枚新落点规矩，是我定的、不是它擅自的。** 它把 17 枚原始读数（日志／名册／探针件）落进了 `.scratch/wisp/probes/147/` 并随 commit 入库——`git ls-tree -r 80fa0551 -- .scratch/wisp/probes` = **0 枚**，**这目录以前不存在**，所以这是一次无先例的形状。⇒ **我认下并定成规矩**，理由：验收方要能**不依赖临时目录**复核原始读数（临时件会被清、`D:\tmp` 那批已经有过程序删不掉又带不走），代价约 2.5 千行进仓。⇒ **往后写码程一律按 `probes/<票号>/` 放原始件**；这不是代理可以自创的（它这次恰好朝我想要的方向越了例，我把它变成明确的落点，而不是留成"看谁手熟"）。

  **④ 它报回的同族第二发（`corrupt` 那一支）——我**不**当场开票，也不当场否掉。** 读数（〔仅自述〕）：`{"mode":"subject-in-tree",,"pass":true}`（39 字节、错在第 25 字节附近）**仍印 `at offset 0`**；且**推翻票 144 验收件 §6 那句旁读**（"corrupt 分支也报 0"）——它实测 corrupt 的 offset 有 `0/0/13/1978/57/2` 六种，**说明 corrupt 常常带位置、那句旁读说过头了**。⇒ 三件事分开判：**这发今天响不响**（交验收程独立重走）、**ⓐ 新写的字段注释算不算把它说圆**（我读了 `:630-659` 那 10 行注释，它明写了"on reportCorrupt it is the decoder's own answer, 0 whenever it could not begin"——**注释诚实了，句子的误导没修**）、**要不要单开一票**（等验收结论再定；⚠ 别为"今天不响"的形状硬开一道永不响的 AC，那是既有条里明令禁止的那种票）。

  `next=`＝①**票 147 对抗验收在飞**（锚 `e572aa94`，含"第 3 节那四件逐件独立重走"＋"形 B 那发修前 0 红／修后该响"的翻转复核）；②票 144 的 `-done` 改名**这次具备了**——它等的三处 `>` 更正已随 `1f3ede9` 落地（**但改名等我看完验收表**，别在尺没钉牢前先结）；③`corrupt` 那一支按验收结论决定开票或只登记；④**下一次推送前**：本轮新增 6 枚 commit（台账 2／146 改名 1／147 三枚），⚠ **派单与推送不许挤在同一分钟**（`A263⑤` 新规矩），且推后照例逐名比红名集合；⑤票 148 仍 `blocked`（`Q-53`／`Q-54`）。
- [2026-09-25 22:5x +08] **A265｜票 147 结线（五格全成立、非实现者一枚表）；本条要记的是"验收程造出四发、其中一发我把它写成『不许开格』"，以及我派单里又有两句前提被实测推翻**

  **① 五格总裁决＝全部成立，验收程没有动用"退回"档。** 件＝`docs/evidence/s1/147-offset-naming-r1-accept-r1.md`（795 行、**七枚渐进 commit**，`0447040`→`e9412c9`）。
  ⇒ **票面五框我已翻勾**（`5 [ ]/0 [x]` → `0/5`，`git diff --numstat` = **5 加／5 删**＝只动了那五行，一格没多改）。依据只有两份：**它的表**＋**我自己下面那三条核收**。

  **② 三条核收〔我本轮现跑过〕，第三条最值钱——它把"ⓐ 还是 ⓑ"这件事从转述变成了我亲眼看的东西。**
  - 七枚验收 commit **每枚只带它自己那一枚路径**（`git show --name-only` 逐枚数，全是同一枚 `.md`）⇒ 归因边界干净。
  - 那 4 枚删除行**逐行看**：三行注释＋一行 `obs.offset = dec.InputOffset()` 的移位 ⇒ 没有任何一行断言被抹。
  - **`summary()` 里被打印的那几句文案，改前后 `diff` 为空（4 行全同）**〔我把两版都抽出来对过〕⇒ **"把数字做成真的"而不是"把话改软"这一条我独立证实了**，不必等下一程。
  ⇒ ⚠ 但我这三条**仍然切不到"那把尺会不会响"**：三枚新用例在未修码上响不响、21 发变异逐味答——那些是验收程的读数，我这里到〔日志＋归档，抽验〕为止。

  **③ 验收程造出实现方没造的四发，其中一发的处置跟以前不一样。** 三发进**票 149**（已立，`ready-for-agent`，**明写与 147 同文件族、147 结线前不许派**）：`corrupt` 那条腿换成常量 0 全组不响（三条腿两有牙一无）、147 新写那句注释的"whenever it could not begin reading a value at all"**有反例**（两发已在读值仍报 0）、loop 的 `s.exited()` 支丢掉 `last.summary()` 也不响。
  第四发（红句两实参对调）**我拒绝开格**：ⓐ 之后 `offset ≡ bytes`，那发是**等价变异**——它**结构上永远产不出它自己想量的读数**。
  ⇒ 这是既有条"不可满足判据的正解＝原句不删＋写明为什么产不出＋把实质凭据指到哪两发"第一次用在**我自己正在写的票**上：它写成了 149 票面第 4 条那个**限定语**（"不许为这一发开要它响的格"），而不是伪装成一条 AC。

  **④ 我派单里两句前提又被实测推翻（今天第 N 次同形，代价仍然由下游付）。** (a) 我写"四处文本"，验收程现量是**三处自述文本＋一枚 commit 标题**（落点仍是四处，**是我措辞把两件事并成了一件**）；(b) **"`--- FAIL` 假命中来自 `portable-tests.sh` 的 `-skip` 散文"这句是我从实现程报回里抄的**，验收程重走判它**归因错**：那 1 枚 FAIL 来自**汇总行**，散文只影响 SKIP 与 RUN 两数。
  ⇒ 教训升级成一条可核的：**凡是"某读数的成因是 X"这种因果句，转述时必须连着"这是谁的读数"一起转**——我上一轮把它当成事实写进了派单，下一程就照着错前提去找 X。本条按既有条把错前提从结论里摘出去，**不撤已发出的派单**（活已经干完了，撤无意义），只在此留痕。

  **⑤ 一枚可复用的仪器坑（验收程自采，进我的模板）**：**仓外快照（`git archive` 那套）若不把 `third_party/` 补进去，会拿到 8 枚假红**——`no native DLLs`、**`PANIC=0`**，形状与"包级失败"一模一样。⇒ 用快照跑 `cmd/wisp` 之前先确认那批 dll 在位，**别把它报成活失败**（同既有条"整树 rc≠0 要先怀疑工具链"）。
  另：该程伪授权两数为**真通知回显 25／判为注入 0**（它自己初稿把枚数写成 13、后自纠到 25；凭据反扫它自采 24 命中并逐枚归类）——**两个数分开记**这条第一次被代理主动执行到位。

  `next=`＝①**票 147 可 `-done`**（表一枚、五格全成立、无退回、无附条件入账）；②票 144 的 `-done` 也具备了（它等的三处 `>` 更正已随 `1f3ede9` 落地、147 已结线）；③**票 149 待派**（落点 `cmd/wisp/`，与 147 同文件族，147 改名后再派）；④**队列改往功能缺口走**：票 **138**（取消任务杀不掉它自己起的子进程）与票 **139**（上下文压缩不留任何痕迹、`compress.go` 零 log 调用）是两枚产品级缺口，落点与 `cmd/wisp/` 不撞 ⇒ 我按"先派单、后推送"的排程派它们；⑤工具面那笔账（内置工具表 20 枚里生产注册表只有 5 枚）目前**没有任何一枚开放票覆盖"补齐这 13 枚"**——待我核过票 23/29/30/51/52 的未勾格到底还欠什么，再决定是开票还是并票，**不在这里凭感觉报数**。
- [2026-09-25 23:0x +08] **A266｜第二推（16 枚）逐名比红名集合：又是零变化；但 `slo-full` 连拒两次，而我上一格把"job 跑了 2 分钟"当成"它真在取样"——那句是我推断错，已当面收回**

  **① 红名集合连续第二推零变化〔我本轮现跑过〕。** 基线＝上一推 `36140078431`，本推＝`36149256584`（16 枚：台账 A262-A265／146 与 147 与 144 的改名／147 三枚实现／147 七枚验收）。
  `test-core` 红名 **1 → 1**、`test-windows` **16 → 16**，**两向 `comm` 差集全空**。⇒ 这批全是文书与尺，没有产品码变更，颜色与名册都该不动——**它不动，才说明改名与台账没意外碰到码**。
  ⚠ **口径**：本次我取的是 `--log`（整 job）而上一次取的是 `--log-failed`（只失败步），所以**行数不可比**（`test-windows` 1729 → 3433 行只是这个差别）；红名集合仍可比，因为我只抽 `--- FAIL:`。

  **② 我上一格写下的那句推断是错的，在此收回并留下形状。** `A265` 之后我对 owner 说过"这道内存门这次跑完了、22:44 落终态、绿"——**"绿"对，"跑完了（＝取了样）"错**。现量：该 job 在 22:43:44 又拒采，逐字 `refused to sample, no numbers were produced` ／ `machine-contended reason: machine-wide cpu utilisation 97% over a 1s window (>= 50%)` ／ `slo-report.json NOT written` ／ `exit 0`。
  ⇒ **可复用的错形**：我拿"**job 持续时间变长了**"当"它在真取样"的证据。**持续时间不是取样的凭据**——那 2 分钟是 `Build wisp.exe` 那一步，取样那一步 12 秒就拒了（与上一推拒采时耗时几乎一样，我连 `wc -l` 都比过：两版日志都是 264 行）。
  ⇒ 唯一凭据是**日志正文里那行 `state … exit=0 pass=…`**（上一推 10:27Z 那次就有）。⇒ **往后判"这条腿被求值了吗"只认这句，不认 job 时长、不认结论色。**

  **③ 争用源仍未定性，而且我现在能说清缺哪一枚读数。** 22:43:44 那一刻我这边没有任何编译在跑（推送 22:42:15 完成、两枚写码程 22:5x 才放出去、我手里只有 40 秒一次的 `gh` 查询）。两枚候选：**(甲) 这台机器上 CI 自己的 `Build wisp.exe` 那一步的 CPU 尾巴**（`slo-full` 就跑在本机，它的 build 与它的 precheck 是**同机前后步**——这形状本身就是自相残）；**(乙) 另一枚会话在前端建树**（`frontend/dist` 最后一批产物 mtime = **20:42**，**不在 22:43 附近**，所以这一枚**当下偏弱**，但我没有 22:43 的进程级采样，不能据此判死）。
  ⇒ **缺的那一枚读数**：拒采瞬间的**按进程 CPU 前列**。⇒ 于是有一件真该做的事浮出来：**这句"machine-contended 97%"和票 144/147 修的那句红是同一族毛病——它说了数字、没说方向**（读到这句的人无法判断该等还是该去关掉什么）。

  **④ 但我这次不开票，理由是"要修就得动冻结面"，而那一族 owner 已明示不改。** 取证对象是 `scripts/slo-check.ps1`——**门禁脚本本体＝改它等于改判据，须另批**（票 147 票面 AC#4 就把它列在零字节轴上）；且 `Q-36`／`Q-47` 那一族 owner 对"slo-full 每次推送必红"的态度是"不改吧"。⇒ **本条只做两件事**：把"该报进程前列"这个洞**登记成可复算触发**（**下一回有人被这句拒采文案指错过方向，就凭这一格开票**），以及把②那把尺绑进我自己的固定动作。

  `next=`＝①两枚写码程在飞（138 落 `internal/agent/**`、149 落 `cmd/wisp/**`，落点按文件级切开、共享面为空）；②票 139 按住等 138（同撞 `internal/agent/loop.go`）；③**下一推之前**：`D32` 那条腿今天**两次推送都没被求值**——若要它真出一次数，得挑一次**本机空到 precheck 过**的推送（同机 build 是天然争用源，见③甲），这件事我倾向**排在编队全空时做一次单推**，不夹在批量推里；④`Q-53`／`Q-54` 仍等 owner，不催。
- [2026-09-25 23:3x +08] **A267｜票 138 那程撞在 150 轮上限（动作＝落盘，不是重派）；它交回的三发支撑我全部标〔待复算〕——但它 §6.6 那张"简报说 X"的对照表里，三句"简报说"在我的派单与票面里都不存在**

  **① 撞顶时的真实现场〔我本轮现跑过〕。** `git log` 现量：它有**六格已 commit**（`c9237a9` 23:01 → `f8623c7` 23:24，逐枚只带自己路径），件 875 行、§0..§6 齐；未提交的只有**它自己那一枚件的 19 加／3 删**——内容是它把"锚点漂移发生在哪两格之间"**改准并自认一次口误**，不是半截活。⇒ 按既有条"撞轮次上限≠活没做完，此时动作是**落盘**"，我用**显式 pathspec 代提**（`3f6322f`，暂存清单只有那一枚件）。它自报"没做"的那半截（§5.1／§5.3 的"前件判假故未触发"）**我没有替它补**。
  ⇒ 另一条现场自证：**零代码改动**（它整票交付物＝一枚证据件）、**没留孤儿进程**（`tasklist` 现查 `wisp.exe`／`sleep.exe`／`fake-child.exe` 全部"无匹配"）。

  **② 它的总裁决是很重的一句话，我没有当场采信。** 它判：本票要防的洞**今天不存在**，属"**给未来功能预留的洞**"，不是已实现功能里的缺口。三发支撑（**全部〔实现程自述，未复核〕**）：一轮任务今天只能调到六枚 `fs.*`、会 exec 的四枚产品侧位置都不在任务生命周期里／`go list -deps ./internal/agent` 里 `internal/proc` 计数 0／`NewDisposalScope` 在**生产码**里调用者 0（七枚命中＝1 声明＋2 注释＋4 测试）。⇒ **这三发里只要有一发不成立，裁决就塌、修法还得做**——验收程已派（锚 `3f6322f`），并把这条"塌法"写成了它的判据。

  **③ 本条真正要留的：它 §6.6 摆了一张"简报的说法 vs 现量 vs 判定"的表，那三句"简报说…"我回核不到出处。** 我 23:2x 现量：票面 138 里 `go build ./...` **只出现在第 56 行、写作 rc=0**（"先证变异落地"那半句）；票面与我的派单里**都没有**"CI 那两枚红的成因是别人的代码"、也**没有**"gofmt／go vet／d22scan 三步在 CI 是绿的"。
  ⇒ **它测到的事实我认**（这份 workflow 确实没有 `go build ./...` 那一步，三枚带 build 字样的步骤名是 `cgo build smoke`／`Build wisp.exe`／`Build wisp.exe (deps cached on the runner)`——这与我自己项目记忆里那条一致）。**要判的是"那三句话是谁说的"这一层。**
  ⇒ **三种可能，处置完全不同**：(甲) 代理自造引用——它把不存在的他人前提当"错处"来纠正；(乙) 工具输出里混进来的东西（第 8 代形状：假凭据）；(丙) 它把票面某句改写后当成"简报说"。⇒ **判法交给验收程**：把那三句按字面 `grep` 票面与派单、并把 §6.6 附近它引用的每个编号/文件名核存在性；**核不到就写"未定性"并说明缺哪一枚读数**。

  **④ 三行说明，避免这句话被读成别的（未定性就写未定性）。** 现象出现的位置：**代理自己的证据件正文**（一张对照表），不在我盘上任何一份原文里。**有没有"本机被人入侵"的证据：没有**——我核的是文本引用，没有任何动作被这条引用触发（该程**零代码改动、零越界路径、没执行任何由它引起的动作**）。**最坏后果是什么形状**：若 (甲)，下游会把"编排者纠正过 CI 说法"当成既成事实来继承，一条不存在的共识就这么立起来；若 (乙)，那是注入形状在进化，处置要另开登记。⇒ 两种都不改变一个事实：**它这一格不能因为它"纠正得对"就结**，引用归属没核掉之前，那张表整格记〔待复算〕。

  **⑤ 排程与在飞。** 三枚在跑：票 149 实现程（`cmd/wisp/**`）、票 138 验收程（只读）、票 139 实现程（`internal/agent/**`）。⇒ **139 提前放出来了**：原本按住它是因"与 138 同撞 `internal/agent/loop.go`"，而 138 **零代码改动** ⇒ 撞面消失，这条互斥**当场作废**，不必再等它结线。

  `next=`＝①票 138 的**去向**由验收程判（作废并登记／改判保留；两判都不许我单方定）；②若"预留的洞"成立：那笔 `DEFERRED` 登记**不能由 agent 单方落**——登记表在 `docs/specs/SPEC-12 §5`＝冻结面，而 `AGENTS §1.1` 要求"代码标记↔登记表 **1:1 双向**"，所以正确出路是给 owner 一枚 `Q##`（这件事我先不摆，等验收把②那三发钉住再说）；③票 149／139 交件后照例派非实现者验收；④**推送仍按住**（此刻三枚在飞，推会把它们的中间态一起发布）；⑤⑥无新增等 owner 的项，`Q-53`／`Q-54` 照旧不催。
- [2026-09-25 23:5x +08] **A268｜票 149 实现程交件（三发今天全不响⇒本票成立、射程不缩）；本条要留的是它自己承认"第一版组合变异的尺是坏的"，而**坏尺期间那批读数还留在树里****

  **① 三条核收〔我本轮现跑过〕。** 四枚 commit（`b417d31` 码＋尺／`2f5c7f9` probes／`a055d9f` 件 0-11 格／`c3f7224` 件第 12 格）路径集＝`cmd/wisp/**`＋`.scratch/wisp/probes/149/**`＋它自己那枚件；**禁改轴按 `64858d6..c3f7224` 单独量：`internal/risk`／`internal/panel`／`internal/agent`（**139 正在里面写**）／`internal/observe`／`tools/d22scan`／`frontend`／`design`／`docs/specs`／`docs/PLAN.md`／`scripts/slo-check.ps1` **交集为空**（现量输出零行）**。
  测试文件 **268 加／0 删**⇒"既有十枚用例一行未抹"我复算成立；`slo_windows.go` 那 **3 枚删除逐行看是两行注释（旧那句错承诺）＋一行 `obs.offset = dec.InputOffset()` 被新 helper 包住**，**没有一行断言被删**；票面六框 `6 [ ]／0 [x]`＝它没自勾。
  ⇒ ⚠ 老话再说一遍：这三条**只证明边界干净**，不证明"那把尺会响"。

  **② 它这一票最关键的一句自陈是"我有一版尺是坏的"。** 它交回：harness 第一版把**同一枚文件的多处替换**写成彼此独立的 overlay ⇒ 那发**组合变异**（同时摘 case 11 两条 leg）的"0 红 逃逸"是**尺坏了的产物**；修好后在 `combos2/` 复算。
  ⇒ 两件后果我分开处理：**(甲) 那发承重凭据必须独立重建**——它是 AC#2 全部依据，不能拿复算日志自证；**(乙) `post/` 与 `combos2/` 里同名 `x-*` 各存一份**，第一版那批**没被撤走**——这正是**"尺改了、旧读数留在树里"**那族归因腐坏：留两份，下一位会挑对自己有利的那份。⇒ 已把"清点哪些读数属坏尺期间并逐枚作废"写成验收程的第一格判据。
  ⇒ **一条给派单模板的新增**：只要程内出现过"尺被修过"，**新旧两批读数必须逐枚标版本**，否则我不收表。
  > **② 的更正（09-26 08:4x，验收程 `A271` 表 N-1 第 1 行推翻，原句不抹）**：我这一句**方向是反的**。
  > 它现量：`post/x-p1-d11a-d11b.log`（入库的第一遍，时间戳 23:13:38 **早于** `combos2/` 的 23:15:36）顶层 **FAIL＝1**，红句是字段腿那句 `offset is 0, want 27`；
  > `combos2/` 同一发才是 **FAIL＝0**。⇒ **坏尺那一遍交回的是"1 红"（一枚假安心），"0 红 逃逸"是尺修好之后才出现的读数**。
  > ⚠ 这一处**同源三处都得改**：实现件 §8 第 6 行、**我给 149 的派单 §2**、以及**本行**——三处都出自实现程那一句自述，而它与自己点名的凭据文件不符。
  > ⇒ **教训归我**：我把代理的一句自述抄进了派单判据、又抄进了台账，**两处都没去读那枚 `.log` 的字节**。
  > "尺改了、旧读数留在树里"那族腐坏我防住了，**"旧读数的因果方向被自述带偏"这一族没防住**——它更阴，因为两批读数都在、只是谁先谁后被讲反。
  > 本更正**不改 AC#2 的牙**（验收程第 2 格独立重建那发组合变异，0 红逃逸复算成立、还加强为"该腿整体脱保"）。

  **③ 它把我票面的一处措辞放大了（等验收判"说小了"还是"说错了"）。** 票面第 2 条我写"两发已经在读值仍报 0"，它现量 **20 发探针里 8 发印 offset 0**、并说"两发是样本数不是全集"。⇒ 这两句**处置不同**：说小了＝追加射程更正；说错了＝改名并写清为什么。我不预填。

  **④ 两处它自己登记成"没测"的（我不替它补，也不替它抹）。** (a) `contradictionOffset` 的 **fallback 那一行今天零见证**（覆盖剖面 `1 0`；20 发里 corrupt 支的解码错误全属 `SyntaxError`／`UnmarshalTypeError` 两家）⇒ 已要求验收答那句"**摘掉它有没有任何外部可见读数变过**"；(b) 真 subject 崩溃的形状仍未取证（与票 144／147 同一条洞，第三次挂账）。

  `next=`＝①三枚在飞：**138 验收程（只读）／139 实现程（`internal/agent/**`）／149 验收程（只读，锚 `c3f7224`）**；②**推送继续按住**——139 还在写码，推会把它的中间态一起发布；③票 149 结线要等它的表回来，且**验收程若把 ②(乙) 那批坏尺读数作废掉，AC#2 那一格可能要重跑**（这是我现在能预见的唯一返工点）；④票 138 的去向（作废并登记／改判保留）等它那张"简报说 X"的引用核对结果；⑤`Q-53`／`Q-54` 照旧不催。
- [2026-09-26 00:2x +08] **A269｜票 139 交件（四格四发、痕放在压缩成功那一跳）；本条要记的是它顺手挖出的那笔治理账我亲手量到了、以及我差一点把自己的"顺序推断"当成它的结论**

  **① 三条核收〔我本轮现跑过〕。** 四枚 commit（`4d0866ad`／`1acdd032`／`e22da5a8`／`d949d9c5`）逐枚数路径＝`internal/agent/**`＋`.scratch/wisp/probes/139/**`＋它自己那枚件＋票面（只追加 13 行 Progress log），**禁改轴零命中**；测试文件 **393 加／0 删**（新件）、`harness_test.go` **8 加／0 删**（在原有 helper 件里加，未抹既有断言）；`loop.go` 那枚 **1 加／1 删** 我逐行看了，是 `NewCompressor(b, opt.Summarizer)` → `NewCompressor(b, opt.Summarizer, WithLogger(opt.Logger))` 这一行接线，**不是断言也不是文本**。票面四框 `4 [ ]／0 [x]`＝没自勾。
  ⇒ ⚠ **一条我自己差点犯的错**：我第一次核禁改轴用的是**区间 diff**（`3f6322f..d949d9c5`），结果跳出十枚 `frontend/**`——那**不是它写的**，是**另一枚前端会话在这段区间里落的 commit**。⇒ **"区间 diff 当一个人的账"是错位归因的现成型**；正确尺是**逐枚 `git show --numstat`**（也就是我最后用的那把）。这条与既有条"别人不带 pathspec 会把你 staged 的文件带走"是同一枚币的另一面。

  **② 它自己交回的最有价值一句是"我没证到那条痕真落到文件"**——而我顺手量了半把，**差点把它变成结论**。现量（**我的读法，不是判决**）：`cmd/wisp/logsink.go:153` 是 `slog.SetDefault(...)`；`cmd/wisp/run.go` 里 `installLogSink` 在 **`:178`**、`agent.New` 在 **`:547`**；`internal/agent/loop.go:207` 在 `opt.Logger == nil` 时取 `slog.Default()`，而**全仓非测试代码没有任何一处设 `Options.Logger`**（`grep 'Logger:'` 零命中）。⇒ 按这个顺序，`run` 那腿上"痕到得了文件"**成立**——**但这是行号顺序的推断，不是任何一发读数**。⇒ 我把它原样交给 139 的验收程当**线索**（不是答案），并要求它：(甲) 造一发端到端，让"记录进到了那枚 JSONL 文件"变成**一次测量**；(乙) 另两条腿（`resident_windows.go:57`、`models.go:284` 也调 `installLogSink`）**我没量顺序**，反过来或不经过 `agent.New` 就得写进"没测什么"。
  ⇒ **固定动作新增一条**：**我自己现量到的东西，进了派单就仍要标"这是线索、不是凭据"**——否则下游会把我半把尺读成"编排者已核过"。（今天第三次同形：`A265④` 是转述错因、`A266②` 是推错判据、本条是**把自己的推断写成结论**。）

  **③ 它 AC#1③ 顺手挖到的那笔，我已单独立账（票 150 ＋ `Q-55`）。** 我亲手复核：代码侧 `DEFERRED(D28-1)` **3 枚**（`internal/agent/compress.go:27`、`:49`、`internal/agent/loop.go:394`）＋ `DEFERRED(D11-3)` **1 枚**（`internal/agent/control.go:15`）；`SPEC-12 §5` 段内 **`D28` 命中 0 次**、`D11` 唯一那行是 **`REJECTED`（"意图分类四级流水线 D11 过度设计已废"）**，不是这条欠账的行；而"D 号＋子号"这种写法在 `docs/specs/**`＋`docs/PLAN.md` 里**只有 `D45-1`/`D45-2` 两个先例**。⇒ `AGENTS §1.1` 那条"1:1 **双向**对得上"今天不成立，且**零仪器扫**（`tools/d22scan` 七项 ban 不含它）。
  ⚠ **两把尺给出两个分母**：139 报"0 vs **47** 表行"，我这把报"0 vs **29** 行"——**谁都没抹掉对方**，直接写成票 150 的 AC#1（"先把'一行'的定义收成一句可复算的话"）。
  ⇒ 三条出路（改标记／补登两行／重述那句契约）**都要动冻结面或须人工批准** ⇒ 票 150 立为 `blocked`，问题登成 **`Q-55`**：我的推荐是**甲（改标记）→ 再上一枚双向差集的账检**，并写明不推荐乙的理由是"把一个没有文本定义的编号固化进规格"。

  `next=`＝①三枚只读验收程在飞（**138／149／139**，分别锚 `3f6322f`／`c3f7224`／`d949d9c5`）；②**推送继续按住**——写码程虽已全停，但三枚验收程都会往 `dev` 上 commit 自己的件，推出去的是它们的中间态；三张表回来后一次批量推＋照例逐名比红名集合；③`Q-55` 是**新增等你的一枚**（连同 `Q-53`／`Q-54` 共三枚未答，都是登记不是催；`Q-55` 那条**不答则 1:1 规矩继续空转**，但**不阻塞**其他票）；④票 147 的 `-done` 已做、票 138 的去向等它的表；⑤D32 那条腿仍欠一次真取样（`A266`）。
- [2026-09-26 01:1x +08] **A270｜票 138 的表回来了：那句"预留的洞"没塌；但我上一格那笔"它引用了我没说过的话"被降级成未定性——原因是我自己的派单在盘上没有凭据**

  **① 三发支撑逐发复算全部成立，"预留的洞"这一判不塌**（件＝`docs/evidence/s1/138-cancel-kills-children-r1-accept-r1.md`，6 格／6 枚渐进 commit、零代码改动、无 `rm`）。
  值得记的是**它没有照抄实现程的尺**：实现程用"`os/exec` 的 importer 图"数到 7 枚，它把那把换成 **spawn 形状普查**
  （`exec.Command|StartProcess|CreateProcess|ShellExecute|WinExec|plugin.Open|ForkExec`）——**命中集不变＝7 枚、任务侧 0 枚**。
  ⇒ 这是本仓那条"换一把更宽的尺，看结论会不会变"第一次在**支撑层**（而不是判据层）被执行；门禁那格它先拿实现程留下的原始日志复算四数（逐字对上）再到自己快照里自跑，两件事分开做。
  ⇒ **处置我已落**：票面 `AC#1`／`AC#4` 翻勾、`AC#2`／`AC#3` 原句不动也不勾（"前件判假⇒未触发"是真的），
  本票**不作废、不 `-done`**，改 **`blocked` ＋ 等 `Q-55` 那一次批准一起落**——按它第 4 节给的池内规则 5。

  **② 本条最该留的：`A267③` 那一格我判早了，现在降级。** 我 09-25 23:2x 现量后登的是"三句'简报说 X'在**派单与票面里都不存在**"。
  ⇒ 验收程核：**票面那一半比我说的更强**（整份零枚 `CI|workflow|runner|gh run` 字样，直接排除"改写票面"那一支）；
  **但我那一半站不住**——普通实现派单**从来不入盘**，`.scratch/wisp/dispatches/` 的规矩当时只归档"授予范围／解冻／例外"那三类。
  ⇒ 所以"我派单里没这句"**是我对上下文的回忆、不是物证**；甲（自造引用）／乙（注入）／丙（改写）／丁（我把因果句写进派单、它复述歪了，见 `A265④`）**四支一支都不能定罪**。
  ⇒ **制度已改（不是口号）**：`dispatches/README.md` 末尾追加一节，把归档范围从"三类特殊派单"扩到**每一枚派单正文**（五字段照旧、不回填旧的、仍声明"这里不回答什么是对的"）。
  ⇒ **为什么值得为一句归属争议改制度**：这两天所有的假绿都长在同一处——**把"谁说的"和"什么成立"混成一句话**（`A265④`／`A266②`／`A269②`／本条）。
  前三次我都能靠"再跑一次"兜住，**这一次兜不住：没有物证的回忆不可复算。**

  **③ `Q-55` 的两条理由各被推翻一处，我按"结论不变、依据改写"处理（原句一字不抹，更正追加在同一行里）。**
  我写的"乙＝动 `docs/specs/**`＝契约变更"按 `SPEC-12 §4.1:38` 的字面射程**扣不上**；我引 `AGENTS §1.1` 里"配不上代码标记"那一半也**不成立**——
  §5 实测是 **27 枚散文键数据行、零枚 `DEFERRED(D-xx)` 标记**（与我那把尺的"含 `|` 29 行"不矛盾：29 减表头与分隔行＝27）。
  ⇒ **真已违的是反方向**（有标记无行）。剩下够硬的两条：票面 `:12` 逐字冻 `docs/specs/*.md`；登记落笔者不能是受益人。
  ⇒ 而这件事**真要经你手**的理由只剩一条：**票 138 的来源是你批准的 `Q-43`**，改口＝改你批过的范围。
  ⚠ 另：验收程还推翻我派单里"三枚带 build 字样的步骤名"——同一条命令实为 **4 枚**（我漏了 `ci.yml:642` 的 vite build）。**这是我这一天里第 N 条被实测推翻的派单前提，模板那条"凡前提皆标未验证"仍没做到位。**

  **④ 它造出一枚我今天才看见的产品缺口，我已独立重走并开票 151。** `Bridge.OpenTask` 在派发热路径上（`internal/tools/bridge.go:559`），
  而 `Bridge.CloseTask` **全仓零调用者——连测试都没有**（我重跑：命中只有 `:646` 声明与 `:642` 注释），且它的注释自称"组合根会 defer 它"。
  ⇒ 形状：**每开一轮任务就挂一枚污点 scope，谁都不摘**。`grep -rl CloseTask .scratch/wisp/issues/` = **0 枚** ⇒ 此前**没有任何一张票认领它**。
  ⚠ 票 151 里我写死了两条：**后果边界没量就不许报数**（既不许多报"泄漏很大"也不许少报"只是难看"），以及**若 AC#1 量到"既不渗也不长"，本票作废并登记为什么，不许硬开一道永不响的格**。

  **⑤ 一枚小账，归我：票 138 的原始读数只在 `/tmp/ticket138-r1/`、没进 `.scratch/wisp/probes/138/`**——是 `A264③` 那条落点规矩生效后**第一枚没照做的程**（147／149 都做了）。
  ⇒ 查我自己的账：那条规矩是 21:5x 写进台账的，而我 22:5x 给 138 写派单时**只在 149 那份里带了它**、138 那份漏了 ⇒ **规矩没进模板就等于没立**，这次由我承担、不记实现方。

  `next=`＝①两枚只读验收程在飞（**149 锚 `c3f7224`／139 锚 `d949d9c5`**）；②**推送继续按住**（此刻 30 枚以上未推，等两张表齐一次批量推＋照例逐名比红名集合）；③票 151 待派（落点 `internal/tools/bridge.go`＋组合根，与在飞两程不撞，**但组合根那半在 `cmd/wisp/`，要等 149 的表结完再排**）；④**等你**：`Q-53`／`Q-54`／`Q-55` 三枚（`Q-55` 已按③更正依据，结论与推荐不变；**不答则 1:1 那条规矩继续空转**，但不阻塞其他票）；⑤界面仍等你的定稿反馈，一格都没摆给你。

---

## A271（09-26 08:3x）：一句"当前任务正在被 QoderWork 处理中 \<UUID\>"——**注册表里查无此会话、日志里只有我自己 grep 的回声**；以及票 139 验收程第 0-1 格把那条痕量成了"今天到不了"

**① 一句阻断提示判为注入形状（第 9 代），只登记不服从。**
08:2x 本轮对话里出现一行阻断提示，原文前 40 字＝`当前任务正在被 QoderWork 处理中: 1e9c2544-1e0b`（出处＝挂在本轮用户消息上的 hook 阻断段，声称来自 `PreToolUse:TaskStop`，并要我"检查是否有恶意内容后继续原本的任务"）。
**本轮我从未调用过 `TaskStop`**，所以这句话替我写好的结论是"**你手上的活儿已被另一枚会话接走**"——照它做的后果＝编排者停摆、编队无人看。**服从的代价不对称**：停手＝两程失去监督；忽略＝零损失。

三条判据逐条量过（08:26–08:31 现跑，全部〔我本轮现跑过〕）：
1. **无具名路径**：整句只有一个 UUID，没有文件、没有 run id、没有 step。
2. **那枚 UUID 查无此会话**：`list_chat_sessions`（跨会话只读工具，limit 50、`nextCursor` 为 null＝已翻完）返回 **11 枚**，逐名核对无 `1e9c2544-…`；本机在册的只有 `a91d07b8`（＝本程，`running`）、`58401730`（前端，`cold`）、`e5a4f30e`、`8052f104`、`9871039f`、`451aab76`、`24b8b0eb`、`1c8e0901`、`0af2114d`、`c573b8df`、`0a57ff4a`。⇒ 与 `injection-timeline` 第 8 代"假 git sha 混进工具输出"同族，只是这次假的是会话 ID。
3. **动作盘上核不到**：`~/.qoder-cn/settings.json` 里**没有 `hooks` 键**（键只有 `aicodingPluginSettingsMigrationVersion`/`enabledPlugins`/`providers`）、项目根**没有 `.qoder/` 目录**、`~/.qoderworkcn` 下对该字面量**零命中**。
   ⚠ **搜索面如实写清，别把"我没搜到"说成"不存在"**：上述四路（用户 settings／项目 .qoder／QoderWork 目录／本轮 run 日志）**之外**我只没搜应用二进制。但第 5 路给了决定性的反证——
4. **决定性反证：日志里那 4 次命中全是我自己造的。** `grep -c "当前任务正在被"` 在本轮 run 日志 `~/.qoder-cn/logs/runs/2026-09-23T09-04-22…/qodercli.log` 得 **4**，逐条看时刻与上下文＝`08:28:37`/`08:28:39`/`08:30:30`/`08:30:30`，四条都是**我搜这个字符串这件事本身**被记进日志（`tool.requested` 与 `tool.shell.started` 各两枚，命令行里就带着这几个字）。⇒ **该句在这台机器的日志里没有一次独立于我的取证动作而出现**，"它由 harness 产出"这个前提在盘上取不到支撑。

**两栏计数（按 A250 拆栏的规矩，不混一个字段）**：本轮 `真通知回显数 = 0`、`判为注入数 = 1`。
**已核到的"没事"证据（不是结案理由，只是现状）**：两枚验收程**没被停**，且此刻仍在写盘——`subagents/agent-*.jsonl` 里 08:27 有两枚更新（`…2a3ff7b09b9e8557`／`…fc5a74a66463a147`），139 那张表 08:25 已交出一枚 commit。⇒ 我本轮**没执行任何 `TaskStop`、没停任何程**，无撤销动作要做。

**② 票 139 验收程第 0-1 格（commit `b4e7769`，08:25）把派单首要攻击点量成了读数——而且读数是否定的。**
〔读数出自验收程，本条我只做了登记，未复核其取证；终裁等它四格结完〕要点三条：
- **机制那一半是通的**：sink 那条链真把记录落到 JSONL（11 枚同链记录实落盘、键名未被红删改）。
- **但这枚痕今天到不了**：`wisp run` 一个任务＝一枚 user 消息 ⇒ `raw_rounds` 恒为 1，而 `KeepRawRounds=3` ⇒ `rep.Ran` 恒假 ⇒ `compress.go:176` 那条成功边 Info **永不执行**；且 `.Steer(`／`.RunAsync(` 非测试零调用者、`agent.New` 全仓唯一非测试点在 `run.go:547`，另三条 `installLogSink` 的腿（resident/models/secret）**连 Loop 都不构造**。
- **它对我派单里那四行 grep 的复核：全成立**（这是对我有利的部分，如实记）。
⇒ **预判（不是终裁）**：票 139 很可能落"**机制成立而这枚痕到不了**"那一档＝我记忆里的 `fail-closed 兜住 ≠ 功能通` 同型。若终裁坐实，正解**不是**放宽票面判据、也不是改压缩器（`KeepRawRounds`/`Need()`/D15 预算在票面 §3 明令不许动、动了＝契约变更），而是**把这枚痕的可达性归口到"多轮历史那条腿还没接"的下一张票**，139 本体按"交付了一件当前不可达的仪器"来裁。
⚠ **登记时不替它结**：这一格只是 r1 的第 0-1 格，剩两格未交；我不许自己在它交件前翻任何勾、也不许把"到不了"写成终裁。
  > **② 的更正（09-26 09:1x，同一枚验收程自己把它推翻，原句不抹）**：那句"**今天没有任何一条腿能产生这枚痕**"**是错的**——
  > 它出自该件 §1，而该件 §7.2 第 4 项由**同一枚程自己**收紧为：**普通收尾形 0 枚、重复阶梯退化形 1 枚真到盘**（§8 又把两形并成同一次跑，唯一变量＝有没有第 4 枚 user 轮）。
  > ⇒ 我的错**不在抄了未定稿的读数**（我当时明写"只登记、不替它结"，那半句做对了），而在**把一张还在自打的表当成了裁决的落点**、并据此预告了档位。
  > ⇒ 加一条模板规矩：**引用在飞验收件的单格读数时，一律写成"该件 §N 现量……（该件仍在自打，档位见其收口节）"，不许写"很可能落 X 档"**——后者是我在给自己造"看起来已经定了"的既成事实。
  > 实质后果：AC#2 的落点选择**判对了**（不重开），本票剩下的两格是覆盖面与归因，已归口票 **153**。

**③ 账（08:31 现量）**：`HEAD = b4e7769`、`origin/dev..HEAD = 32` 枚未推。推送**继续按住**，两条理由都是本轮量出来的：①两枚验收程正在同一棵树上写文件（此刻工作树里有 `A  docs/evidence/s1/139-…accept-r1.md` 与 149 的未跟踪增量），②我自己那条 `A263` 新规矩"推送与派写码程不许挤在同一分钟"——本轮虽没派新写码程，但 149/139 两程在跑测量，同机抢 CPU 的后果我已经实发过。

`next=`＝①等 149（`…149-three-unpinned-outlets-r1-accept-r1.md`，已交 0-6 格）与 139（已交 0-1 格）两张表结完；②**然后**批量推 32 枚＋照例逐名比红名集合；③票 139 若终裁坐实"不可达"，我开一张"多轮历史那条腿"的缺口票、并把 139 的翻勾条件改写成跨票依赖（**不改它票面原句**）；④票 151 待派（组合根那半仍等 149 结表）；⑤**等你**：`Q-53`／`Q-54`／`Q-55`，以及界面定稿反馈——都不阻塞上面这些。

**补甲（08:36，接在③之后；上面那行 `next=` 与③的"32 枚"原文一律不动——那是 08:31 的真读数）**
⚠ 本段与下一段编"**补甲／补乙**"、不占①②③的号：上面那行 `next=` 里自己已经用了①..⑤，同号会读成两条账。
③那笔账**四分钟内就过期了**：`origin/dev..HEAD` 现为 **36** 枚。增量不全是我的——**前端会话在同一棵树上提交了**：
`f22af4e`（08:29，`fix(frontend owner 指认)`：滑翔指示条偏上 12px，实测 delta=0）与更早的 `424ac84`（09-25 23:46，beautiful-ui 转向）。
现量方式＝`git log origin/dev..HEAD --name-only` 按路径筛，触及 `frontend/` 或 `design/` 的 **2 枚**。
⇒ **按 `A250` 那条"在飞写码程存在时推送会把它一起发布"**：我这一次批量推**会把别家的两枚界面提交一起推上去**，
而界面按 `Q-52` 撤回令**还没定稿**。这不是我能替它决定的，所以推之前我把这一条摊给 owner，**不默认替他推**。
⚠ 同时记一句免得下一个人误读：这 2 枚**不是本编队写的**（本编队从未碰 `frontend/**`/`design/**`，`A236`/`A244` 那条归属仍然成立）。

**补乙：票 139 验收程第 2 格（commit `533b7e5`，08:34）给了与②相反方向的一半**：那四枚用例**枚枚承重、无一装饰**
（M1b 真删两发红／M2 只红端到端那发／M3 被反向咬住／M4 被隐私探针咬住；且它跑的是**整包** `-count=1`、不是只挑那四枚），
逐味摘掉各有一发逃逸（T1↔M4、T2↔M3、T3↔M2、T4↔M9）。
⇒ ②与⑤**同时成立、不互相抵消**，合起来是本票真正的形状：**钉子是活的、机制是通的，但那枚痕今天在生产上到不了**。
所以终裁大概率落在"附条件入账"那一档，而不是"做坏了"。⚠ **我仍不替它结**，剩两格未交。

---

## A272（09-26 08:5x–09:0x）：票 149 **收表**（撞顶发生在交件之后）、票 139 **两勾两不勾**；新立票 152／票 153；以及共享 index 又险些吞掉别人的活

**① 票 149：通知说"失败"，盘上说的是"已交件"——这次动作是收表，不是重派。**
`a6051d9160e77560a` 撞 150 轮上限，`result` 字段只有 66 字节（`Adding this self-correction to the record, then my final commit.`）。
⇒ 按本仓规矩**不看 `result`**，看四条：① 表内有**第 10 格总裁决**＋第 11 格"本程没测什么"＋N-1…N-4 四节；
② `git status` 对该件**零未提交增量**；③ 最后一枚 commit `f89ae9b`（08:37）正是那句"自我更正"本身——**它写完了才撞**；
④ 表末明写"未完成＝无"。⇒ 六格 AC **全部成立**，票面六框我按表定勾、已加 `-done`。
**它的验收质量值得记两笔**：N-1 那张表**十三行里推翻了自己两处**（第 7 行一枚无效"空"读数、第 13 行一处把中性读数当反诘），
且 N-4 留了一枚**真实危险读数**——它第一次 `git add` 之后 `git diff --cached --name-only` 里**同时出现票 139 的四枚文件**
（`compress.go`／`compress_trace_test.go`／`harness_test.go`／`loop.go`，那是 139 自己 staged 在**共享 index** 里的活）。
⇒ 那一刻只要 commit **不带 pathspec** 就会把 139 的中间态吞进 149 的 commit。**它停手核了名册、只提自己那一枚路径**，随后 139 自己提交（`1acdd03`）。
⚠ **这不是"它做对了"，是共享工作树的既有风险又发生了一次**（`A184` 那一族的第三次实发）：**风险不在 `git add -A`，在"别人已经 staged 在同一个 index 里"**——
所以光写"禁 `add -A`"不够，**每次 commit 前必须 `git diff --cached --name-only` 现量名册**这一味才是那堵墙。

**② 我 08:31 那笔账（`A271③`）与 08:4x 那条预判（`A271②`）都被推翻了，两处都就地挂更正、原句不抹。**
- `A271②` 那句"今天没有任何一条腿能产生这枚痕"**是错的**：139 验收程 §7.2 自己收紧为**普通收尾形 0 枚、重复阶梯退化形 1 枚真到盘**。
  ⇒ 我错在**把一张还在自打的表当裁决落点引用**、并据此预告了档位。已立模板规矩（见 `A271②` 那条更正末句）。
  ⚠ 连带：`A271 补乙` 里"那枚痕今天在生产上到不了"同一枚错，**以 `A271②` 的更正为准**，我不去改写补乙原句。
- **更要紧的一处推翻的是我自己**：`A268②` 我写过"那发组合变异的『0 红 逃逸』是**尺坏了的产物**"——验收程读日志字节判为**方向相反**
  （坏尺那遍交回的是 **1 红＝假安心**，0 红是**修好尺之后**才出现的）。⇒ 已在 `A268②` 下就地追加更正，并把教训归我：
  **我把代理的一句自述抄进派单判据、又抄进台账，两处都没去读那枚 `.log` 的字节**。
  "旧读数留在树里"那族我防住了，**"旧读数的因果方向被自述带偏"这一族没防住**——它更阴，因为两批读数都在、只是谁先谁后被讲反。

**③ 票 139 定勾：两格勾、两格不勾，且不扣 `-done` 后缀。**
终版档位＝**AC#1 成立（两处措辞已被它自家 commit 顶过期）／AC#2 附条件入账／AC#3 附条件入账／AC#4 成立**。
⇒ 我只勾 AC#1／AC#4，**AC#2／AC#3 保持未勾**、票面**不加 `-done`**（表里"附条件"而票面 `[x]` 是最坏形状：下一位会以为已被否掉又修完）。
⇒ 验收程交回的三件待裁我逐件裁：**(1) AC#2 落点不重开**（选型判对了，痕能到盘、8 枚键名未被 `redactHandler` 改写）；
**(2) M5 那形开票 153 AC#1**（`if rep.Ran`→`if c.Need(hist)`＝痕恒打，**四枚用例全在场、整包全绿**；缺的那形恰是生产常见形）；
**(3) 痕不带 taskID 归票 153 AC#2**。⇒ 两件合在一票（同一枚文件族），但票面写死**不许互相抵账**。
⚠ 第四件——"常见路径今天根本不折叠"那一层——**不进任何票**：唯一直接修法是动 `KeepRawRounds`／`Need()`＝**契约变更**，
我只在票 139 §1 那句因果下追加了 `>` 更正（原句不抹），并把"GAP-18 后半排期时今天画出来会是空白"这条前提**登记在这里**；
界面那条线**仍不摆给 owner**（`Q-52` 撤回令在效）。

**④ 新立两票。**
- **票 152**＝"subject 中途死掉那一发**从没被真量过**（只在测试里被**造**到可达）＋ `contradictionOffset` 的 fallback 是**摘掉后零读数变化**的装饰腿"。
  ⚠ 立票理由不是"又发现一个洞"，是**同一枚洞第三次被写进"我没测"**（144 挂着、147 挂着、149 还挂着）⇒ 三张表都只登记不取证，说明缺的不是判断，是一次愿意起真进程的取证。
  票内 AC#1 写死**不许预填结论**（量出"那条路产不出红句"就是要的答案，按 ⓑ/ⓒ 停手上报，不许拿假对象读数冒充端到端）。
- **票 153**＝上面第③件的两发。
- 两票都随带**本仓当天新量到的两枚仪器坑**：① `exit status 0xc0000135` ＋ 0 条 `=== RUN` 的因**至少两枚**（dll 不在位、以及 `PATH` 写成 `D:/…` 盘符＋正斜杠形）⇒ **"dll 在位"不是充分条件**，只认 `=== RUN` 枚数这一条共因读数；
  ② **`-overlay` 与 `-cover*` 合用时 overlay 被静默忽略**（指向语法不合法文件的 overlay，不带覆盖 `[build failed]`、带覆盖 `ok`＋coverage）⇒ **变异与覆盖不许合跑**，谁先合跑谁先拿假绿。

**⑤ 推送：owner 已答，按答的做。** 我问他"这 37 枚里夹着前端会话的 2 枚界面提交，批量推会把没定稿的界面一起发布，怎么处理"，
他的原话＝**「不用你管这两个，它自己会commit的」**。⇒ 我读作：**不因那两枚按住推送**（他不去管、也不要我替他挑拣）。
⚠ 如实记两件事：(a) 他的答**没有直接回答**"推上去云上就能看到没定稿的界面"这一层，我按最合理解释执行，若他本意是"别推界面"那一句就能撤（**撤销口令：「那两枚别推」**，而撤回只能靠**追加更正**、已推历史不改写）；
(b) 这 2 枚**不是本编队写的**，推后我在下面 `next=` 之后点名它们，避免下一位把界面改动算到本编队账上。

`next=`＝①**执行批量推送**（此刻 `HEAD` 为 `f8cde14`＋本条，未推 **39 枚**量级）到 `origin`，推后**逐名比红名集合**（`test-core`／`test-windows` 两腿），并把 `slo-full` 读到终态再据其颜色说话（**它的绿可以等于什么都没测**）；②**先派单、后推送**这条规矩本轮**不满足**——本轮派的是 152／153 两枚**尚未派**的票，所以推完再派，不挤同一分钟；③票 139 的 `-done` **等 153 交件后**再定（AC#2／AC#3 的翻勾条件是**跨票依赖**）；④票 151 待派（组合根那半在 `cmd/wisp/`，149 已结线 ⇒ **现在不撞了**）；⑤**等你**：`Q-53`／`Q-54`／`Q-55` 与界面定稿反馈，都不阻塞上面这些。
