...
| 检查 | 一句总裁 |
|---|---|
| 1 sha 可解析性 | 终态抽数 **1252 处／644 枚去重**：**521 枚可解析**；**122 枚不可解析**，其中 **113 枚正常**（63 枚全数字 run/job/字节读数 + 36 枚上下文已定性为假号 + 14 枚根本不是 commit 号），**9 枚是缺陷**（其中 2 枚台账已自纠、**7 枚至今无更正**）；另 **1 枚待更正**（`:5771` 的"五枚 commit"漏了真号 `7cc5050`）。 |
