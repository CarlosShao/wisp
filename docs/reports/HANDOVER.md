...
- ⚠️ **13:3x 追加（接续的人先看这一条，别看下面那几条）**：**票 135 AC#8 与票 133 AC#2 同日翻勾**，裁决方＝非实现者
  ...
  - **在飞（15:2x 现量）**：**1**＝`acceptor-ticket137-ac3-r2`（非实现者，锚＝它开工时 `git rev-parse HEAD` **现取**的那一枚、
    在 `2d029f9` 之后；快照前缀 **`wisp137ac3r2-`**；只裁 **137 AC#3** 那一格，**明令不许顺手裁 AC#4／不许动票面**）。工作树干净。
    ⛔ **上一条里那枚 `acceptor-ticket137-ac3-r1` 已作废、名字不再复用**：它交回的"§0–§6 已裁完／47 棵树／66 发读数／commit 到 `99319c7`"
    **盘上一枚都不存在**（`git cat-file -t 99319c7` 失败、证据文件不存在、`/d/tmp` 近 3 小时零新增、工作树干净），
    且它简报里那枚核心前提"本机没有 Docker"被我实测推翻（`docker version` server **29.6.2 rc=0**、`golang:1.27` 在本地、`docker ps -a` 里本仓昨天那枚容器还在列）。
    **⇒ AC#3 目前是"无凭据"状态：既没判成立也没判退回，什么都没判。账与三条新规矩在 `A155`。**
    ⚠ **我核它的方式（下一程照这个扫，别翻整个 `/d/tmp`）**：r2 的简报已写死"每节 commit 之后把 `git log --oneline -1` ＋ `git show --name-only HEAD`
    的原样输出贴进证据文件"⇒ 我只扫那两行就知道它有没有真提交；**任何一次工具调用被拒它都必须立刻带已有的东西报回，不许"假设成功"**。
    ⚠ **派单前我现量的四枚前提（别当"它自述的"）**：`git show --name-only 6f3817a` 三枚文件**全是 `*_other_test.go`**、
    `head -1 internal/winsec/placement_symlink_113_other_test.go` ＝ `//go:build !windows` ⇒ **AC#3 的读数只能在 linux 侧取，宿主上连编译都不发生**；
    `git diff 4e66817..HEAD -- internal/winsec/` **空**，且归档 tight 树里那枚 `placement_symlink_113_other_test.go` 的 `md5sum`
    ＝`cb8350cb5e5917a685c10b39d57d5275`**逐字等于 HEAD 里同一枚文件** ⇒ 上一程终裁件 §2.2／§2.3／§2.4 那批 MUT-D 读数量的还是今天这版码，
    **可引用为〔日志＋归档，抽验〕**，但我仍要求 r2 至少自己重跑一枚判别对（同一 MUT-D 下 loose 全绿 vs tight 全红）。
  - **排队（顺序即优先级）**：①**137 AC#3**（在飞）→ ②**136 AC#11**（flake 命中率：**必须单独跑、编队要空**——
    它量的是"命中率"，我若同时再派一枚取读数的程就是在污染它自己的分母 ⇒ **上一版这里写"与 ① 并行起"是错的，已改**）→
    ③ 137 AC#4（**单独做，不与 AC#2 并**，带 119 那三枚 sentinel-only 内联断言）→
