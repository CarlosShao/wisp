...
## 4. 在途状态（**最新是下面的 4.0o（09-24 15:0x）；4.0n 及更早各节整段保留供追溯**）

## 4.0o 停车点（**2026-09-24 15:0x 版；新会话从这一节起读**，4.0n（08:5x）及更早只作追溯）
...
- ⚠️ **13:3x 追加（接续的人先看这一条，别看下面那几条）**：**票 135 AC#8 与票 133 AC#2 同日翻勾**，裁决方＝非实现者
  ...
  - **在飞（15:0x 现量）**：**0** —— 编队全空。`acceptor-ticket136-ac11-r1` 已交回（六节齐、锚 `51e29b0` 自量），两枚实现程早已终止。
    ⚠ 它交件时报的 `1049513`／`1b93810` **两枚号盘上不存在**＝它自己的 `git commit` 那一步静默失败（今天第二枚同形状），
    末节留在工作树里 ⇒ **我代提交为 `2088e80`**，代提前替它跑完它自己那三条凭据反扫（候选真值枚数 1／文件命中 0／危险形状 `[]`），pathspec 只写它那一个文件。
  ...
  - **排队（15:0x 刷新；①已结，整段前移）**：① **137 AC#4**（本票只剩这一格；**单独做，不与 AC#2 并**，带 119 那三枚 sentinel-only 内联断言；⚠ 派单前先读 `137-ac3-r1-acceptance.md` §8 与票面那枚 `>` 指针更正——`assertRefused113` 在 HEAD 上是 `:174` 不是 `:127`）→
    ② 136 AC#10／AC#14（**都要 `cmd/wisp` 空出来**；AC#14 的具名解冻**派单时现划、不许照抄 AC#12 那两段坐标**）＋ **AC#15 与这两格同批**（不单独起）→
    ...
    → 顺手核票 119／125 是"只差改名"还是"故意留空的裁定"（**不擅自改名**）。
