...
- ⚠️ **13:3x 追加（接续的人先看这一条，别看下面那几条）**：**票 135 AC#8 与票 133 AC#2 同日翻勾**，裁决方＝非实现者
  ...
  - **在飞（15:1x 现量）**：**2** ＝
    ①`worker-ticket137-ac4`（写码，**只裁票 137 AC#4 那一格**＝那 7 处 raw `t.TempDir()` 逐枚判换/不换；锚 `182daed`、快照前缀 `wisp137ac4-`；
      判据②要它自己在 **MUT-D** 下跑两形名册差集；⚠ **明令不许拿"未变异软链形今天也 11 枚红"当凭据**（那一形修之前就在响＝装饰）；
      ⚠ **119 那三处 sentinel-only 内联断言（`dataroot_symlink_119_other_test.go:203/:251/:308`）只许判定、不许静默改**——要改由我在票 119 另开一格，否则破 AC↔裁决表 1:1）；
    ②`acceptor-ticket125-r1`（**非实现者**，票 125 的**第一份**裁决表，四格 AC#1..AC#4；快照前缀 `wisp125r1-`；唯一写件 `docs/evidence/s1/125-ac1-ac4-r1-acceptance.md`）。
    ⚠ 两枚都带**争用闸门**：我 15:05 那次 push 自启了本机 runner 上的 `ci` run **`35967768017` in_progress**（上一发 9 分钟），取读数那几批要等它结束且 `docker ps` 干净；撞负载的那几发作废重跑。
