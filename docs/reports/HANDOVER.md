...
- ⚠️ **13:3x 追加（接续的人先看这一条，别看下面那几条）**：**票 135 AC#8 与票 133 AC#2 同日翻勾**，裁决方＝非实现者
  ...
  - **在飞（13:5x 现量）**：**1**＝`acceptor-ticket137-ac3-r1`（非实现者，锚＝它开工时 `git rev-parse HEAD` **现取**的那一枚、
    在 `712d92f` 之后；快照前缀 **`wisp137ac3-`**；
    只裁 **137 AC#3** 那一格，**明令不许顺手裁 AC#4／不许动票面**）。工作树干净（`git status` 现量 0 行）。
    ⚠ **派单前我量到的一件事（它若自己重跑就省一整套容器读数）**：`git diff 4e66817..HEAD -- internal/winsec/` **空**，
    且 `/d/tmp/wisp137r2-ac2-tree-d-tight/…/placement_symlink_113_other_test.go` 的 `md5sum`＝`cb8350cb5e5917a685c10b39d57d5275`
    **逐字等于 HEAD 里那枚文件** ⇒ 上一程终裁件 §2.2／§2.3／§2.4 那批 MUT-D 读数**今天量的还是今天这版码**，可以引用；
    但我要求它**至少自己重跑一枚判别对**（同一 MUT-D 下 loose 全绿 vs tight 全红），否则本格只到〔日志＋归档，抽验〕这一档。
