...
## 4. 在途状态（**2026-09-20 23:5x 版，取代上面 23:19 版与更早的 08:1x 版**）
...
- **正在跑（三个写码代理）**：
  **票 70** CI 转绿（AC#1 全仓 gofumpt：**68 个 `.go` 文件正在被重写**，未提交；⚠ 因此
  任何新代理都必须**只 `git add` 自己的显式路径**，且**不许跑格式化**）；
  **票 64** 的 winlive 逐跑复测（补 AC#4 的"不被 skip"缺证，交互四项任一 SKIP ⇒ 退回未勾）；
  **证据更正代理**（A30② 三张假"d22scan clean"章 + A30④ 失效引用，**只追加不覆写**）。
  ⚠ 桌面此刻归票 64 的 winlive 占用——**不要**再派会开窗口的代理。
- **review 中（勿重开已交付的 AC）**：**票 67**（AC#1/2/4 已闭并被我独立复跑；**AC#3 压住**：
  要改 `cmd/wisp/providers.go`，与票 70 的格式化同树）、
...
- **ready 但被显式压住**：**票 69**（C21 表 61 行无机器检查；与票 68 同动 `internal/ball` ⇒ 串行，
  且现在撞票 70 的格式化）、**票 20 残口**（AC#5 桥层 junction 用例 + A18 真 taskkill 残留；
  `internal/tools`/`internal/risk` **都在票 70 的 68 文件里** ⇒ 等它落地）、
  **票 71**（每道门自报工作量，`blocked-on-70` 是**文件冲突**约束不是语义依赖）。
- **工作树**：**不干净，且是有意的**——票 70 的 68 个未提交 `.go` 格式化改动。今天 12 次代理闪断里有
  一次是在批量写文件之后死的（票 66 前两批分别在 102 次调用与更早），所以**读到"树脏"不要 checkout 清掉**。
- **registry 从 A13 一路涨到 A30 + 裁定到 R16**；`docs/reports/pending-and-issues.md` 是唯一真相源，
