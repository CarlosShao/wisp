...
## 4. 在途状态（**最新是下面的 4.0o（09-24 16:0x）；4.0n 及更早各节整段保留供追溯**）

## 4.0o 停车点（**2026-09-24 16:0x 版；新会话从这一节起读**，4.0n（08:5x）及更早只作追溯）
...
- ⚠️ **13:3x 追加（接续的人先看这一条，别看下面那几条）**：**票 135 AC#8 与票 133 AC#2 同日翻勾**，裁决方＝非实现者
  ...
  - **在飞（16:0x 现量）**：**1** ＝ `acceptor-ticket137-ac4-r1`（取读数中）。**写码位 0／3**。
    它的判据＝**按更正后的两条凭据判**（未变异软链形 11 枚由红转绿 ＋ 红因换轨），另加一发实现方没做的进攻＝
    **只回退 7 处里的 1–2 处**，问"**撤掉这处哪条用例变得不响？**"——答不出＝装饰、两形读数一模一样就直接退回不许附条件；
    开工要求先证 `git diff b1010ff..HEAD -- internal/winsec/` 为空。它还要独立否一次**我在票 119 开的那格 AC#7 对不对**（尤其"两味药必须一起下"那句）。
    `acceptor-ticket125-r1` **已交件收妥**（六枚 commit 全在盘上、753 行、只带它自己那一枚文件），见下面那节。
