...
## 4.0o 停车点（**2026-09-24 16:3x 版；新会话从这一节起读**，4.0n（08:5x）及更早只作追溯）
...
- ⚠️ **13:3x 追加（接续的人先看这一条，别看下面那几条）**：**票 135 AC#8 与票 133 AC#2 同日翻勾**，裁决方＝非实现者
  ...
  - **在飞（16:4x，两次取样判）**：**1** ＝ `acceptor-ticket137-ac4-r1` **仍活着**（transcript 69 秒内 1,414,186→1,446,052 字节；表 679→**728 行**）；
    `acceptor-ticket119-ac2-r2b` **已停**（mtime 停在 16:33:55、隔 1.5 分钟未动）。**写码位 0／3，但我故意不放**（见下面第 3 条）。
    ⚠ **`A164`/`A168` 里我三次判它"死了/交完了"都判早了**（`A165` 更正一次、本条第二次）⇒ **判"到此为止"唯一认的尺是两次取样都不动**，通知与自述都不算。
  ...
  - **16:4x 三件"不做什么"的理由（写给接续的你，别当成漏做）**：①**不推送**（35 枚攒着）——`r1` 还在容器取数，我一推 `dev` 就自启本机 `ci` 抢 CPU；
    ②**不放 `#50`**——它会改 `internal/winsec/**`，正撞 `r1` 的"码面未动"前提（这条今天已救过它一次）；
    ③**票 137 五勾／0 未勾但不改名 `-done`**——`AC#7` 那笔挂在票 119、119 自己 `next=` 那 7 条我还没裁完。
