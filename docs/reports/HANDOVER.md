...
- ⚠️ **13:3x 追加（接续的人先看这一条，别看下面那几条）**：**票 135 AC#8 与票 133 AC#2 同日翻勾**，裁决方＝非实现者
  ...
  - **在飞（14:0x 现量，时刻取 `date`＋`git log %ad`，别再信我上面那些"15:x"——见 `A156`）**：在飞 **0**（137 那两枚终裁程都已交回）；
    票 137 **AC#3 已翻勾**（凭据＝`137-ac3-r1-acceptance.md` §2–§6＋`## 总判`，五枚 sha `4f61430 8d62096 6155a86 4c9ea23 fc8c4b3`，锚 `2d029f9`）。
    本票现值：**AC#1／AC#2／AC#3／AC#5 四勾，只剩 AC#4 一格** ⇒ 不结、不改名 `-done`。
    ⚠ **同格被我双派过一次**（第二份＝`137-ac3-r2-acceptance.md`，744 行、§0–§7 齐、总判在 `58701bc`，七枚 sha 见票面那一格）：
    两枚**独立**表都判"成立、无附条件"；我做的是**数据级**交叉核对（软链形 `=== RUN` 分母 45 枚逐名相同、顶层 `--- FAIL` 名册差集 0、红行逐名同组），
    r2 另读 r1 给出**四层无冲突** ⇒ **本勾挂 r1**、r2 记为第二 witness（不算第二格、不互相抵账）。
    ⛔ **上面那条"15:2x 现量＝r1 已作废、名字不再复用"整条作废**（成因在 `A156`：我 13:32 把一枚还在跑的程判死并重派；r1 的第一枚 commit 落在 13:44）。
    ⚠ **`A156` 里最该学的一条给了我自己**："撞轮次上限"的通知**连"这一段是终点"都证明不了**——r2 中途也发过一次这种通知、随后自己跑完了。
    此后此类通知到达只做两件事：`date` ＋ 看它的 commit 序列／`task-*.json` mtime，然后**等它交回或明确超时才动**。
