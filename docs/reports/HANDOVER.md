...
- **owner 手上待拍 5 条**（问过未答，按你的规矩已入档）：`Q-31` 封装修剪日志算不算私有数据 · `Q-32` 要不要查那串注入文本 ·
  `Q-33`（**已按本次冻结改为"等外部 agent 做完界面再一起签收"**，不再主动催）· `Q-34` 那张自称「用户已更新编码规则」的表是不是你设的 · `Q-25` CI 并发组加 `github.sha`。
