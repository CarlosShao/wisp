...
- **这一节只给"从哪儿接"**；这 2 小时的账在 `docs/reports/pending-and-issues.md` 的 **`A145 … A147`**。
- **已落定、新会话不要重开的判断**：
...
- **在飞（10:2x 现量）**：写码 **1**＝`worker-ticket135-ac8-mg`（`cmd/wisp/**`，M-G 那发家族变异）；
  终裁 **1**＝`acceptor-ticket136-ac12-ac13-r1`（锚 **`7d73b5f`**、快照前缀 `wisp136r1-`；
  点名要它判 **§5-5 那枚没补的 `dropped_reads` 该不该补**、**越界 hunk 有没有出 `:431-443` ∪ `:470-501` 这两段**、
  **消费面 11→13 之后新 key 有没有真读者**、以及**自己重跑**它自述的那发 MC 探针）。
  ⚠ 派单里已写死：**禁止读脏工作树冒充被验版本**（此刻工作树有 `cmd/wisp/leg_dispatch_gate_133_test.go` 未提交＝135 那枚的活）。
- **推送状态**：`origin/dev` 与 `cnb/dev` **都已＝本地 HEAD**（本轮把压着的 18/10 枚一并推上去了，
  推前核过 `rev-list --count HEAD..<远端tip>`＝0 ⇒ 两枚都是 fast-forward、无 force）。
  ⚠ **旧的那条"两个 commit 压着别推"的禁令我已重验并作废**：它的两个理由都已消失（`frontend/` 早在双方 HEAD 里、票 88 已 `-done`）。
  ⇒ 记一条：**禁令跟建议一样会腐坏，它比建议更容易被无条件服从**，遵守旧禁令之前先重验前提。
- **排队（顺序即优先级，别乱序起）**：`acceptor-ticket137-ac2-ac5-r1`（**下一步就派这枚**）→
  **`#40` 全树终判据复算**（仍按住，等 135 M-G 落地）→ 137 AC#3（按更正块形状、锚 MUT-D）→ 137 AC#4（**单独做，不与 AC#2 并**）→
  135 原六格＋AC#7 → 136 AC#10／AC#11 → 114 接线 → 33 宿主 → 122（42 条 staticcheck ＋ `ci.yml:113` 那枚钉）→ 123 →
  128 AC#4／AC#5 → 132 → 112 → 86 → **票 138** → **票 139**。
