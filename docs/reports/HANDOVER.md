...
- **正在跑（三个）**：**票 70** CI 逐 job 转绿（从 `f342413` 之后那次 run 接续）、
  **票 64** winlive 逐跑复测（占桌面）、**票 62 草案** `docs/evidence/s1/62-visual-spec-draft.md`
  （20 态视觉映射表，等 owner 签字；A30④ 的缺口）。
  ⚠ 新派代理时**不要让三者并发碰同一包**：winlive 占 `internal/ball`，票 70 的 R16 改动要碰 `cmd/balldebug`。
