...
## 4.0o 停车点（**2026-09-24 17:0x 版；新会话从这一节起读**，4.0n（08:5x）及更早只作追溯）

- **这一节只给"从哪儿接"**；这几小时的账在 `docs/reports/pending-and-issues.md` 的 **`A145 … A170`**（09-24 17:0x 现量：台账末条＝`A170`）。
- ⚠️ **13:3x 追加（接续的人先看这一条，别看下面那几条）**：**票 135 AC#8 与票 133 AC#2 同日翻勾**，裁决方＝非实现者
  ...
  - **在飞（17:0x 现量，写码 1／终裁 0／只读 1）**：`worker-ticket119-ac7`（**只许动 `internal/winsec/dataroot_symlink_119_other_test.go`**，票 119 `AC#7` 四条判据，出 `119-ac7-impl.md`，**不翻勾**）
    ＋ `auditor-ticket119-next`（**只读**，替我量 `next=` 第②/⑤/⑥ 三条，只新建 `119-next-audit-r1.md`、不 commit）。
    `acceptor-ticket137-ac4-r1` **已停并交完**（表终版 **754 行／九枚 `3a49745`→`b1ea719`**）；两枚 119 验收程也各自交完（见下面"同格双表"那条）。
