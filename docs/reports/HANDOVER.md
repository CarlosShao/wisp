...
## 4. 在途状态（**最新是下面的 4.0o（09-24 15:1x）；4.0n 及更早各节整段保留供追溯**）

## 4.0o 停车点（**2026-09-24 15:1x 版；新会话从这一节起读**，4.0n（08:5x）及更早只作追溯）
...
- ⚠️ **13:3x 追加（接续的人先看这一条，别看下面那几条）**：**票 135 AC#8 与票 133 AC#2 同日翻勾**，裁决方＝非实现者
  ...
  - **在飞（15:5x 现量）**：**2**，**都是验收程、写码位 0／3** ＝
    ①`acceptor-ticket125-r1`（票 125 的**第一份**裁决表，四格 AC#1..AC#4 ＋ 那笔"动了冻结件 `resolve.go` 而台账无授权记录"的三层问题；
      渐进 commit 已落三枚：`6c0ad3a` §0＋AC#1／`20a6397` AC#2／`a088515` AC#3）；
    ②`acceptor-ticket137-ac4-r1`（**非实现者**终裁票 137 AC#4；开工先证 `git diff b1010ff..HEAD -- internal/winsec/` 为空；
      ⚠ **我给它那两条凭据是按更正版写的**（见下面那节），并另加一发实现方没做的进攻＝**只回退 7 处里的 1–2 处**，
      判据问法"**把这一处撤掉，哪条用例会变得不响？**"——答不出＝那一处是装饰，两形读数一模一样就直接退回不许附条件。）
    ⚠ 两枚都带**争用闸门**（`gh run list` 无 `in_progress` ＋ `docker ps` 干净才取数；撞负载的批作废重跑）。
