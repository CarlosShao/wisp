...
## 4. 在途状态（**2026-09-20 23:19 版，取代上面 08:1x 版**——那版说"无在途任务"，现在读它会让人重做一整批活）
...
- **done 计数：18/65**（本文档 08:4x 版写的是 13/61，已过时）。今天新收口：
  **10**（agent 循环核心，我复验：整体退回旧实现→终止测试 10.00s 变红）、
  **11**（LLM REST 适配器，对抗验收 AC#6 PARTIAL）、
  **62**（液态玻璃球——**owner 判"赝品"，仅 INTERIM 临时通过**，质感返工归票 65）、
  **63**（`wisp secret` CLI，AC#6 转票 12 并已闭环）。
- **正在跑（写码）**：**票 66** SLO 判据仪器返工（`internal/proc`/`internal/observe`/`cmd/wisp/slo*.go`/`scripts/slo-check.ps1`）。
  ⚠ **它占着桌面的独占计时测量权**——它跑完之前**不要**派任何会 `go build`/`go test`/开窗口的代理，
  那会污染它的 CPU 样本（这是我今天自己复跑时证实的：观测者单次读就 ≈1.3ms，250ms 间隔下本身 0.52% > 0.5% 门）。
  前两批代理分别在 102 次与更早被平台闪断杀掉，核心两半已落 `86e868d`/`00bbb76`，剩余插桩由我存成 `63b61ef`。
- **review 中（勿重开已交付的 AC）**：**票 64**（0 BLOCKER/0 MAJOR/3 MINOR；AC#4"不被 skip"缺逐跑记录，
  桌面空后跑 `-tags winlive -v` 逐条记 PASS/SKIP，**交互四项任一走 SKIP 就把 AC#4 退回未勾**）、
  **票 68**（AC#1 三列表 + AC#4 已交付；**AC#2/AC#3 blocked-on-owner**）。
- **blocked-on-owner**：**票 65**（缺图1/图2——到今天为止没有任何代理见过参考图）、
  **票 68 的 AC#2/3**（R15 #3/#4/#5 未答前不得翻 `prototypeVisuals` 默认值）。
- **ready 但被我显式压住**：**票 67 AC#3**（要改 `cmd/wisp/providers.go`，与票 66 同包）、
  **票 69**（C21 表 61 行无机器检查；`internal/ball` 现已空，但等 66 让出 CPU 才派）、
  **票 70**（CI 转绿；**AC#1 是全仓 gofumpt 重写，必须等树里没有其他写码代理**）。
- **工作树**：干净（所有代理都按显式路径提交；我今天三次核对避免了 `git add -A` 吞别人 WIP）。
- **registry 从 A13 一路涨到 A30 + 裁定到 R15**；`docs/reports/pending-and-issues.md` 是唯一真相源，
  **新会话先读它的「待 owner 拍板 R15」九项**，其中 6 项不答就会卡住票 68/70/65。
