...
- ⚠️ **13:3x 追加（接续的人先看这一条，别看下面那几条）**：**票 135 AC#8 与票 133 AC#2 同日翻勾**，裁决方＝非实现者
  ...
  - **在飞（14:4x 现量）**：**1**＝`acceptor-ticket136-ac11-r1`（**非实现者**，只裁票 136 AC#11 那一格，它第一枚 commit `2546b76`）。
    ⚠ **AC#11 现状**：修法落地＝**`f06a8d0`**（作者时刻 14:14:06，只带 `internal/observe/goroutine_test.go`，生产码零改动、无 Sleep／无 Skip／阈值未动）；
    实现方证据表 **`docs/evidence/s1/136-ac11-impl.md`（365 行）** ＋ 我的并排账 **`docs/evidence/s1/136-ac11-orchestrator-readings.md`（§7 是更正节）**。
    数（**分口径写，不许加总成一个百分比**）：改前 整包单发 **2/60（我自跑）**＋2/30＋2/30；插桩重放 **33/500**；同进程 `-count=500` **2/500**＝0.4%。
    改后 **0/60（我自跑，树 md5 已复验＝修后版）**＋实现方 **0/30**。归因＝**两枚空体腿的进出与中段两次计数读之间缺 happens-before**（`Count()` 11/500 少响、`PerTask` 33/500 多响，与红句全落在 `:33` 独立同向）。
    ⛔ **前一程报回的"必须解冻 `internal/observe/goroutine.go`"我判不批**——`f06a8d0` 就是反证。
    ⚠ **今天第三次"误判代理没干活"**（把"它报的 sha 不存在"当成"它没提交"；它的 commit 20 分钟后自己落地了）
    ⇒ **判交付的正确仪器换成三条**：`git log --since=<派它那一刻> -- <它的地界>` ＋ `ls` 它的临时件前缀 ＋ `wc -l` 它该产出的文件，**全空才叫没交付**。
    **⇒ 这一格的勾等非实现者那张三态表，我不给自己翻。** 它交回之后才起 ② 137 AC#4（那格要安静机器）。
