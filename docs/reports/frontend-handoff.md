...
> **⚠ 2026-09-25 06:5x（前端会话 `T4` 的核对结果＝**不成立**，此处保留全过程因为错在我这边）**：
> 它提出"第 5 节把真机差分截屏挂在 `R-92-5`／票 114 `AC#6` 名下是判据错位，`AC#6` 判的是窗口生命周期"。
> 我**回读票面后推翻它的这一条**：`114-*.md:52` 原文就是
> **`- [ ] **AC#6** 可见性：`R-92-5` 欠的那一笔——真机**差分截屏**（改档位前后各一张，能看出输入框上的档位文字变化）`**，
> 且票头 `:5`/`:6` 与 Progress log `:103`/`:106` 三处都写"AC#6＝`R-92-5` 真机差分截屏、保持未勾、`skipped=frontend(owner-delegated)`"。
> ⇒ **上面第 5 节那行表格是对的，不改**。它读到的 `:101` 一带是别的格（AC#2 的门禁读数），**行号错位导致判据错认**。
> ⚠ **我自己的那一发更要记**：我最初把 `T4` 写成"我复票面成立／〔独立复现〕"，而我那一轮**只读了 `:99-:103` 四行、没读 `:52` 的定义行**。
> ⇒ 规矩：**"我回读了票面"必须点到那条判据自己的定义行**；被下游推翻时先核它的**出处**，别核它的**结论**（结论会带着你只去看它引的那几行）。
