...
## 11. 共享工作树里发生的两件必须记账的事

1. **别家的活确实进了 index，但没被我吞。** 第 3 枚 commit 当场：我 `git add` 我的证据文件之后、
   `git diff --cached --name-only` 打出来的是**两**枚路径——多出来那枚
   `docs/evidence/s1/d22scan-gitignore-accept-r1.md` 是验收程在我两次检查**之间**暂存的（本程开局时 index 为空、
   第 1 枚 commit 前也为空）。**处置**：没有停手，因为派单指定的形式本身就是解药——
   `git commit -F - -- <显式 pathspec>` 只会带我的路径；提完立刻 `git show --name-only` 核过，
   三枚 commit（`686d7e7`／`a7792fd`／`50ea2bb`）里 **`d22scan` 命中 0 次**，且它那份暂存条目随后由它自己提交、内容未丢。
   ⚠ 记下这条的**分寸**：这不是"被授权动别人的文件"，恰恰相反——它是"别人正在这棵树上活动"的**实时证据**，
   所以 `git add -A`/`.` 这类形式在共享树里**永远不能**用，本程全程未用。
2. **HEAD 在本程活着的时候一直在被别家往前推**：本程**读到过**的锚点序列是
   `5540087`(开局)→`42d141d`(编排者登记本炸弹那枚)→`98873a5`→`038eb02`→`c42d420`→`a7792fd`(我)→`50ea2bb`(我)；
   ⚠ 这是**我抽样读到的**序列，不是完整历史——两次读数之间它还可能被推了我没看见的枚数，所以别把它读成"一共 6 次"。
   ⇒ 所以本文件每一节的 `file:line` 都写明它读于哪一枚锚点；§1 那张前提核账表、§6 那四数、§8 那普查，
   分别在 `42d141d`／`98873a5`／`038eb02` 上重跑或重读过，**没有拿开局锚点的旧行号充新数**。
   另：派单引的包路径 `github.com/github.com/CarlosShao/...` 是抄写错，真实读数
   `github.com/CarlosShao/wisp/internal/observe`（已在 §1 更正）。

**推送状态**：`git rev-list --count origin/dev..HEAD` ＝ **99 枚未推**（`08:46` 现量，含我这三枚；
⚠ 我早先一次读的是 97，2 分钟内别家又推了 2 枚到本地——**报数前现跑**这条对这种共享树不是套话，是这 2 枚的差额本身）。
本程**从未 push**，按派单与 `AGENTS.md` §1.4 只 commit。
