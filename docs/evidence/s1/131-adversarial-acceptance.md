...
X1…X10 每发都在**独立副本目录**（`/tmp/wisp131-acc131r2-x*`）里就地改，基线快照本身没动过；
...
| **AC#2** models 腿补钉，进程内驱动、不 `t.Skip`、查内容不只查存在 | **通过**〔独立复现〕 | X1 我复算＝3 枚红点名 models；基线钉的 `-v` 原文显示记录 0 是 install 且 `dir` **等于** `logSinkDir(数据根)`、其后恰 1 枚 `models: hand-off REFUSED ... state=Error`、exit=1；**X9** 再把 install 挪到 `<数据根>-elsewhere` ⇒ 这枚钉立刻红（⇒ 它不是"有个 jsonl 就行"）；
`grep 't\.Skip('` 在两枚新文件里 **0 命中**（唯一含 "Skip" 的是 `leg_sink_nail_131_test.go:52` 那句"NO t.Skip"注释——
⚠ 仪器坑照旧：`grep -c 't.Skip'` 会把注释算成命中，我在 121 的账上见过，这次直接用带左括号的形）。⚠ 一格口径要说清（不算缺陷）：钉注入的是 `modelsIO.dataDir`，所以它锁的是"给定数据根 ⇒ 落在 `<根>\logs`"，**不锁**"生产解析出的数据根"（后者由票 117 的 `TestAC3LogSinkLandsInsideTheEnvDataRoot` 守着，见 `R-131-3`）。 |
...
| **AC#4** 收敛判据：清单做成用例、**新增一条腿不给钉 ⇒ 当场红** | **退回**〔独立复现〕 | 前半（从源码枚举、非硬编码、不是恒绿）成立：X1/X2 两发点名红、X6 复现"新腿不给钉当场红"、X3c 证明整枚 switch 换形时它自报仪器坏了。后半被**我造出来两次**：X4 那枚 `--diag` 腿在真二进制上装了听众、写得出 jsonl，门**不红也不报少一条**，再拆掉它的 install 仍 **82/82 全绿**；X8 把 `case "slo":` 的标签写成命名常量 ⇒ 那条腿**静默出账**、门 `PASS`。两发同一根：清单来自"分发写成什么形状"，不是来自"main() 到底有几条腿"。按派单硬线（"AC 声称要防的结局如果被造出来 ⇒ 判退回，不许附条件通过"），这一格判退回，登记 `R-131-1`。 |
...
| 号 | 现象 | 能不能重跑 | 严重度 | 建议归属 |
| --- | --- | --- | --- | --- |
| **R-131-1** | **枚举门的"腿清单"只认两种分发形状，形状之外的腿免检**——`main()` 里 `switch args[0]` 的**字面量** case 与 `if len(args)==0` 那两形。两个独立复现：<br>**形 a（X4）**：一条经早退 `if` 分发的 CLI 腿（`--diag`）**根本不进账本**，门既不红也不报"少了一条腿"，整包 82/82 绿；把它的 install 再拆掉仍 82/82 绿。真二进制上它 `rc=0` 且落得出 `wisp-<day>-<seq>.jsonl` ⇒ 不是我种的死代码。<br>**形 b（X8）**：`case "slo":` 的标签写成命名常量 ⇒ 那条腿**静默出账**（账本里 `leg slo` 零命中、换成两行都叫 `default`），门 `PASS`；落空的正是"记了日志却没装听众 ⇒ 必须有裁决"那一行，也就是 `R-117-1` 的守卫。<br>⇒ 票 131 的立项理由（"装听众、拆掉、没人知道"）在这条门外**第九次逐字复现**。 | **能**：补丁都是分钟级——形 a＝`main.go` +3 行、新增 `cmd/wisp/diag131.go` 一枚函数；形 b＝`main.go` 换 1 个标签 + 1 行 const。快照目录 `/tmp/wisp131-acc131r2-x4-ifleg`、`-x8-constlabel`；读数在 §4 X4/X8 | **高**（本票唯一硬判据那一格被造出来了；今天无产品后果，因为这两条腿是我种的，但它正是"下一次同族"的入口形状） | **票 131 续单**，两条各一行量级：① 断言"`main()` 里除已归类分支之外不再出现 `os.Args` 的读点"（读得到就红、点名那一行）；② `caseLabels131` 吃不下某个标签时（`Ident`/常量）**必须红**，而不是把它当 `default` 或与另一行 `default` 撞键后静默。⚠ **与 `R-121-1`／票 133 不是同一件事**：133 要的是"腿在 `switch` 里、但没人分发到 handler"（**下游**那一跳的可达性），我这条是"**腿清单本身**来自形状而不是来自事实"（**上游**少了一条）。两者同族、互补，且 **133 的 AC#1 硬判据（"必须能红 N-3 那一形"）今天并不覆盖 X4/X8 这两形** ⇒ 建议把这两发作为**票 133 的第二、第三发判据**并进去，别再攒第三张同族票。 |
...
AC#4 声称要防的结局（"新增一条 CLI 腿、装了听众、不给钉、没人知道"）**被我真实造出来两次**
...
### 8.2 能不能挡住第九次 —— **挡不住，但差得很小**

派单最后那一问的答案：它挡住了"**第八起逐字重演**"（拆掉某条在册腿的 install），
挡不住"**换一条不在册的腿**"。X4 那一发只用 12 行生产码就复现了票 131 的立项理由原文。
修法我写在 `R-131-1` 里，两条各一行量级（`main()` 里未归类的 `os.Args` 读点 ⇒ 红；标签吃不下 ⇒ 红），
**不需要**新架构、不需要动别人地界 ⇒ 所以我判"退回续单"而不是"整票推翻"：
AC#1/#2/#3/#5 的交付是真的、可重跑的；只有 AC#4 那条"收敛"主张没到底。

### 8.3 票 131 能不能结案 —— **不能，按本表续一格**

- 阻断项只有一个：`R-131-1`（AC#4 那半格）。它需要一发新变异自证：
  "把 X4 那枚 `--diag` 腿种进**已经补好判据**的门 ⇒ 必须红、红名点到 `--diag`"，还原 ⇒ 复绿。三态读数缺一不可。
- `R-131-2`（归属列）建议同批做（同一枚文件）；`R-131-4`（一句措辞）顺手。
- 结案判我写死成可复算的形式：**X1/X2/X3/X6/X9 五发保持红名不变 ＋ X4/X8 两发从绿变红 ＋ §2 那张表四数不降**，
...
| --- | --- | --- |
| `R-131-1` | `R-121-1` / 票 133 AC#1、`A105④`、`A110④`（"能力装好没人调"第九次半例） | **不是同一件事，是同一族的上下游**。133 核"在册的腿还有没有人分发到 handler"；我这条核"清单收没收到这条腿"。**133 现在的唯一硬判据（"必须能红 N-3 那一形"）不覆盖 X4/X8** ⇒ 建议把这两发直接写进 133 当第 2、3 发判据；若编排者愿意让 131 续单自修前两行，也可以，但**别让它变成第三张同族票**。 |
...
- **〔独立复现〕**（我自己在 `56d8026` 纯净快照上跑出来的）：§1 两记完整性反查、§2 基线全部读数
  ...
  候选①～④ 共 **11 发变异**（X4 分两拍 ⇒ 12 次读数）：X1(=M1)、X2(=M5)、X3(=M-B1/`R-121-1` N-3 形)、X3c、X4 两拍、X5、X6(=M3)、X7、X8、X9、X10(=M6)、
