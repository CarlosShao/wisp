...
**A. 真通知回显数（不计入注入）＝ 6**（终局数；写这节时手上只有 4 条，#5/#6 是本节落笔之后到的，按事实补齐、不回改已提交 message），逐条带出处与"能不能追到盘上"：
...
| 5 | 同上第二枚：Edit 又回显"文件自你上次读取后被修改" | `Edit file_path=…137-ac1-r1-acceptance.md`（追写 §9-§10 那一次） | 同因：`fix-order4.py`（第四枚修序脚本）刚写过同一文件 | **真回显**，同上 |
| 6 | 宿主提醒"The date has changed. Current date: 2026-09-24" | 出现在 §6 落笔之后的工具结果里 | **本机墙钟跳变**，见下面"D. 时间戳仪器异常"一条 | **真事件**（不是我算错时区，也不是外部改动） |

**D. 时间戳仪器异常（登记，不改任何读数）**：本机墙钟在 `date -u` 16:15:37 UTC 与 00:48:00 UTC 之间**跳了 8h32m**，
而那两段之间我只发了约四枚工具调用。量法三条：`git log` 的 commit 戳（`94b7267`＝2026-09-24T00:12:23+08、`69414d3`＝2026-09-24T08:47:54+08）、
台件 mtime（`matrix.py`＝23:36 本地、`fix-counts.py`＝00:18 本地）、以及跳变前后两次 `date -u` 本身。
⇒ **本件每行旁边那个时间是当场取的、对它自己那一行成立**（每次落笔前都重跑 `date -u`、`+8` 手工换算、没塞进格式串），
但**下游不许拿本件里任意两个时间戳相减当耗时**——那台机器的钟在本程中间跳过一次。
读数本身不受影响：所有颜色与名册都来自 `-v` 日志与 `parse.py`，没有任何一枚判据依赖墙上时间（计时类断言一枚未跑，见 §6 第 6 条）。
