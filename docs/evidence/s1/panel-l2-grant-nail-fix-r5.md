...
> **§6/§1.1 四处更正与补列（F-R5-1，非实现者表 `panel-l2-grant-nail-fix-r5-accept-r1.md` §11 给的原话，编排者 16:2x 时代抄落盘）**
> 本节**不改写上面任何一行正文**，只追加。四句按表逐条：
> **(a)** §6 那枚残余 `X-list+row+pin` 是**一族三枚**、不是"一枚"：C01 前缀（全绿 `asked=462`）／C02 后缀（`352`）／**C05 words（`480`）**。⇒ 下一程别只去找前缀那一枚。
> **(b)** 同一段应补一句：**"两点协同已守"**（K2 的 11 枚证人 ＋ C03 全红，验收程在 `ddea3c3` 真身副本上重打复算）。**不补这句的后果是明确的**：下一程会把"协同编辑结构上守不住"当成已定案，从而**不去复查那 11 枚**。⇒ 本件的边界因此**不是**"改测试文件的人守不住"，而是**"同一枚文件内三处互相圆场的改写"**（可核的三点边界在验收件 §7.3）。
> **(c)** §1.1 理由 1 的归因要换成验收件 §10 ⓓ 那一发：正控留 4 枚路由的**独占依据**是 `panel.mode.add` **多答**形（F05 红／F06 绿）；原句那种读法会把"少答声明路由"当成只有正控在兜——实际 `bridge_test.go:68` 与 `:2053` **也兜**（F07）。⇒ 结论不变（该留 4 枚），**理由换掉**。
> **(d)** "两处协同才拆得掉 F-R4-1"这句需加限定：**改名那枚常驻用例仍是一枚编辑**（I02，`RUN=98`），只是那一枚**必动名册** ⇒ 它的兜底是人工名册差集，不是这把尺本身。
