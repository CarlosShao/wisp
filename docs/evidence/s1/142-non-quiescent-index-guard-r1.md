...
**枚数更正（append-only）**：上面那句"这批的四枚 commit"少计了。本程落在 `dev` 上的完整一枚序列是
**六枚**：`9d06544`（生产 + 用例）· `64b404d`（用例三腿）· `640cff1`（本文件 §0-§3）·
`cb60b94`（§4-§5）· `1ed231e`（用例块 5a）· `9d7a3f3`（§6-§8）。
其中**只有前两枚带生产代码**，路径集合仍是 §5.1 那四枚文件；核对用
`git show --name-only --format="" 9d06544 64b404d 640cff1 cb60b94 1ed231e 9d7a3f3 | sort -u`。

**交件状态**：票面 4 格一律未勾（勾由编排者对着非实现者的表落）；未写
`docs/reports/pending-and-issues.md`；未自派 `A##`；未 push；未改名 `-done`。

> **三行 append-only 更正（09-25 13:5x 编排者代非实现者落；原句一字不抹，出处＝`142-non-quiescent-index-guard-accept-r1.md`）**
> **① §1.2 那枚 `scan_test.go` 的哈希过期**：本件引的是 `e64b9631…`，那是 **`9d06544` 那一版**；交付版（含 `64b404d` 三条腿之后）是 **`3fa6c24b…`**。⇒ 形状＝本仓那条"引别件的读数必须带它是在哪一版上量的"；下一位拿 `e64b9631` 去对 `HEAD` 会对不上而误判"文件被改过"。
> **② §3.4 那句唯一性说重了**：原文写"存在一种变异能打不红任何其它东西"。验收程实测**不成立**＝删掉块 3／块 4 之后，块 5 那枚静止前提**照样红**（验收件 `AR-3`／`AR-5`）⇒ 这两块**有牙但不唯一**。⚠ 这不改总裁（三味单点回退各有红：`quotePathList`→3 枚、`maxStraysNamed`→恰 1 枚、`strconv.Quote`→3 枚），改的是**这句话能不能被下一位当"只有它能守"的依据**。
> **③ §6 第 2 项"没跑 CI"已被今天覆盖**：本件写这条时确实没跑；`cd87354` 之后编排者推过两次，`ci` run `36094258734`（job `lint 107943039081`）里 **D22 两步＋`gofumpt` 步全绿**、那枚新用例在 ubuntu `--- PASS`、真扫描八枚数与验收程的纯净快照**一字不差**；同一 job 那枚红的 `staticcheck` 排在 D22 **之后**、与 `tools/d22scan` 无关。⇒ 这一格**不用再补人跑**，但也**不许**由此把"ubuntu 那半边有分母"升成"整链已验"——Linux 容器那一发是验收程自己真跑的（挂卷当场证明非空：17 条目／2382 行）。
> **另外三笔入账挂在编排者名下（不折进本件）**：**F-142-1** 生产那一跳今天**无断言持有**（装配口被人挪走 ⇒ 用例全绿：验收程 `S-4` 实测；`buildGitIndexState` 生产唯一入口 `gitignore.go:282`）；**F-142-2** `note()` 第三行的**自陈会说谎**属**先于本票**的缺陷（`1b98c7b:459`，本票 diff 零命中），本票把"窄集溢出"那一支**结构性堵住了但没有任何断言守着**（`ignoreSet` 全仓仅 3 处引用）；**F-142-3** spawn 顺序掉包＝同族残余，注释级。⇒ 三笔都进下一枚 `tools/d22scan` 票，**不许**把 5a/5b 写成"生产那一跳已钉住"。
