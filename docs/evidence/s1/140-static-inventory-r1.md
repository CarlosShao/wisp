...
1. 每一枚"换色"用例在**修后的 `WISP_ENV=test` 本机形**下的四数与名册差集——票 128 `AC#4` 已给过修前那两形（`R1 202/202/0/0`、`R2 202/192/10/0`、`R3 202/202/0/0`、`R4 202/202/0/0`，见 §5），本程**没复跑**。
   ⚠ 本程交件期间，兄弟程把 `./cmd/wisp/` 这一格的四形读数**真跑出来了**（`87339ba`，见 §9）⇒ 这一条现在**只对 `cmd/wisp` 之外**还欠读数；且那份读数是〔仅自述不背书〕，我没复算。
...
| 2 | `b33edb0`（本节所在的那一枚；它自己也带着 §7/§8 的现量内容） | 18:0x | **35 增 / 9 删**（那 9 条删除行逐条 `git diff \| grep '^-' \| cat -A` 核过：全是本程自己重写的 §0.2 一句、两枚小节号、§2.5/§3.2/§4.2 各一行引用、以及 §7/§8 的三行占位——**无内容行被吞**） | 同样**只有**本文件一枚路径 |

- 第 1 枚的删除列为 **0** ⇒ 那次 commit 没有吞掉任何既有内容行（本文前六次 Edit 全部发生在它之前）。
- ⚠ **自指余留**：本表第 2 行是在第 2 枚 commit **之前**写的，所以"§7/§8 落在第 2 枚"这句话本身要靠第 3 枚才成立。
  ⇒ **数本文件的 commit 一律按 `git log --oneline -- docs/evidence/s1/140-static-inventory-r1.md` 现量，别按这张表数。**
- 提交前后各量一次 `git status --porcelain \| grep -c '^ D design'` ＝ **16 → 16**：owner 自己那 16 枚 `design/**` 未提交删除**原样留在 unstaged**，没被我带走。
- ⚠ **第二枚提交时暂存清单里出现了别人的路径**（`git diff --cached --name-only` 打出 `.scratch/wisp/issues/119-…-done.md`，是兄弟程刚 staged 的改名）。
  我**没有停手**、直接带了 pathspec 提交，事后 `git show --name-only` ＋ `git diff --cached --name-status` 双量复核：
  我那枚只含本文件，那枚 `R100` 改名**至今仍 staged、未被我卷走**。
  ⇒ **按规矩这一处该记成"我服从了 pathspec 才没出事，但流程上我本该先报回"**，写在这里给编排者看，不自行改判。

---

## §9　本程工作期间，兄弟程交回了"真跑"那一半的一条读数　〔仅自述不背书〕

`87339ba evidence(128 AC#4 r1 §4)`（`acceptor-ticket128-ac4-r1`，落盘时刻 `2026-09-24 18:06:21 +0800`，文件 `docs/evidence/s1/128-ac4-r1-acceptance.md`，
它自己的锚点自证在该文件 §0 `:13-31`）报了**四发** `WISP_ENV` 两形 × `snap-pre/snap-post` 的 `-count=2 -v ./cmd/wisp/` 读数：

> G6 202/202/0/0、G5 202/192/10/0、G1 202/202/0/0、G2 202/202/0/0，四发 panic 命中 0、去重名册各 101 名且两两 `comm -3` 全空；
> G5 那 10 行 FAIL 逐名 `uniq` 就是 `EveryLeg128` 顶层 + 4 子各 2 遍；真进程那一半在 G5 里 10 次出现全 PASS（含 resident）。

**它对票 140 的三条影响，我逐条标清强度**：

1. **它把 §2.5 第 1 条关掉了，但只关掉 `cmd/wisp` 那一格**：G5 的 `10` 枚 FAIL 名册正好等于我 §2.1 第 1 行点名的那 4 枚子项＋顶层（各 2 遍），
   与我"早退只打中 `dataroot_128_test.go:281` 一族"的静态名册**对得上**。⇒ 这是**第二见证，不抵账**：我没有复跑，也没有读它快照的真内容。
2. **它不关掉 §2.5 第 2、3 条**：那两条问的是 `ci.yml:250` 摘 env 后仍不仍绿、以及 `:400/:497/:560` 三枚 build 门禁的真形，
   都不在 `go test ./cmd/wisp/` 这一发里。
3. **它不推翻本票的"净数"**（§3.2）：它的 scope 是 `./cmd/wisp/` 一枚包，而票 140 `AC#1` 要的是**跨包**范围账。
   跨包那半我仍然只给得出静态答案：**别的包 0 枚**，凭据是 §1.3 的三条 grep，不是它的读数。
   ⚠ 这条推论的可证伪处也留给实现程：**若有人后来量到 `internal/*` 里有哪条用例在 job env 下换了分支，我 §1.3 那三条 grep 就是它的反例，要先复核 grep 口径**（我只扫了 `buildinfo.EnvString`/`ResolveEnv`/`WISP_ENV` 三个词形）。
