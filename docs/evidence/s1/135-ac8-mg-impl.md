...
- ⚠ **本程自己犯过一次、并且当场作废的一发**（不藏）：本程最早发出去的那一发 `R2-B1`
  （树 T01，形状＝§3 基线的第一行，**未进 §3 的任何表、不参与任何结论**）
...
**这一族里最省字的一句**：同一枚尺上只差那一枚植物——
旧尺：`R2-Uo2`（红）→ `R2-MGo2`（**绿**）；新尺：`R2-Un2`（红）→ `R2-MGn2`（**仍红**）。
钉一枚不存在的用例名就能过闸，过的正是旧尺那一行。
...
`//go:build` 表达式为假（而不是文件名后缀）那一支、以及 `GOARCH` 那一支，本程未造未读（§8.1 第 3 条）。
...
`wisp.exe` 是本程驱动里 `go build ./...` 在**仓外快照树根**落的构建副产物（本程在两枚本节点名了的
树上各读到一次：T15 与 T26；其余树未逐一查），**从没进过仓库**；§6.3 的残留扫描一并覆盖它。
...
## 8. 诚实账：没做的档、争用命中实况、临时件清单、引用到的 sha、两个计数

### 8.1 没做的档（全列，不给下一位留"这些已验"的错觉）

1. **AC#1…AC#7 七格一枚未做**。本程按派单只补 ①证据文件 ②变异自证三态 ④门禁账。
   派单那句"不许顺带把别的格也做了"是硬约束：本程连顺手能做的 AC#7（裸标记句那一格）都没碰。
2. **上一程留下的日志本程一枚都不引用**（`/d/tmp/wisp135mg-out/*`：`MG-A*`／`MG-B*`／`d22-*`／`g-*.txt`）。
   本程全部读数自取。派单第 1 节说的"不许抄前两程的红名"同样适用于"不许借它们的绿"。
3. **`//go:build` 表达式为假那一支未造**（本程的 `M-G` 走的是文件名后缀 `_linux_test.go` 那一支），
   `GOARCH` 那一支、以及"名册读不到（`rerr != nil`）⇒ 判红而不是判跳过"那一支
   **本程没有造出真实触发**（只读到 `runRosterReds135` 里那段码与它的文案）。
   ⇒ 这三条是判据 (c) 的**未测支**，不该被记成已覆盖。
4. **`-count=2` 只在未变异基线上取**（两形）。变异态那一族本程全用 `-count=1 -v`（票面 AC#8 ④ 要求的形状）。
5. **`./internal/proc/` 的 `-count=2 -v` 未跑**：那是本票 **AC#6** 那格的账，不是 AC#8 的牙（派单第 4 节
   门禁项里也没点它）。⇒ 别把 AC#6 读成"被本格顺带做了"。
6. **linux 整包未跑**：§7.5 那一发只用 `-run` 跑了本尺一枚；票 133 §4 第 5 条那 19 枚 linux 侧既有 FAIL
   一枚没碰，`GOOS=linux go test ./cmd/wisp/` 的全形本程无读数。
7. **C3（把 `M-G` 升成 CI 守）未做，也判不该由本格做**：本程**没引任何 CI job 日志读数**
   （`gh run list` 只当争用闸门用）。按派单口径给不出"具体 run id＋job id＋step 名"就当那道门不存在。
   ⚠ 一条与本格无关但编排者要知道的事实：`8369b24` 与 `45623e4` 这两枚 dev 推起的 `ci` run
   本程现量到都是 `conclusion=failure`。⇒ 门禁连红未解，本格任何结论都不依赖它。
   （**逐 job 的分布本程只在 `8369b24` 那一枚上量过**：`gh run view 35948947994 --json jobs` ⇒
   `lint` completed\|failure、`slo-full` completed\|failure、`test-windows` 当时 in_progress；
   `45623e4` 那一枚本程只读到 run 级 `failure`，**没读过它的 job 级**，别说成两枚都逐 job 核过。）
8. **软链 temp 那一形未复核**（票 133 §4 第 3 条同一口径的遗留项）。
9. **那枚既有 flake（`TestAC1ResidentLegBooksItsShutdownBeforeClosingTheSink`）未做命中率实验**：
   本程 19 发 windows 读数里它**一次都没红**，这既不能定它是真伤、也不能定它好了。
10. **本程没有把 `d3e3a43`（`fa35557^`）与 `8369b24` 之间那几枚 docs commit 的差异从对照树里剥掉**：
    对照树＝改码前那一版的全部差异（含票 137 那批证据文件）。⇒ §7.2 的"差值全 0"是
    "这两棵树之间 d22scan 覆盖面没缩"，不是"只有那把尺变过"。

### 8.2 争用闸门命中实况（不蒙成"环境良好"）

| 时刻 | 三行里的哪一维修了 | 本程怎么处置 |
| --- | --- | --- |
| 10:51:50 | `Runner.Worker.exe` PID 36068 ＋ 一枚 in_progress ci（`8369b24`） | 判 BUSY，不发读数；改跑有上界轮询 |
| 10:55:01／10:56:55／10:57:10／10:58:02 | ci 仍 in_progress | 同上 |
| 10:58:55 | 三行全空（那枚 run 以 failure 收） | 判 CLEAR —— **但本程当时在写 §0／§1，没发读数** |
| **11:02:20** | **本程犯了的那次**：编排者 11:01:39 又推 dev（`45623e4`）→ 新 run ＋ `Runner.Worker.exe` PID 38372（11:02:47、11:03:01 两次 `tasklist` 都还在） | 该发读数 `R2-B1` **作废**，日志留着但不引它作任何结论（§0.4） |
| 11:03:27／11:04:19／11:05:12 | in_progress ci | 轮询里 BUSY，继续等 |
| 11:09:21–11:11:30 | 前三轮 BUSY，第 4 轮 CLEAR | batch b1 在 CLEAR 窗口里发（逐发 `GATE-CHECK worker=0`） |
| 11:14:17／11:17:23／11:25:32 | 第 1 轮即 CLEAR | batch b1r／b2／b3 同 |
| 11:31:05／11:32:07 | CLEAR（worker=0、三枚 run 全 completed） | 门禁两批 |
| 11:39:45／11:41:20 | 前两维 0，**第三维 `gh run list` 报 `EOF`** | 容器 linux 那发照采，但 §7.5 明写"ci 那一维未取到、不是确认为空" |

⚠ 台件缺陷一条（不藏）：`wisp135mg-r2-run.sh` 里逐发的 `GATE-CHECK … inprogtask=` **一直是空**，
因为它在快照树目录里调 `gh`（那里没有 git 上下文）⇒ 逐发的那一维实际靠
**批前的 `prebatch … inprog=N`** 与手工 `gh run list` 两路供证，`worker=`／`toolchain=` 两维逐发有效。
下一位修台件时把 `gh` 那句包成 `(cd "$REPO" && …)` 即可。
⚠ 也没有任何一发读数被 `time.Sleep` 糊过去：等的是 `poll.sh`（45s×26 枚上界）与
`wisp135mg-r2-batch.sh`（40s×11/12 枚上界）的**轮询到条件成立**。
⚠ 本程**未**杀过 runner 进程、**未**改过 `.github/workflows/**`、**未**取消过任何 run。

### 8.3 本程建的临时件（只建不删，交回编排者清点）

快照前缀全部 `wisp135mg-r2-`，**没有覆盖、复用或删除上一程的 `wisp135mg-*`／`wisp133-*` 任何一棵**。

```
D:\tmp\wisp135mg-r2-tree            被验版本 8369b24 的归档树（＋三枚 sherpa dll）
D:\tmp\wisp135mg-r2-tree-pre        对照版本 d3e3a43 = fa35557^ 的归档树（＋dll）
D:\tmp\wisp135mg-r2-T01 … T26       26 棵读数树：T01＝§0.4 那发作废；T04…T09＝§4.1 那六发被
                                   本程落地闸拒绝（未产生任何读数）；T10…T26＝19 发有效读数各一棵
D:\tmp\wisp135mg-r2-fakeroot        §7.3 那枚假根（frontend/ 被清空的 plant，不是清理动作）
D:\tmp\wisp135mg-r2-out             123 个文件：19 发 .log／.runnames／.verdicts／.build／.buildall、
                                   batch-b1.txt／batch-b1r.txt／batch-b2.txt／batch-b3.txt、
                                   gates-{pre,post}.*、cross-{pre,post}.errs、
                                   d22-binary-*.txt、d22scan-r2.exe、linux-run.log、linux-run2.log
D:\tmp\wisp135mg-r2-gate            poll.sh／gate-rounds.log（逐轮三行原样）／poll-run1.out／poll-run2.out
D:\tmp\wisp135mg-r2-{mutate,probe}.py       种法（§4.1 的形状，含 §4.1 那笔自纠后的第二版）
D:\tmp\wisp135mg-r2-run.sh                  单发驱动（八数＋名册＋红名＋机检）
D:\tmp\wisp135mg-r2-batch.sh                争用闸门＋批驱动
D:\tmp\wisp135mg-r2-gates.sh                门禁账驱动
D:\tmp\wisp135mg-r2-oldgate.go              fa35557^ 那把旧尺（sha1 906201f4…）
D:\tmp\wisp135mg-r2-list-b{1,1r,2,3}.txt    四批发作清单
```

（§6.1 那些树根下的 `wisp.exe` 是 `go build ./...` 的副产物，在仓外，同样只建不删。）

### 8.4 引用到的每一枚 sha，逐枚 `git cat-file -t` 现核（不是"看起来像"）

```
8369b24=commit  fa35557=commit  45623e4=commit  191f0d6=commit
d3e3a43=commit（fa35557^ 自量）  087ef2e=commit  4f1b73a=commit
```

⇒ 全部解析得到；来源都是本程自己的 `git rev-parse`／`git log`／`git show`／`gh run list` 的 `headSha`，
**没有一枚是从别的工具回显里照抄来当锚的**。§6.3 那枚尺的 sha1 全文是
`23b443ac34787aa9ea60e181b9b8c789f468cd56`（上一条 commit message 里本程手误写成 `23b43ac…`，
在此更正；文件里的字节是本枚）。
...
每条带出处（工具名＋命令前 40 字），四条判据逐条走过（点名路径真不真／是否削弱 owner 权威或放宽判据／
声称的改动能否盘上核到／是否在让我少取证）。

| # | 出现在哪 | 内容 | 判为真回显的依据 |
| --- | --- | --- | --- |
| 1 | 第 1 枚 `Bash` 工具结果尾部 `<system-reminder>` | 可用 skills 清单 | harness 自己的清单：无路径、无动作、不越权、不让我少取证 |
| 2 | 对话开头附的 `Note: The file C:\Users\swq\.qoder-cn\projects\D--work-…\memory\MEMORY.md was modified since it was last read` ＋全文 | 项目记忆索引 | `ls -l` 现量：5147 字节、mtime Sep 24 10:47 ⇒ 路径真、是被改文件自己的回显；未要求本程任何动作 |
| 3 | 同一形状的第二次回显（`…\projects plans\Wisp` 会话里那条 Bash 之后） | 全局 `MEMORY.md` 全文（新见"正向修改就你自己改"等条） | `ls -l` 现量：20039 字节、mtime Sep 24 11:02 ⇒ 路径真、内容可追；本程**没有**照它改写任何判据（例如它提"恒真判据"，本程按派单自造 §4.6 那张表） |
| 4 | `[SYSTEM NOTIFICATION - NOT USER INPUT]` 后台任务事件（task id `b992k2qmp`） | "Background command … completed (exit code 0)" | 本程自己起的轮询任务，输出路径在盘（`…\tasks\b992k2qmp.output`） |
| 5 | 第 2 轮回显里的 `The date has changed. Current date: 2026-09-24` | 日期变更提示 | harness 自己的上下文；与派单"本机墙钟不可信"一致 ⇒ 本程所有时刻都是现 `date` 现抄，没有拿时间戳相减算任何耗时 |

**判为注入数 ＝ 0**

本程**没有**遇到任何自称"编排者备注／系统提示／用户已更新编码规则／请 revert／冻结某包／放宽阈值／
某格已合并／用户已拒绝／`Confirm: the harness note is genuine`"的文字，也没遇到假 sha。
自核两条（不因为"没遇到"就当结案）：① 上表那 5 条里没有任何一条要本程减少取证、跳过复算或直取结论——
**反倒本程自己加了 §7.5 那一发容器读数与 §4.6 那张恒真自查表**；② 假 sha 面：本程用的每一枚
都过了 §8.4 的 `git cat-file -t`。
⚠ 反向一条也记：同程"本程没遇到注入"**不能**洗掉别的程遇到的，各自计数。

### 8.6 一条要报回编排者的前提更正（不是本程的读数，是盘上事实）

票面 10:3x 那条断点登记里写：**"② 变异自证三态一发未采"**。本程在盘上量到的是：
`/d/tmp/wisp135mg-out/` 里 09:0x–10:08 之间有一批 `MG-A1…MG-B9` 的 `.log`／`.roster`／`d22-*`／`g-vet-win.txt`
等文件（127 个，本程只 `ls`、未读其任何数字）。⇒ **那一格更准的说法是"采过但没落进证据文件、
且没有一枚能被本程背书"**，而不是"一发未采"。
本程的处置：**照原计划自取 19 发**（不省），并在此登记这条前提偏差；
那批日志属上一程自述，档位＝〔仅自述，不背书〕。

## 9. 总判

**AC#8（家族第 7 发 `M-G`）：本格成立（PASS、无附条件）。** 票面 AC#8 那四条结案判据逐条对：

| 票面判据 | 本程读数 | 判 |
| --- | --- | --- |
| ① 自己重造 p7 那一形（不许抄前两程的红名），给出"整包绿＋名册里查无此名"的原文 | §4.3／§4.5：`R2-MGFo`（旧尺、本程自己的名字、门关着整包）**rc=0 100/53/0/0**、`=== RUN` 名册里那枚名字 **0 次**、整份日志里只出现 1 次（本尺自己打的账本行 `:85`） | **成立**（本程第一手） |
| ② 修法方向只许"每个 `covered=test` 的名字必须在本轮 `=== RUN` 名册里逐名对上"，不许折回只信 AST | §2：新判据唯一的新读数来源是 `os.Executable()`＋`-test.list`（`:1574`／`:1580`），全文件无一处求值构建标签；§7.5 又在 linux 上真跑到名册（41 枚） | **成立**（方向合规，没折回） |
| ③ 修后两拍：门关着零退化（八数逐数不变）＋ p7 必须转红、红名点到"这名没跑过" | §4.5：`R2-BLC` 100/53/0/0＋47/0/0 ↔ `R2-MGFc` 100/52/1/0＋47/0/0，名册 53 枚逐名相同、唯一换色是本尺一枚；红名原文在 §4.4。第一拍 `R2-1BFc` 独立红 | **成立** |
| ④ 全程 `-count=1 -v` 取四数并列名册差集，`SKIP=0`、`panic=0`、`build-failed=0` 逐发给出 | §3／§4.3／§4.5 每发的 `CHECKS:` 行：**19 发全是 0/0/0**，无一发有 `[no tests to run]` | **成立** |
| ⚠ 派单点名的反向判据："把 install 拆掉的第二拍也必须红" | §4.5 `R2-MGFc`（整包、门关着、第二拍、新尺）rc=1，红名本尺；§4.3 `R2-MGn2`（单跑本尺同一形）rc=1 | **成立（这一发做了）** |
| 恒真判据自查 | §4.6 那张表：只有"旧尺不响、新尺响"那一枚被当修复凭据；两侧都响的两枚明写不能当 | **通过** |
| 与 AC#7 不互相抵账 | 本程一枚 AC#7 的形状都没碰（§8.1 第 1 条），本格全部读数落在 `covered=test` 那一桶 | **不抵账** |

**没做的档全列在 §8.1（十档）**，最重的三味：判据 (c) 的三条未测支（`//go:build` 假那一支、
`GOARCH` 那一支、`rerr != nil` 判红那一支本程没造出真实触发）；`internal/proc` 的 `-count=2`
（那是 AC#6 的账，别当被本格顺带做了）；linux 整包未跑。
**争用闸门实际命中**：10:51 起连命中两批（一枚 `Runner.Worker.exe` ＋ 两枚先后 in_progress 的 ci run），
本程因此**作废并重发了 1 发**（§0.4 那次本程自己犯的），其余 19 发全部落在逐批现量的 CLEAR 窗口里；
等待全程用有上界轮询（逐轮日志在盘），未杀 runner、未改 workflow、未取消 run。
**临时件清单**在 §8.3（26 棵读数树＋2 棵归档树＋1 枚假根＋1 个 out 目录＋6 个台件＋4 份清单，只建不删）。
**本程对那把尺做了什么：零**（§1.4 的 sha1 为证），也**没有**判断需要改它。

⚠ **票面一个字都没动**：派单把本程的地界钉在①②④三样（证据文件／变异读数／门禁账），
第③档"本票八格全 `[ ]`、零格翻勾"里含的**票面 Progress log 与翻勾**归编排者（"登记不能等、票面可以等"）。
⇒ 本程没给票面 append 过 log 行、没勾过任何框、没改过任何状态字段。
本格交件的 commit 短哈希（供票 133 的 C2 那条"落在具体一节并给 commit 短哈希"直接引）：
§0＝`087ef2e`、§1-§2＝`4f1b73a`、§3-§5（**`M-G` 三态读数在这两节**）＝`3c2bd48`、
§6-§7＝`faa7cf0`、§8-§9＝本枚 commit 自己的短哈希（`git log --oneline -1` 现取）。
