...
- **但 ⓐ 不是"唯一能让这一格变绿的形状"**：本程能列出票面没枚举的两支——
  ...
  ⚠ **本程把这两支写得太轻了，这里就地收紧**：AC#2 的触发条件是**逐枚点名**的
  （"只有'就地改掉返回值里的 `Params` 或那三枚切片'才能触发"）⇒ ⓓ/ⓔ 把 `Params` 从返回值里拿掉之后，
  **那一发按字面就写不出来了**（无从下笔 ≠ 写了且为真）。它们要成立，必须先把 AC#2 改写成
  "对返回值里**任何**引用槽就地写"＝**改票**，与本程在 §6.3 判为假句的那两条是同一类动作。
  ⇒ 所以正确的说法是：**在票面给的两支里，ⓐ 是唯一收得下 AC#2 字面的**；ⓓ/ⓔ 是**下一张票的形状**，
  不是"实现者本可以顺手选的第三条"。**这一条不追加成本票 146**，只是把本程自己那句"ⓓ 更便宜"的预期
  钉在"要先改 AC#2 措辞"这一格上（它的省预算账仍按 §9 那发归因：省 64 % 字节／22 % 分配次数）。
