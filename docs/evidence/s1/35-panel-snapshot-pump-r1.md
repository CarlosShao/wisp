...
> **收尾程更正（F-PUMP-5，2026-09-25，锚点 `7af2e13`）：上面那句"五枚里只有 2 枚"的分子配错了，
> 按本件自己这张表应是 1/5。原句一字不抹、上面那张表一字不动——表里每格读数都对，
> 错的是从它得出的那个分数。**
> 逐行重走"新增在包外吗"那一列：打 ✅ 的只有两枚，`UnsetWorkspaceView`（`cmd/wisp/panel_pump.go:84`）
> 与 `WorkspaceViewFromRoot`（`cmd/wisp/panel_pump.go:86`）；而**后者不在 `5821e24` 点名的那五枚里**
> （那五枚是本节表头列出的 `NewSnapshot`／`NewComposerState`／`NewModeView`／`ModeUnknownView`／
> `UnsetWorkspaceView`；`WorkspaceViewFromRoot` 与 `CardViewFromDecision` 是本程为完整起见附在表尾的另外两行）。
> ⇒ **五枚里 gained 跨包调用者的只有 1 枚**；那句"只有 2 枚"是把表尾那一枚 ✅ 记到了五枚的账上＝分子配错分母。
> ⚠ 一句口径，免得下一程把两把尺的读数互借（本件与验收件在这段上是两把不同的尺，**两把都对、数不同**）：
> 本件表里 `UnsetWorkspaceView` 那行"改后 4（新增 2 枚）"数的是**全部非测试调用点**（同包 `pump.go:163` ＋
> 跨包 `panel_pump.go:84`）；验收件 §4.3 把同包与跨包分栏，于是同一枚符号读成"同包 2→3、跨包 0→1"。
> 4 = 3 + 1，两把尺说的是同一件事。**跨包那一栏的总数仍是 4 枚**：除上述 2 枚，另两枚是
> `NewStreamLog`（`run.go:420`）与 `NewSnapshotPump`（`run.go:421`）＝**新符号自己的建造点**，
> 不属于"某枚既有建造者 gained 包外调用者"。
> ⇒ 这一段的准确说法只到这里为止：**五枚建造者（`5821e24` 点名的那五枚）里只有 `UnsetWorkspaceView`
> 一枚真 gained 跨包调用者；1 枚（`NewModeView`）前后一枚都没多。**
> 对本次更正的方向性核对（**这是一句推理，不是测量记录**，别当读数引用）：把 ✅ 从
> `WorkspaceViewFromRoot` 那一行抹掉，上面那句结论一个字不用改；从 `UnsetWorkspaceView` 那一行抹掉，
> 结论才要改 ⇒ 那枚 ✅ 从来不该进五枚的账。
...
> **收尾程更正（F-PUMP-1，2026-09-25 18:0x，锚点 `7af2e13`）——被更正文里的三个数（6／3／1）撤回，原句一字不抹。**
> **为什么撤回、而不是补测量记录**：本件 760 行里没有任何一处把这把"会亮的尺"的定义
> （量哪一族符号、怎么去注释行与测试行）、那两枚历史提交的 sha、或逐枚命中清单写下来过。
> 本程现量：全件匹配"亮 N 次"这一形的行**只有被更正的那一行**（python 扫全文 → 1 命中／760 行）；
> 唯一一枚与"尺"同段出现的 sha 是 §2.3 的 `94071ff`，那打的是**另一把尺**
> （"有没有新造通道"：`http`／`websocket`／`ListenAndServe`／`os.WriteFile` 一族，正控 7 枚命中），
> **量不是这一段的入站授权边**。⇒ 既然出处找不回，两条出路里：ⓐ"把那把尺的定义＋sha＋命中行原样补进件里"
> 本程**做不到**——能补的只会是本程自己另造一把尺的读数，把它挂到上一程那句话底下＝给一句从来没有过出处的
> 自述**造一个出处**；把验收程 `35-panel-snapshot-pump-r1-accept-r1.md` §1.2–§1.4 那三张表抄进来充数
> ＝把别程的读数伪装成本程的（派单点名禁这一条）。⇒ 所以走 ⓑ：**数值撤回、不替换**。
> **撤的是那三个数，不是这一段的结论**：本节真正的主张是"没有任何一条入站批准被接上"，
> 它在**本件**里有自己的现量：§1.2 那四条（WebView2 宿主侧调用 0 处／`BuiltinAssets` 唯一生产调用者是 CLI 诊断／
> 无 HTTP·SSE·websocket 服务端／`runResident` 不 import `internal/panel`）＋ §6.3 的 C17 名册 18 枚全量（各 0，
> 两枚非零逐枚处置）＋ §6.4 的改前／改后对照。这三格不受本次撤回影响。
> ⚠ 顺带把**归属**写清，免得下一程读错：验收程用**它自己另造的尺**在三枚正控上量到 7／4／2、
> 在被验那批量到 1（它的 §5.1）。那是**它那一程的尺与读数**，既不是被更正文里那把尺，
> 也不是本件那三个数的复算——**两组数对不上这件事本身，就是本件当初没留测量记录的症状。**

> **对上面这块更正自己的更正（同一程，写下后本程复量发现自己那句 over-claim，原句不抹）**：
> 那块里说"**唯一**一枚与'尺'同段出现的 sha 是 §2.3 的 `94071ff`"——按"sha 与'尺'同段"这把谓词现量，
> 命中的不止一枚（python 扫：行 232／343，另加本程自己刚写的那块）。**行 343 是第三把尺**
> （`Snapshot` 四键与 `panel.ts` 的"双向尺"，配 `aeba6ff` 的 `git diff --stat`），同样不是入站授权边那把尺。
> 谓词换成"sha 与（入站／路由／正控）同段"时**才只有行 232 一枚**（这一条本程也现量了，命中 1）。
> ⇒ 结论不动（那三个数仍然没有任何出处），但"唯一"那两个字是我说满了，按上面这两个谓词各一枚读数收回。
