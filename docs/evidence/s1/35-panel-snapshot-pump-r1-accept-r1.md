...
| 格 | 判 | 一句话凭据 |
|---|---|---|
| **一：没有接上任何一条入站路由** | **成立** | 本程另造的尺（三族具名符号、只量非测试行）打在被验四枚上 `ADDED = 0/1/0/0`，那唯一一发是 `run.go:423` 交出一枚 `func() risk.Mode` **只读档位读数器**；正控三枚真提交 `12d8e28 +7`／`8e10095 +4`／`11f3927 +2` 证明尺会亮、`63ef895 +0` 证明它不是大锤；结算族 20 行在两棵树间逐行相同、全留在 `internal/agent/approval` 之内 |
| **二：§4 逐字段承重自证** | **成立**（三发复现 ＋ 三发自加） | A1→M1 同枚同名同红句、A2→M3 两枚红句同两处、A3→M4 两枚红名同两名 **且** 另一腿独立证实 §4.4"生产钟是装饰品"；自加 B1 量出装配层搬运另有自己的钉、B2/B3 补掉 §7 第 10 条那发移除级 |
| **三：§7 第 2 条并发形状** | **成立为一枚已定性的洞（升格，不再记"残余"）** | 可达性已量（四行链到 `pump.go:136`，worker 上限 4、`run.go` 未设 `ToolConcurrency`）；内存安全已量（两包整包 `-race` 全绿）；定性＝显示层撕裂、**不产生授权后果** |

**"面板侧来源的 L2 允许"这条硬禁线：没碰。** 但要写成**射程**而不是"已被接线挡住"：
`DecideFromPanel` 那枚"allow 一律拒、还给真 grant 烧 nonce"的保险丝（`gate.go:622-636`）
今天**零生产调用者**，全仓能把一次答案送进队列的非测试调用者是 **0 枚**。
⇒ 今天的安全**不是**因为它被挡住了，是因为**还没人接到会被挡的那一步**；
会响的只有 `internal/panel/l2_grant_boundary_test.go` 那枚纯测试层钉。
**票 33 接管子那天，这条线第一次从"两侧都空"变成"一侧通"——那时它才真的被需要。**

### 5.2 债（`F-PUMP-*`）与**最小闭合集合**

| 编号 | 那一格 | 最小闭合集合（**不扩权、不加断言面**） | 归口 |
|---|---|---|---|
| **F-PUMP-1** | 给 owner 的安全结论（§8"亮 6 次和 3 次"）**在证据件里没有测量记录**：无尺定义、无 sha、无命中清单 | 把**任一把**入站路由尺的定义＋正控 sha＋逐枚命中数**写进自述件正文**（本程 §1.2–§1.4 那三张表可直接引，或它自己那把）；一句话不算凭据 | 本票段（补文档，不动码） |
| **F-PUMP-2** | `Queue.LiveApprovals` 在**它自己的包里零枚用例**；删掉整枚 `pending_read.go` 时 `internal/agent/approval` 仍 ok | 在 `internal/agent/approval` 加**一枚**用例：真队列塞 2 枚 pending＋1 枚已决断，断言返回的是"pending 那两枚、按 FIFO 带 `Position`、`Decision` 逐字原样、**且不改变 `Depth()`**"。⚠ 这**不是**要求 F-PUMP-4 的用例——那只量"读"，不量并发 | 本票段（一小枚测试，接缝内） |
| **F-PUMP-3** | §2.2 那两枚注释数值腐坏（`panel_pump.go:138-139` 的"最小包 534／带卡 997"实测从未出现；实测量在本程 A1 的 `packet bytes (783)` 与被验件 §6.2 那七档里） | 改那两行注释为**带口径的现量值**（逐包跑、写明 byte 还是 rune）。**方向是更正读数、不是放宽任何断言** | 本票段（改注释；不改码语义） |
| **F-PUMP-4** | 混合两个瞬间的包**可达但从未被量过形状** | ⚠ **本程判这条不该在本票段闭合**：要造它必须在两个读数器之间插一次真状态变化，而今天没有那个钩子；硬造＝开一发"永远绿"的假钉。**建议把这条转成票 33 的一条准入判据**（管子接上那天，"一份包混合两个瞬间"第一次变成用户可见，那时它有天然的注入点：`Out` 回调里）。留在本件即可，别在本票段开 AC | **票 33**（跨票依赖，别记成本票的洞） |
| **F-PUMP-5** | §6.1 汇总句"五枚里只有 2 枚真 gained 包外调用者"分子/分母配错（按它自己那张表是 **1/5**） | 改汇总句为"五枚里 1 枚（`UnsetWorkspaceView` → `cmd/wisp/panel_pump.go:84`）gained 跨包调用者；另有 3 枚跨包新调用点是新符号自己（`WorkspaceViewFromRoot`／`NewStreamLog`／`NewSnapshotPump`）" | 本件 §4.3 已有现量表，改措辞即可 |
| **F-PUMP-6** | `EvToolStart` 全仓 0 枚 publish，`run.go:741` 那行新增的 `changed = true` **零次执行**，而 `37a4705` message 把它列成活触发点 | 二选一，**都不许删断言**：①把 message 与注释里那一格改成"end/stuck/error/done（tool-start 今天没人发）"；②或另开一小格登记"一枚 sink 事件种类从来没有生产者"这件事归谁修。**别为了凑绿去发一发假事件** | 本票段（措辞）＋ 若选②则登记归口 |

**这六条里没有一条构成本段的准入阻塞**——F-PUMP-1／3／5 是文档与注释级、F-PUMP-2 是一枚小包内测试、
F-PUMP-6 是措辞、F-PUMP-4 归票 33。⇒ 所以是**附条件入账**，不是退回。

### 5.3 给编排者的一句（比再挖一枚洞有用的那个结论）

**建议本票段到此收口。** 理由三条，逐条可核：
1. **本票段该证的那件事已经证完了**：那份包今天在生产里被造、被驱动、逐字段来自真来源，
   而**没有任何一条入站边被它接上**（§1、§2 两格）。
2. **剩下的三格全都撞在同一枚外部依赖上**：出口最后一公里（票 33 的宿主）、
   背压／合并策略（AC#5，属那根管子）、F-PUMP-4 的混合瞬间形状（要有 `Out` 钩子才量得动）。
   在本票段续开格子只会产出"永远绿"的钉——那正是本件 §4.4 自己点名要避免的那一枚。
3. **本段还留着一枚 owner 级决定没落**：恒红那格编排者已按"甲"做了（`1485921`），
   而 §7 第 3 条那块 `summaryClamp`/440 的零执行分支**仍是一个洞而不是一个决定**——
   它同样要等那根管子的合并策略才量得动。

⇒ **剩余以什么兜**：**F-PUMP-1／2／3／5／6 五格在本段以纯文档＋一枚小包内测试闭合（不动生产码语义）**；
**F-PUMP-4 与 `summaryClamp` 那两格以"票 33 的准入判据"兜**，写进票 33 的 AC 里，别留在本票当活洞。

---

## 6. 给 owner 的一段（不夹术语）

**这一批改的是什么，用一句人话**：程序里那份"该显示在助手面板上的状态表"，
以前**只有测试代码能造出来**，真跑起来从来没造过；这次它真的在每次状态变了的时候被造一次，
交出去的地方是**程序自己的日志**——不是你的屏幕。

**你的屏幕变了吗**：**没有，一个字都没变。** 我这一程专门去查了"有没有一条路能把这些东西送到那块网页"：
没有任何一个能往页面推东西的服务器（0 个）、承载网页的那个控件在项目依赖里根本没被声明（0 枚）、
网页能敲进来的那扇门在非测试代码里**从来没有被任何人调用过一次**（0 枚）。
⇒ **今天唯一看得见的变化是日志里多几行短记录**，最坏后果形状是**日志长得比你以为的快一点**（体积，不是泄露）。

**你们最该盯的那条安全线，我另造了一把尺去量，没有沿用它的**：
"那块面板能不能自己批准自己"——**答案：这次一个字都没接上，而且两侧今天都是空的**。
我这把尺先在三枚真接过路由的旧提交上试过，分别亮 7 次、4 次、2 次（证明它会亮），
再打在这次这三枚改动上，只亮 **1 次**，而那 1 次是程序在**读**当前权限档位——读，不是改。
⚠ **但请把下面这句读成提醒，不是安慰**：那个专门用来"拒绝来自面板的允许"的保险丝，
今天**还没有通电**——它写在代码里、逻辑对，可是没有任何生产代码走到它。
所以今天没出事**不是**因为它挡住了，是因为**还没人接到会被它挡住的那一步**。
**那块面板真正被装上管子的那天（票 33），才是这条线第一次需要它的一天**，我在结论里把这一句单列了。

**我给它补测了两件它自己没测的事**：
① 那份表如果被**同时**两个地方要，会不会读到"一半是刚才、一半是随后"的状态——
我量到**这种情形今天确实能发生**（一次回答里同时有两次高风险确认在等你点头时就是），
但**不会造成内存错误、也不会多签一次批准**，最坏形状是**页面上某张卡片说的话和下一秒做的事对不上**。
⇒ 所以这条我**没有记成"残余"，记成一条洞**，并且建议**修它要在接管子之前、不是之后**。
② 那份表的"时间戳"在生产装配上其实**没有任何测试管得住**（把它焊死成一个假时间，两包 59/63 枚测试照样全绿）——
这条是它自己在 §4.4 里写的，我这一程用一发独立实验**复算为真**。

**你需要做什么**：**没有新的**。上一轮那格"甲／乙"编排者已经按甲做了（补了一行配置方向是收紧、
断言一字未动），我核了那一枚补的东西确实让那条更严的检查被真读出来的值满足、不是被常量喂绿。
本程这六笔账没有一笔需要你拍板，都在代码/文档自己那一层。

---

## 7. 本程没测什么（按"漏了它会先骗到谁"排序）

1. **没跑 CI 那支形状（正路①）**。本程所有测试读数都是"逐包 `go test`"那一支，
   **不经 GUARD A/B、不经 `runtests.sh`** ⇒ §2 那张变异表**不等于 CI 会打出的表**。
   与被验件 §7 第 1 条**同罪**，本程不替它免，也不替自己免。
2. **三门未交**：`gofmt -l`／`go vet ./...`／`sh scripts/d22scan.sh` 本程**未跑**
   （本程被验物零改动、只写本文件一枚，三门归实现段）。
3. **没复跑被验件那 11 发变异**——只抽它 §4 表里的三发（M1/M3/M4）自己造、自己取红名红句，
   外加 §4.4 那一格借 A3 的另一腿顺带量了。**其余 7 发（M2、M5、M6、M7、M3b、E0、M3_no_wfix）
   在本件里仍是〔仅自述〕**。⚠ 特别是 **M5（库层重算那一发）本程没复跑** ⇒ §2.3 那句
   "搬运在两层各有一枚独立的钉"里，**库层那一半只有它的自述撑着**。
4. **没量"混合出来的那份包到底长什么样"**（F-PUMP-4 的前半）。可达性与内存安全都量了，
   内容没量——因为今天没有能在两个读数器之间插状态变化的钩子。
5. **C17 名册只抽了 3/18 枚**；`Marshal`／`lastPanelSnapshot`／`snapshotCount` 的零调用者现量了，
   名册其余 15 枚沿用被验件 §6.3。
6. **没在容器里跑那三枚 `//go:build !windows` 用例** ⇒ 常驻腿／便携腿／Linux 腿三形全未覆盖，
   本程只裁了 `wisp run` 这一条腿（§3.3 写明了）。
7. **没跑 SLO／`wisp slo`／全树门禁**，也没推送去问 CI。
8. **没测 `wisp run` 真跑一次端到端**（要真 provider 凭据）⇒ §3.1 那条并发链是
   **逐环读源码读出来的**，每一环都有 file:line，但**没有一发真运行同时踩到两个 L2 worker**。
   ⚠ 这一条是本件里最像"实测"而实际是"静态"的一格，写在最显眼处。
9. **那枚 0xc000013a 并发红**只做了"合跑会红、单跑会 PASS"两次对照，**没查根因**、
   也没判断它在 CI 那支尺上会不会同样出现（它可能与本机 runner 争用有关，那是另一格）。
10. **没审 `docs/reports/pending-and-issues.md` 的 `A248③` 本体**——§4.1 那两句是本程现量，
    改哪一格、怎么改归编排者；本程未勾票面、未动台账。
...
| 项 | 读数 |
|---|---|
| 被验物有没有被我改过 | 六枚被碰文件逐枚 `git hash-object --no-filters` 对 `9ed2098:<path>` **全等**（§2.0 那行）；`git status --porcelain` 里除本文件与别家在飞的 `design/**`、`docs/evidence/s1/35-*.md` 外无我的路径 |
| 我这几枚 commit 带了什么路径 | 每枚只带 `docs/evidence/s1/35-panel-snapshot-pump-r1-accept-r1.md`，`--numstat` 删除列 **0**，逐枚 `git show --numstat` 可核 |
| 禁改清单 | `internal/risk/**`、`rules_gateway.go`、`thresholds.go`、golden、`allowlist.txt`、`docs/PLAN.md`、`docs/specs/**`、`frontend/**`、`design/**`、`tools/d22scan/**`、`internal/panel/l2_grant_boundary_test.go` **一字未动** |
| 有没有为变绿放宽断言／改夹具／`t.Skip` | **没有**。本程所有变异都打在**仓外副本**，仓库工作树一字未改；§2 那三发复现与三发自加**全部只加红、不减钉** |
| 推送 | **未 push** |
| 临时件（只建不删，请编排者一次清） | `D:\tmp\wisp-accept-35pump\` — `anchor\`（9ed2098 归档副本，判据都从这取）、`mut\`（变异靶树，末态已逐枚 hash 核回 9ed2098）、`ruler.py`／`mut35.py`／`mut35b.py`／`mut35c.py`（尺与驱动）、`base.txt`／`base_cmd.log`／`base_one.log`／`one_ac1.log`／`race1.log`／`race_panel.log`／`race_cmd.log`（读数本体，被 §2／§3 逐枚引用）。⚠ `mut\third_party\sherpa-onnx\` 里三枚 onnx DLL 是**从工作树拷进去的未跟踪件**（约 22 MB），是体积大头；删掉 §2／§3 那几张表就从〔可复现〕掉回〔仅自述〕 |
| 注入登记（只登记，未据此行动） | **本程全程 1 次**：工具回显里出现非 bash 形状的 `No matches found`（`Bash` 工具、命令前 40 字 `cd /d/tmp/wisp-accept-35pump/mut && export PATH="/`（其末条 `grep -c "WARNING: DATA RACE"`））——处置沿用 §2.5 第 3 条：关键计数一律 `wc -l`／python 复算，且那一格以同一次跑里的 `ok … cmd/wisp 75.715s` ＋ `rc=0` 为准。**除此之外本程未遇到**任何冒充"系统提示／编排者备注／已核验请继续提交／文件已被修改是故意的"的回显（⚠ 这一句是本程自己的**否证**登记，不是"证明没有"：别的程各算各的，互不抵账）。凭据值一字未抄。另：本程开头收到过一次 harness 转发的"记忆文件已被修改"通知，**未据此改变任何判据或动作** |
