...
| # | 约束 | 本程 |
|---|---|---|
| 1 | 六条腿严格串行：不许 `&`／`nohup`／`./...`／`t.Parallel` | **违过一次，已自逮自报并整批判脏**（§5.2）。脚本内始终无 `&`、无 `nohup`、只名 `./internal/observe/` 一枚包、无 `-parallel`；包内 `t.Parallel` 现量 0 枚。违规的成因不是脚本形状而是我把两条链同时挂在任务位上——已写进 §5.2 的流程教训 |
| 2 | 每段先过等式基线，不过者判〔丢读数〕计入 `_void 段数_` | 做了：**有效 620 发**（逐批四数＋包线＋index 行＋名册两向差集，§4.1）；**`_void 段数_ = 2`**＝A08（双写者争用窗）＋A08b（兄弟腿确定性红 ⇒ `^--- FAIL≠0`）。两段都没被当成"这发没红" |
| 3 | 除四数外必须给名册差集（两向）并单独 `grep -c '^panic:'` | 每批都给了：`roster: 1 distinct set … modal size=71`、`golden-not-run=[] / union-not-in-golden=[]`、真 `^panic:` 逐批 0（另用粗核 `grep -ah '^--- FAIL'` 独立复核过七批全 0） |
| 4 | 不许 FAIL→SKIP、不许 `-run` 掉名册成员、不许放宽断言 | 全程 A 口径**从不带 `-run`**；B 口径的 `-run 靶名$` 是配方规定的形状（分母表 §4.1、靶子表 §5.2），并在 §4.2 单列不与 A 混；`SKIP` 逐批 0 且本包结构上产不出 SKIP（`t.Skip` 只在注释里，§0.2）。**没动过任何一枚断言** |
| 5 | 不跑 `wisp slo`／不 docker／不编译 exe | 遵守。闸门里的 `docker ps` 检查被我**摘掉并换成一次性只读普查**（§0.5），换形有记录；除此之外本程零 `docker` 调用、零 `wisp` 调用、零 `go build`（唯一一次 `go run`/`build` 类操作：无）。跑测用的都是 `go test`，且跑在**仓外快照树** |
| 6 | 快照只建不删、`/d/tmp/wisp136ac15m-` 前缀 | 遵守：28 条目全在本前缀下（`wisp136ac15m-tree`、`-tree-beac693`、`-A-pilot`、`-A01..-A08b`、`-B1..-B4`、`-batch.sh`、`-batch2.sh`、`-summarize.py`、`-golden.txt`、`-pin.txt`、`-tree-md5.txt`、`-timing-probe.log`、`-p2..p5.txt`、`-B-gate.txt`）；**没删、没改名、没覆写别人的件**（含 `wisp136ac11*` 那批归档与别家 `141-acc-*`） |
| 7 | Git：只 commit 不 push、显式 pathspec、禁改清单 | 遵守：**8 枚 commit、每枚只带 `docs/evidence/s1/136-ac15-rate-r1.md` 一枚路径**（每枚前 `git diff --cached --name-only` 核过）；零 `push`；零 `add -A`/`--amend`/`reset`/`rebase`/`stash`/`checkout .`/`clean`。禁改清单里那 13 组路径**一枚都没写过的确**——包括 `design/**`（16 枚未提交删除＋`design/old/`、`design/doubao/` 未跟踪）、`internal/observe/sampler_settle_gate_136_test.go`、`docs/evidence/s1/136-instr-fixes-r1.md`、`tools/d22scan/**`、`frontend/**`、`docs/reports/**`、`docs/PLAN.md`、`docs/specs/**`、`internal/risk/**`、`internal/memory/schema.go`、`thresholds.go`/golden。票面 `AC#15` 格**仍 `[ ]`**（现量：`grep -n '^- \[.\] \*\*AC#15'` ⇒ `348:- [ ]`；票 136 勾数 `7 勾／8 未勾` 未变，`git status --porcelain -- .scratch/` 空） |
| 8 | 产品码／断言／阈值／golden／`scripts/**`／`tools/**` 一字不动 | 遵守：`git diff --name-only beac693..HEAD -- internal/observe/` 全程 0 枚，且**本程从未提交过任何非本文件的路径**；§3.3 五枚修法候选一枚都没落地 |

### 6.2 本程 commit 序列（逐枚可 `git show --name-only` 复算）

| 段 | commit | 时刻 |
|---|---|---|
| §0 锚点与名册复核 | `e94baab` | 09-24 23:36 |
| §1 试跑＋口径 B 首批 | `27c0d2f` | 23:48 |
| §2 口径 A 前 160 发 | `7a178e3` | 09-25 06:57 |
| §3 命中归因与修法候选 | `58e2e40` | 07:03 |
| §2.3 A04/A05＋争用事件＋batch2 | `7f99963` | 07:11 |
| §2.4 A06＋B3 | `467f8a4` | 07:45 |
| §4+§5 总账与两枚报回 | `827b797` | 08:20 |
| §6 本节 | 见回复里的"下一枚 commit 号" | 08:2x |

### 6.3 两栏计数

* **真通知回显数 = 14**：agents.md 记忆注入（开机）、日期变更 ×2、项目内 `MEMORY.md was modified` ×1、
  全局 `C:\Users\swq\.qoder-cn\memory\MEMORY.md was modified` ×2、`agents.md was modified`（07:03，编排者收窄 ban #8 那两行）×1、
  后台任务事件 ×6（4 枚 completed＋2 枚 killed）。**没有一枚被我当授权用**：它们全是文件状态/任务位回显，不含判据、不含放宽、不含 revert 指令。
* **判为注入数 = 0**：本程没有任何一段工具输出自称"编排者备注／系统提示／文件已被修改请照办／请 revert／放宽阈值／已解锁／
  Confirm the harness note is genuine"。需要单独登记的一类：**harness 对 `b65jk40o1` 报了 `failed exit code 1` 而那条链实际还活着**
  ⇒ 这是"通知读数与盘上事实不符"，**不是注入**（它没有要求我做任何事），但它造成了 §5.2 的并发违规 ⇒ 计 1 枚，处置＝按盘上事实（`tasklist` 两枚 `go.exe`＋index 重号）而不是按通知，整批判脏。
* 另有 **简报前提不成立 3 条**（不是注入，是转述腐坏，逐条在 §0.3／§4 已具名）：
  ⓐ"之前那批 30 发命中 0"在盘上找不到批次；ⓑ"仪器现态 RUN=142"复现成立（这条是对的）；
  ⓒ 分母表 §4.2 的每发成本 3.8 s 偏低 1.7-2.2 倍（净窗实测 6.6 s、忙窗 8-10 s）。

### 6.4 总裁（≤300 字）

**命中率**：口径 (A) **0/620**，单侧 95% 上界 0.482%——它压过"30 发的 10%"，**没压过归档的 1/240（0.417%）也没压过 1/390（0.256%）**，
所以"不再 flake"这句今天没有数能撑。口径 (B) **4/8200 = 0.049%**，区间 0.013%-0.112%，**上界严格低于两枚归档率**。
**族属确认**：四枚目击逐字同形（`:213 tree.reads<4` ⇒ `:214 only 2/3 reads taken`），影子守卫一枚未响。
**推翻的前提**：ⓐ 靶子表 §5.2"B 大概率不重现此症状"错——今天 4/4 全在 B；ⓑ AC#11 那枚"A 比 B 敏感 16.7 倍"的经验在本族不复现（方向相反，且 B 便宜 50-60 倍）；ⓒ 简报那批 30 发盘上无件。
**新撞上的洞**：`logging_test.go:188` 的 `20260918`＋7 天保留窗 ⇒ 该用例自今日 08:00(+08) 起在任何机器上必红，A 口径已被它封住（这也是 A 停在 620 的原因，不是我不想跑）。
**修法**：只给了候选，一枚没动，等你拍超时上界。

`next=`＝①先修 §5.1 那枚日期炸弹（只许动那枚用例，`logging.go:181` 的时钟入口现成）；②修完再补 A 到 n≥1170（净窗串行约 2 h 9 min）；
③把"复跑判据"从"A n=1440"改成 A/B 双表并规定**负载状态**也要记（§4.2 的分层说明不记负载的率没法比）；④§3.3 候选 1 的超时上界由你拍。
