...
## §7 追加更正（本程 14:4x 现量，`date` 为证）——上面有几处被我写错了，逐条钉住

1. **本格有一枚实现程是"迟到交件"，它其实提交了**：`worker-ticket136-ac11-r1` 在 **14:34** 交回三枚真 commit——
   **`f06a8d0`**（码，14:14:06）＋ **`c2a3bd6`**（证据表 `docs/evidence/s1/136-ac11-impl.md`，**365 行，现在在盘上**）＋ **`0050300`**（§9）。
   ⇒ 本文件开头"**两枚都没写证据文件**"那句**只对第二枚成立**，对这一枚不成立；
   它交件里报的号（`d667888`）确实是错的号，**但活是真的**（**同一个形状今天第三次**：号写飞 ≠ 没干活）。
2. **`-batch-after/` 我数成"12 份"是数在批次中途**：我那次 `ls | wc -l` 在 **14:19**，它那批 30 份到 14:3x 才写完 ⇒ **现量 30 份、命中 0**。
   ⇒ 我写进 §4 的"它自述 56、盘上只有 12 ⇒ 与盘上不符"这条**作废**（它自己的终版交件也已把 56 更正为 30）。
   **新教训：数"几份/几枚"必须带取数时刻**——同一个数在 14:19 与 14:34 可以差 2.5 倍，**别拿一次 `wc` 去指别人虚报**。
3. **产物归属反了**：`-batch-before/`（30）、`-batch-after/`（30）、`-probe-whichkey.log`（500 次重放）、`-tree-probe`、`-tree1` 都是 **`worker-ticket136-ac11-r1`** 的；
   我原文写成"前一程"把两枚程混成了一枚。同格另有并行程前缀 `wisp136ac11b-*`（其 `tree-after` 与 `tree0` 都在盘上）。
4. **我自己的两批我用 md5 复验过，结论不动**：`/d/tmp/ac11-orch-tree` 里那枚测试件 md5 ＝ **`14afa08…`＝未修版（`51e29b0`）**；
   `/d/tmp/ac11-orch-after` ＝ **`336a50f…`＝修后版（`f06a8d0`）** ⇒ **改前 2/60、改后 0/60 这两枚数是有效的**（每发都 `RUN=65`／顶层 `PASS=65`／`SKIP=0`／`panic=0`）。
5. **合起来的数（口径要分开写，别加总成一个百分比）**：
   改前——本程 **2/60**；`r1` 的 B1 **2/30**、B3 **2/30**（**整包单发口径**）；探针重放 **33/500**（**同窗口重放口径**，不是同一件事）；
   `r1` 另报**同进程 `-count=500` 只有 2/500**（≈0.4%）⇒ **命中率随口径变**，任何"这格 X%"的写法都必须带口径，否则就是下一位误用的源头。
   改后——本程 **0/60**、`r1` 的 B2 **0/30**。
6. **本格的裁决表正在由非实现者程 `acceptor-ticket136-ac11-r1` 出**（它 14:34 已落第一枚 `2546b76`）⇒ **本文件只是补账，不是裁决**；勾等那张表。
