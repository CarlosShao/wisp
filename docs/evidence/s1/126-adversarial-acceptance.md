...
- 快照清单：
  ...
  | `/tmp/ac126-sq/mut/<发名>` | 每发变异一份 `post` 的副本（9 份） | 变异矩阵 |
  | `/tmp/ac126-sq/skipmut` | `post` 副本，枚举改判"每枚字母都拒绝" | 模拟单卷 runner |
  | `/tmp/ac126-sq/probe`、`/d/tmp/ac126-sq/probe2` | `post` 副本 + 我自己写的探针 `_test.go` | 量 `R-126-2/3` 与同类下一层 |
  全部在仓库外；两棵 `/tmp` 树的分歧见 8.1 的 R-ac126-2。
- 四数一律从 `-v` 输出量，口径与 `scripts/winsec-tests.sh` 逐字相同：
  `grep -c '^=== RUN'` / `'^--- PASS'` / `'^--- FAIL'` / `'^--- SKIP'`（顶层锚定，子用例缩进不计）。
- 变异先发落地再读名：`diff` 出被改的那几字节（证明落地）→ `go build ./internal/winsec/` rc=0 → 才读 `--- FAIL` 名单。
...
**非测试代码里对这两枚函数的取反调用数为 0**。
（取反只出现在新用例自己的 5 枚 CONTROL 腿里——那是断言，不是生产读数方向，别把两者混成一枚读数。）
...
**这一小节里有一枚我自己没取到的东西，如实标出来**：那发 run 的 `headSha` 是否就是 `09b5285`，
**我只拿到 job/step/日志三样，没拿到 run 级 `headSha`**（我第一次 `gh run view --json` 只选了 `jobs`
字段；回头补取时 `api.github.com` 连撞两次 TLS handshake timeout）。
⇒ 可比性这枚结论**不依赖 sha 标注**：它是"那发日志里的四数＝86/46/0/0 与我本机 `09b5285` 快照的四数
逐字相同"这一枚**内容对内容**的比对；"那发就是 `09b5285`"这半句我按自述记、**不背书**。
