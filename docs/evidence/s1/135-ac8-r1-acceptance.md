...
| C2 要求的每一味 | 本表哪一节 | 该节现文本所在 commit（短哈希，`git log -L` 逐节查） |
|---|---|---|
| 种件落地行（`grep -n` 原文） | §4.1（旧尺那侧） | `6391225` |
| 种件落地行（含 `installLogSink-hits=0` 那一味） | §6.1（新尺那侧） | `c59150a` |
| `go build` rc（`BUILD-cmdwisp` ＋ `BUILD-all` 双 rc） | §4.1 与 §12.1（逐发在 `<label>.verdicts`） | `6391225`／`497f0bc` |
| 红名（逐字句子） | §5 ＋ §5.1 两支红句并置（§5.1 里 `:208`→`:209` 那处更正随 §5.2 一起进） | `6391225`（更正＝`c59150a`） |
| 红的归属（每发只出自 `:229`） | §5.2 | `497f0bc` |
| **两拍都齐** | §6.2（第二拍，单跑＋整包两粒度）＋ §6.3（第一拍独立红、不抵账） | `c59150a` |
| 零退化（旧/新两把尺整包名册双向差集为空） | §7.2 | `497f0bc` |
| 落在 `docs/evidence/s1/135-*.md` 具体一节 | 本文件 §4／§5／§6／§7.2 | 同上 |

⇒ **C2 满足**，出表人是**非实现者**（本程，裁决方）——这正是编排者在 `4113cd7` 里说"本格不翻勾、等那张表"要的那一味。**本表一格都没引用被审对象的读数当依据**（§3 那句名册比对只证"同树同名字"，见那里的 ⚠ 注）。本程全部 6＋1 枚 commit 只带 `docs/evidence/s1/135-ac8-r1-acceptance.md` 这一枚路径（逐枚 `git show --name-only` 可核），**未 push**。本节这张表与 §13 自身的最后一枚更正 commit＝`git log --oneline -1` 现取。
