...
⚠ **一处编号更正（append-only，不改写已提交的 commit message）**：`52551db`（§3 那枚）与 `0e7489a`（§4 那枚）的 commit message 里写的 `R-136-8`／`R-136-9`／`R-136-10` 是我**还没查完号占用之前**的编号。我随后核出 `R-136-1..R-136-6` 已被上一程验收方用掉（`136-ac1-adversarial-acceptance.md:333-338`）、`R-136-7` 空着，于是把本程新账**整体上提一格**。⇒ **以本文件 §7 那张表为准**：`R-136-7`＝settle 侧部分未测无痕（中）、`R-136-8`＝`samples:null` 让腿 A 一支防呆永不响（低）、`R-136-9`＝`fakeTree` seam 的 `len(mu)-1`（低）。**`R-136-10` 不存在**，编排者翻票面/台账时请按 §7 的号抄。
