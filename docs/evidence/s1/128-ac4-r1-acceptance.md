...
| §10.5＋§10.1 台账刷新 | `2ebe5fc` | 交回前复跑那发（G7）与临时件收尾数 |
| 结构修复 | `6ef73df` | **修的是我自己造成的两件事**：(a) 逐节 Edit 的锚点选在已挪过位置的文本上 -> §6 一度落到 §10 之后（现已按 0-10 顺序排回，内容零增删）；(b) 一枚 Edit 的 `new_string` 被我发到一半就截断 -> §10.3 那张账表被换成残句（现已补回并顺手写全） |
...
**10.6 收尾回显（把"最后一行永远靠下一枚"这句闭合掉）**

```
$ git log --oneline -- docs/evidence/s1/128-ac4-r1-acceptance.md | wc -l
12

$ git show --name-only --format="" HEAD        # 收尾时刻现量
docs/evidence/s1/128-ac4-r1-acceptance.md      # 只有这一枚路径

$ git status --porcelain | wc -l
18    # 全是 owner 自己的 design/**（16 枚删除 + 2 枚未跟踪目录），本程一枚未碰
```

最后三枚（`2ebe5fc` §10.5＋§10.1 台账、`6ef73df` 结构修复、`9fcb2ed` §7.1 口径更正）逐枚 `git show --name-only --format=""` 都只列本文件；
**这一小节自己那一枚仍然列不进来** —— 但把口径写死就不再需要下一位补：上面那次 `wc -l = 12` 取在**本小节落盘之前**，
记 §10.6 与这张账的那一枚就是第 **13** 枚 ⇒ **`13` 是本表的终态**，此后每多一枚都是追件（用 `git log --oneline -- 本文件 \| head -1` 看是谁追的）。
