...
（空）  rc=0                              <- 改前那版也干净：与实现件 §6.5 **不矛盾**（它需要 -w 的那枚是本票新增的文件，
                                            在改前那版里根本不存在）⇒ 这一发只支持"当前两版都干净"，不支持任何反诘
...
本程另外新增一枚读数：**改前那版树 gofumpt 也空**（两版都干净；这一发**不构成对实现件的反诘**——它 `-w` 过的那枚文件是本票新增的，改前那版没有它，详见 §7.3 那行的更正）。
以及 §7.5 那次本程自我污染。
...
| 13 | **本程自己在第 7 格 §7.3 第一版写的那句**："改前那版 gofumpt 也干净，所以'本票改完才需要 gofumpt -w'那句**不成立**" | 本程重读实现件 §6.5 全文：它写的是"第一遍量到 `cmd\wisp\slo_report_144_windows_test.go` 一枚，`gofumpt -w` **只对那一枚本票文件**跑过" ⇒ 那枚文件是**本票新增的**，改前那版里根本不存在，所以"改前也空"是它那句话的**必然推论**，不是反证 | **推翻本程自己一处推断**（把一枚中性读数当成了反诘）。更正已就地写进 §7.3 与 §7 判定行；本程**没有**据此说实现件任何一句不实 |
