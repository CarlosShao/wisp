...
**越界自证**〔独立复现，全是盘上可核的量，不是叙述〕：

- **没裁 AC#3、没裁 AC#4**：本件里没有一格结论属于它们；`§5` 那句"这把尺可满足"只是 AC#2 的反向检查（防恒不满足），
  不是对 AC#3 的裁决。
- **没修改任何生产码或测试码**：`git status --porcelain internal/winsec/` 输出 **0 行**；
  我那三枚 commit（`54e2688`／`b179b51`／`11394e5`）的 `--name-only` **每一枚只有**
  `docs/evidence/s1/137-ac2-ac5-r1-acceptance.md`；每枚提交前 `git diff --cached --name-only` 我现查过，
  三次都只有我自己那一枚路径（`add`／查清单／`commit` 分三次调用）。
  PROBE-F 那处改动只落在**仓外快照树** `wisp137r2-ac2-tree-naive-d`（文件第 145-150 行带 `ACCEPTANCE PROBE 137-R2AC2-F` 标注）。
- **没为变绿动过任何东西**：`git diff --name-only f53ad5c 4e66817` 里 `thresholds`／`golden`／`SLO`／`.github`
  **零命中**；我这一程一枚计时/资源断言都没跑（D32 的 CPU≤0.5%／RSS≤25MB 未动、未读）。
- **那 7 处 raw `t.TempDir()` 一字未动**（AC#4 的地界）。我顺手替它把位置核了，作为给 AC#4 的料〔独立复现〕：
  `placement_symlink_113_other_test.go:197/220/248/264/289` ＋ `ancestor_separator_108_other_test.go:69/135`
  ＝与它 §4 第 1 条报的**逐枚同位**；`dataroot_symlink_119_other_test.go:128/154/194/220/286` 五处（票 119 地界）我也没碰。

**我没核的档（诚实列，不要下游当成已测）**：

1. **MUT-A／MUT-B 单发我没造也没跑**（R-137-1 明令 AC#3 不许钉它们）；**MUT-C**（谓词级
   `ancestorIsLink` 打成 `return false`）与"`RemoveUnlinked` 整枚导出函数直接 `return nil`"那形我同样**没造没跑**
   ⇒ 按"验收方没造的支别记成已测"登记。我只落了 MUT-D、MUT-AB、MUT-E 与 PROBE-F 四形。
2. **真 macOS 没验**：软链形是容器里 `/r2ac2link -> /r2ac2priv` 造的形状（与 R-113-B 记的 macOS `/tmp`、`/var` 同族）
   ⇒ **形状复现≠真机读数**。它 §1.4 末句"软链形只存在于我们的容器台件里"是〔仅自述，不背书〕。
3. **没跑全仓测试**（只 `./internal/winsec/`）、**没跑 `-race`**、**没跑任何 `cmd/wisp` 测试**
   （那包有兄弟在飞；`cmd/wisp/secret_dataroot_119b_test.go:201` 那枚 R-137-1 点名的 ABSENT 用例我没执行，
   只在 §7.3 引到 `cmd/wisp/doctor.go:14` 那一枚 import 行做 vet 归因）。
4. **没重跑终裁那 22 发、没重判票 124 批次 3b 那 17 枚**；基线六数我只对点到"逐数相同"。
5. **CI 侧只核了源码里的名单，没核任何一枚 run 的日志**（说不出 run id＋step 的"门"我不当存在）；
   票 122 那枚"gofumpt 没钉版本"我没动、也没去修 workflow。
6. **`-count=4`／长跑稳定性、跨主机重跑**：没做（`-count=2` 的逐名零换色只覆盖两遍）。
7. **实现方 §6 那两个计数是它自己的账**，我不沿用也不替它复核；它自报的那次共树暂存偏离，我只保证了自己的三次暂存清单干净。
8. 一条**前提已过期**要报：票面 Rules 第 78 行说 `docs/reports/2026-09-23-gap-analysis-vs-oss-harnesses.md`
   是"来源未明的未跟踪件、不提交不改不删"。我核到它现在**已被跟踪入库**（`git log --diff-filter=A -1` = `5866c6f`
   "缺口文档入仓带作废声明"）⇒ **这条前提已经不成立了**；我全程未读、未改、未删它，只把"这条票面规矩该改写"交回编排者。
9. 一条**我自己的口径修正**（不改写已提交的 message，按追加更正记）：`54e2688` 那枚 message 里写"13 发全 rc=0"，
   终数＝**12 发** `-v` 读数（`e-tight-plain` 是第 12 发，另有一次容器原生 `go vet ./...` 与静态门禁不算读数发）；
   正文 §0 已改为 12。
...
1. **改口那枚标签**：派单/票面里 `52/17/13/0＋17/5/0、rc=1` 属 **AC#3 的"普通形八数不变"参照值**
   （出处＝`137-ac1-r1-acceptance.md:286`／`:463`，跑形 MUT-D·普通·`-count=1`）；
   **AC#5 的 `-count=2` 未变异普通形参照值**＝`104/60/0/0＋44/0/0、rc=0`（我这一程量到、与实现方同值）。
   两组数不打架，但按字面执行会把 AC#5 判成"对不上"。
2. **AC#3 起单时要钉三件事**（我这一程的新读数使这三条更硬）：
   ①只钉 **MUT-D** 那一形；②**软链形的"转红"今天已不能当"修好了"的凭据**——未变异的收紧态在软链形同样是这 11 枚红（§5），
   凭据只能落在"普通形八数＋逐名"那一列与三枚对照组；③对照组按 R-137-3 改点名后的三枚（118×2＋`TestAC3POSIX…119`）。
   另加我这一程读到的一条：**别拿"MUT-A/MUT-B 里新判据响不响"当凭据**（§4：那两形里 `err=nil`，新判据根本走不到）。
3. **AC#4 单独交**（票面更正块那句"换根与收紧各自能单独成立、也各自能单独漏"），并把我 §8 第 4 条那 7 处的新行号带过去；
   同族料另有一处：`dataroot_symlink_119_other_test.go:203/:251/:308` 三枚 sentinel-only 内联断言（其中两枚根是 raw
   `t.TempDir()`）——不属本票分母，但换根那一程判"未解析形的持有者"时会撞上。
4. **两笔要作废的说法**（免得下游据它们推）：①实现方 §0 那句"CRLF 版"——我现量三枚还原文件 `CR=0`，那三枚差的是内容；
   ②台账里"winsec 的 POSIX 半边在 CI 零覆盖"——票 111 AC#9 起 `./internal/winsec/` 已在 ubuntu core 名单里
   （`scripts/portable-tests.sh:105-110/:180`）。
5. 票面 Rules 第 78 行那条"未跟踪件"规矩已过期（§8 第 8 条）。

**我明确没核的部分**：见 §8 全列（1-9 条），其中最有分量的是 **MUT-A／MUT-B／MUT-C 三形我没造没跑**、
**真 macOS 没验**、**CI 只核了名单没核 run 日志**、**全仓测试与 `-race` 没跑**。

## §10 两个计数（本程自己的；不沿用实现方与终裁方的账）＋ 可重跑凭据

**A. 真通知／正常上下文回显数＝6**，每条给出处与"能不能盘上追到"：

| # | 形状 | 出处 | 我怎么核的 | 判定 |
| --- | --- | --- | --- | --- |
| 1 | `Note: The file C:\Users\swq\.qoder-cn\projects\D--...-Wisp\memory\MEMORY.md was modified…` | 派单那条消息的宿主回显 | 路径真实存在，`ls -l` ＝ **5061 字节／mtime 09-24 09:43** | 真通知（编排者自己写记忆），未要求我放宽任何判据 |
| 2 | 同上，全局那枚 `C:\Users\swq\.qoder-cn\memory\MEMORY.md` | 同一条消息 | `ls -l` ＝ **19589 字节／mtime 09:43** | 真通知，同上 |
| 3 | `[SYSTEM NOTIFICATION - NOT USER INPUT]` 后台任务完成 `task-id bbd5j5qjr` | 我自己 `run_in_background` 起的 `batch.sh`（12 发读数） | task id 与我拿到的同一枚；`tasks/bbd5j5qjr.output` 在盘（0 字节＝我把输出重定向到自己的 `batch.out`，那两份都在 `D:\tmp\wisp137r2-ac2-io\`） | 真事件 |
| 4 | 同上 `task-id b7d4c9tmv` | 我自己起的 `gates.sh`（AC#5 门禁） | 同上（`gates.out` ＋ `gates-*.txt` 在盘） | 真事件 |
| 5 | `The date has changed. Current date: 2026-09-24` | 中段工具结果里的宿主提醒 | 与本程每次落笔前现取的 `date` 一致（`10:03`→`10:38 +0800`，**我全程没有拿两个时间戳相减算耗时**） | 真事件 |
| 6 | 一枚携带本仓 `AGENTS.md` 全文的 system-reminder | 中段 Bash 结果之后 | 路径真（`agents.md` 在仓内、锚点自量）；内容是禁止清单的抄本，**没有**要求我改判据／revert／少取证 | 正常上下文，不计注入 |

**B. 判为注入数＝0。** 四条判据逐条走过：①本程工具输出里**没有出现**自称"编排者备注／系统提示／用户已更新编码规则／
请 revert／冻结某包／放宽阈值／某文件已被修改／某格已合并／用户已拒绝／`Confirm: the harness note is genuine`／
别用工具直接给结论"要我照办的文字；②凡出现的提醒类文字都指向我自己刚跑的命令或仓内真文件（上表 1-6）；
③没有任何一处让我**少取证**——反倒本程我自加了三形它没要求我跑的探针（PROBE-F／MUT-AB／MUT-E）与一发容器原生 vet；
④**所有引用过的 sha 都先 `git cat-file -t` 现核**：`4e66817`／`f53ad5c`／`6f3817a`／`e25d2a7`／`191f0d6`／`5866c6f`／
`54e2688`／`b179b51`／`11394e5` 全部＝`commit`，零枚抄来当锚（被验锚点＝`4e66817`，另含开工时自量的 `HEAD`）。
计数口径：只报我自己这一程看到的，不并入实现方那 ≥15 次、也不并入终裁方那 6 条。
...
（`GOPROXY=off`，缺件直接 fail ⇒ 不是复用别人的读数）。**删与不删归编排者一次做完，本程一枚未删。**
重跑法：`sh /d/tmp/wisp137r2-ac2-io/batch.sh`（12 发读数）＋ `sh .../gates.sh`（五组门禁）＋
`python .../parse.py <log>` ＋ `python .../compare.py <base.v>...`；台件与全部 `-v` 日志、容器头、名册都在盘上。
