...
| §6＋§7 | `2e7d8b4` | 没查的档 + 纪律自证 + 交付净数 |
| §7.6 | `e0f4c07` | 仪器口径不一致那一处报回 + 提交纪律复量（**本表这行是在它之前写的，故"共 7 枚"这句要靠第 7 枚本身才成立**） |

⇒ **全文件共 7 枚 commit**（截至 `e0f4c07`），每枚 `git show --name-only` 都只含本文件一枚路径。
