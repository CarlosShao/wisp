...
> **`>` 更正（2026-09-25 11:3x，编排者现跑 `git blame`，原句不抹＝本行上方那句的归属写错了）**：
> `scan_test.go:1179` 与 `:1194` 那两句的 blame 都是 **`ca84b75dccab…`**，不是 `3bb99aa`。
> ⇒ 这两句与那枚"第三枚半"新测试**出自同一批**（`ca84b75` ＝ index-aware 那一修），所以"上一批写的、不在三件之内"这句**给它们开的豁免不存在**：
> 它们是本批自己地界里的"the one shape …"式措辞，与验收在 §5.3 判过头的那两句同源。
> **处置**：本件不动它们（改了会混进下一格归因），改由后续一程统一处理，出处记在此处。
> ⇒ 归因错的原因很朴素：我按"这两句读起来像旧的"下了结论，而 `git blame` 一条命令就能否掉。**今后凡"这句是上一批写的"这类归属断言，必须带 blame 读数**。
