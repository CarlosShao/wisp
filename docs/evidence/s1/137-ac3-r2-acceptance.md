...
**本节（§2.3–§2.4）落在哪枚 commit——`git log --oneline -1` ＋ `git show --name-only HEAD` 原样：**

```
$ git log --oneline -1
0d6aeda docs(137,AC#3 证据 §2.3-§2.4): 结案格②普通形八数逐数相同＋三档名册两两双向差集全空；结案格③对照组三枚两形都红（未变异两形都绿＝变异归因）＋判别对 s02b/s04/s05
$ git show --name-only HEAD | tail -2

docs/evidence/s1/137-ac3-r2-acceptance.md
```

### 2.5 结案格 ④：panic 账＝0，且**取的是全量读数**

逐份 `-v` 日志现量（`grep -cE '^(panic|fatal error)'` ＋ 三条相互印证的行数账）：

| 发 | `panic＋fatal error` 行数 | 日志行数 | `--- ` 颜色行数 | `=== RUN` 行数 | 颜色行＝RUN？ |
|---|---|---|---|---|---|
| `s01b-dloose-plain` | 0 | 225 | 52 | 52 | 是 |
| `s02b-dloose-link` | 0 | 177 | 45 | 45 | 是 |
| `s03-dtight-plain` | 0 | 225 | 52 | 52 | 是 |
| `s04-dtight-link` | 0 | 187 | 45 | 45 | 是 |
| `s05-relax-link` | 0 | 177 | 45 | 45 | 是 |
| `s06-relax-plain` | 0 | 225 | 52 | 52 | 是 |
| `s07-tight-link` | 0 | 178 | 45 | 45 | 是 |
| `s08-tight-plain` | 0 | 191 | 52 | 52 | 是 |
| `s09b-loose-plain` | 0 | 191 | 52 | 52 | 是 |
| `s10b-loose-link` | 0 | 167 | 45 | 45 | 是 |

⇒ **十发 panic 账合计 0**。⚠ 但"panic＝0"本身不是全量的证明——本仓付过钱的坑是**一枚 panic 会吞掉同包其余几十条读数**，
所以我把"全量"钉成可复算的等式：**每一发的 `--- PASS/FAIL/SKIP` 颜色行数 ＝ 该发 `=== RUN` 行数**（52＝52、45＝45，十发全等）
⇒ 没有任何用例"跑了却没留下颜色"，也没有用例从名册里消失。
再叠两条口径：四数**只从 `-v` 量**（十发全 `-count=1`，`-count=2` 会把四数翻倍这条我按纪律避开，未跑）；
两形分母本来就不同（普通 52／软链 45），那 **7 枚差**我用名册差集**逐名**归因了——全部是票 125 三枚自拒探针的子测试
（`…SeamProbeShapesAreBuiltOnAResolvedRoot125/{control_plain_temp,measured_symlink_spelled_temp}`、
`…SeamGuardStillRefusesEveryHostileShape125/{同那两枚}`、
`…SeamAcceptsTheHonestPOSIXAnswer125/{control_plain_temp,control_unresolved_root_still_refused_by_the_floor_itself,measured_symlink_spelled_temp}`），
**分母那 11 枚一枚不在差集里** ⇒ 软链形没有靠"缩小分母"制造任何绿。

### 2.6 结案格 ⑤：恒真自查——正面作答

问：**有没有哪一发在"未修"的码上就已经响？** 答：**有两发，我逐名量到了，并且两发都没算进修复凭据。**

1. **`s07-tight-link`（收紧态＋生产码未变异，软链形）＝那 11 枚已经全红**
   （名册 `FAIL_n=11`，逐名与 §2.2 那 11 行**完全相同**，红行同样落在 `113:204/237/254/271/296` ＋ `108:90/156`）。
   机制读得到：软链形里容器自己那枚 `/r2ac3link` 站在路径的**第 2 个组件**上，走查在第 2 步就把整条拼写拒掉
   ⇒ 新判据"这枚链接是不是我自己种的"**修之前就在响**。
   ⇒ 这一发**只**用来验票面三条钉之二，**不**进 AC#3 的成立凭据。本格①那句"软链形 11 枚转红"之所以还立得住，
   唯一依据是 §2.4 的**判别对**（同一发 MUT-D 下未收紧侧 11 全绿），不是"它红了"这件事本身。
2. **`s01b-dloose-plain`（AC#2 之前那整棵树＋MUT-D，普通形）＝18 枚红，分母那 11 枚已经在响**
   ⇒ 普通形那一列**对"修没修"没有判别力**；它的作用是**非回归闸**（②"零新增红"），我按这个身份用它。

⇒ 反过来，我列进 AC#3 成立凭据的只有**在"未修"码上确实不响**的那一枚形状：**MUT-D ＋ 软链形**
（`s02b`/`s05` 的 11 全绿 → `s04` 的 11 全红）。这一枚未借任何别人读数（§2.0 十发全是自己跑的）。
⚠ 另按 `R-137-1`：票面原句要复跑的 MUT-A／MUT-B／MUT-AB 我**一枚都没造**（未修码上已全响＝恒真判据；
单发 A 或 B 又会让 108 或 113 那几枚永远不响＝恒不满足的坏尺），也**没有拿"单发 A／B 里新判据响不响"当凭据**（三条钉之三的另禁条）。

## §3 三条钉逐条复核（票面 `:198-204`）——**没有一条被我的实测推翻**

| 钉 | 票面原句要它成立 | 我量到的 | 判定 |
|---|---|---|---|
| 一，只钉 MUT-D | 别复活"复跑 AC#1 同一变异"那枚恒真判据 | 十发全部用 `MUTATION-137-AC3R2-D`（同一对锚点、两枚生产文件各 1 处），A/B/AB/C 一枚未造 | **照做，成立** |
| 二，软链形"转红"今天不能当凭据 | 未变异的收紧态在软链形里也是那 11 枚红 | `s07-tight-link` `FAIL_n=11`、逐名＝§2.2 那 11 枚、红行同一批 `file:line` | **独立复现，成立**（故本格凭据落在 §2.4 判别对上） |
| 三，对照组按 `R-137-3` 点名 | 三枚＝118×2 ＋ `TestAC3POSIXLinkInsideAResolvedDataRootStillRefused119` | 这三枚在 MUT-D 六发里两形全红、未变异两发两形全绿（§2.4 表） | **成立**；另禁条（不许拿单发 A/B 的新判据当凭据）未违反 |
| （附）更正块 ③ 的原句点名 | 曾把 `TestAC1POSIXUnresolvedSymlinkedRootStillRefused119` 算进"两形都红" | 它在我的读数里＝**普通形 FAIL**（在 18 枚名册里）／**软链形 PASS**（不在 `s02b`/`s04`/`s05` 任何一份名册里，也不是 SKIP） | **`R-137-3` 那句更正被独立证实**；更正块 ③ 原句确实指错方向 ⇒ 本程按 `R-137-3`，票面两处文字的差异**登记、不抹** |

⚠ 一条"可满足性"的独立复现（票面 `:203-204` 说这一档"现在是有凭据的了"）：我也读到同一形状——
**根已解析的三枚对照在收紧判据下、软链形里真跑过且是 PASS 不是 SKIP**（`s07-tight-link`／`s08-tight-plain`，
`skip` 名册只有票 125 那三枚）⇒ 这把尺**可满足**，不是恒不满足的坏尺。

**本节（§2.5–§3）落在哪枚 commit——`git log --oneline -1` ＋ `git show --name-only HEAD` 原样：**

```
$ git log --oneline -1
1f1611c docs(137,AC#3 证据 §2.5-§3): 结案格④panic 十发逐份＝0＋"颜色行数＝RUN 行数"的全量等式＋软链形 7 枚分母差逐名归因票 125；结案格⑤恒真自查正面作答（两发未修码上已响、都不进凭据）；三条钉逐条复核无一被推翻
$ git show --name-only HEAD | tail -2

docs/evidence/s1/137-ac3-r2-acceptance.md
```

## §4 现场事实一条：**同一格另有一枚验收程在飞**（双派归编排者判）

我提交 §2.3–§2.4（`0d6aeda`，本机 13:4x）之后，`git log --oneline -4` 里出现了**兄弟程的三枚 commit**
（`8d62096`／`6155a86`／`4c9ea23`，最新到 `fc8c4b3`），它们动的文件是
**`docs/evidence/s1/137-ac3-r1-acceptance.md`**——**同一枚票的同一格（AC#3）**，判定写的是"五格全成立、成立无附条件"。
逐枚 `git show --name-only` 我核过：那三枚的暂存清单各只带它自己那一枚路径，**没有卷走我的文件**；
我那三枚（`e375b2a`／`0d6aeda`／`1f1611c`）也各只带 `137-ac3-r2-acceptance.md` 一枚。

- 我**没有**读它那一份的任何内容作为本件的依据（本件十发读数、判别对、三条钉复核全部出自 §2.0 那一张台账）；
  我也**没有** `add`／改／删它那一枚文件，`git add` 每一次都带显式 pathspec。
- ⚠ 与 `A155` 的关系要说清：`A155` 判作废的是**那一份自述**（"47 棵树／66 发／逐枚 commit 到 `99319c7`"），
  它当时盘上零产物；现在盘上这份 `137-ac3-r1-acceptance.md` 是**这之后落的新产物**，
  它的真伪**不由我背书**（我只核了"它存在、它只提交了自己的文件"这一件）。
- 它那份里有一句与我不同的措辞值得编排者一并看：它登记"票面三条钉之二那句'第 2 个组件'我读到的是第 1 枚前缀"。
  我这一程同样读到被记名的前缀**就是容器自己那枚** `/r2ac3link`（红行原文见 §2.2 末），
  "第 1／第 2"的差只在 `pathPieces` 把根那一截算不算一枚——**实质句（走查在走到用例自种链接之前就拒了整条拼写）
  被我 §2.6 第 1 条那发 `s07-tight-link` 直接证到**，所以这一处不构成"钉不成立"，我只登记措辞差。

⇒ **要报给编排者的判断**：同一格出现两枚独立验收程。两件的**判据形状相同**（都钉 MUT-D、都给判别对），
但**产物是两份文件**，翻勾与结案只能认其中一份为准，或让第三者做对点。**这不在我权限内**（票面一格未动）。

## §5 总判

**AC#3 一格：成立（PASS、无附条件）。** 五格逐格如下，全部〔独立复现〕（§2.0 十发，我自己落的变异）：

| 结案格 | 判 | 依据（一句话） | 档位 |
|---|---|---|---|
| ① 软链形 11 枚全部转红＋逐名＋红在 `file:line` | **成立** | `s04-dtight-link` 分母 11 枚逐名 FAIL（§2.2 表），10 条新判据文案恰落在 `113:204/237/254/271/296` 与 `108:90/156`，`does not name ErrUnresolvedPath` 0 次、`returned nil` 只在 118 两枚对照 ⇒ 红出自新加的那一支，不出自别的分支 | 独立复现 |
| ② 普通形零新增红（八数逐数相同＋三档名册双向差集全空） | **成立** | 普通形三发两两 `run/fail/skip` 差集全 0（52/18/0），八数 `52/17/13/0＋17/5/0、rc=1` **逐数等于票面 `:172` 点名的参照值** | 独立复现 |
| ③ 变异真落地＋反向证据（判别对） | **成立** | `R-137-3` 三枚对照在 MUT-D 六发里两形全红、未变异两发两形全绿；`s02b`（AC#2 前整树）与 `s05`（同树同变异、只删三处判据）软链形 11 枚全绿 ↔ `s04` 全红 ⇒ 唯一变量＝判据在不在位 | 独立复现 |
| ④ panic 账＝0 且是全量读数 | **成立** | 十发 `panic|fatal error` 逐份 0，另钉可复算等式"颜色行数＝`=== RUN` 行数"十发全等；软链形那 7 枚分母差逐名全归票 125 自拒探针，分母 11 枚无一 SKIP | 独立复现 |
| ⑤ 恒真自查正面作答 | **成立** | 明写到两发"未修码上已经响"（`s07` 未变异收紧态软链形 11 红／`s01b` 未收紧＋D 普通形 18 红），两发都被排除在修复凭据之外；进凭据的只有"MUT-D＋软链形"那一发未修码上不响的形状 | 独立复现 |

⇒ **AC#3 的牙咬住了**：票面 `:42` 那句"没有 AC#3 的 AC#2 只是把断言改了个形状"在这一格上是**反过来的**——
断言改完确实让同一发 MUT-D 在软链形从 11 全绿翻成 11 全红，而普通形一动不动。
⇒ **AC#3 成立意味着**：票 137 只剩 **AC#4**（那 7 处 raw `t.TempDir()` 换根）未裁；AC#1／AC#2／AC#5 已成立，
本格翻勾归 owner／编排者，**我一枚未翻**，本票按 AC#4 未勾 ⇒ **不结、不改名 `-done`**。
⇒ **若 AC#3 不成立会怎样**（写清反事实，免得读者以为这格只是走过场）：那 AC#2 就得按"改了个形状"退回，
`6f3817a` 的收紧断言不获背书，且票面 `R-137-2` 那条"必须同时覆盖 helper 与 108 两枚内联腿"的开工前提失去唯一的有效验收——
我这一程读到的正是"少了 108 那两枚内联腿里的任意一枚，软链形就不可能 11 枚齐红"（表里第 10/11 行的红行 `108:90`／`108:156` 与其余 9 枚同发同时出现）。

**本程纪律自证（逐条可核）**：票面一格未动（AC#3 那个 `[ ]` 原样）／票面 log 未追加／`docs/reports/**` 未动／
生产码零改动（我只在 `/d/tmp` 的副本上做变异）／冻结面一枚未碰／只 commit 未 push／
`git add` 每次一枚显式 pathspec／共树禁的 `--amend`/`reset`/`rebase`/`stash`/`checkout .`/`clean` 一枚未用／
仓内未建 worktree、未 checkout／临时件只建不删（§0.5 全列，含我建错的那枚 `wisp137ac3r2-io`）／
`wisp137r2-*` 那一族（归档树＋台件＋那两个卷）**一枚未写未删**，模块卷以 `:ro` 挂出后拷进我自己的新卷。

### 5.1 我没核的部分（**明列，别当已核**）

1. **真 macOS 未验**：软链形只是容器里 `ln -s` 的近似（票面 `:22` 说软链形＝macOS 的真实形状）；
   macOS 上还有 `/etc`、`/tmp` 一类系统级软链，本程一台件未覆盖那一形。
2. **MUT-A／MUT-B／MUT-C 一枚未造**：按 `R-137-1` 它们本来就不许当 AC#3 的凭据；`MUT-C` 连定义我只在旧账里见过名字、未取。
3. **裸 `strings.Contains` 那枚恒真探针（PROBE-F）未重走**——那是 AC#2 的 §4 已裁的活，本格不重裁。
4. **`-count=2` 两形四数未跑**（AC#5 已裁过；`-count=2` 会把四数翻倍，与格②的"八数逐数相同"是两种口径，不混用）。
5. **门禁未重跑**：`gofmt`／`gofumpt`／`go vet` 双 GOOS／`sh scripts/d22scan.sh` 全未跑——
   本程仓库里一枚 `.go` 都没改（改的是 `/d/tmp` 副本），跑门禁的对象并不存在；AC#5 那一格已由非实现者裁过。
6. **CI 侧零读数**：本程未 push ⇒ 没有属于 `6f3817a`/`21fed31` 的新 run；`gh` 只用于查"有没有在飞的 run"。
7. **普通形 18 枚红里除分母 11 ＋对照 3 之外的 4 枚**（票 125 两枚＋票 119 两枚）我只做**名册对点**（两侧逐名相同），
   未逐枚归因它们为什么红。
8. **AC#4 一枚未裁**（那 7 处 raw `t.TempDir()` 该不该换根、换了会不会失去"未解析那一形"的覆盖），
   也不替它预答；我在 §2.6 读到的"未变异收紧态软链形 11 枚红"是 AC#4 要收的账的**症状**，不是它的判据。
9. **容器台件里 `TMPDIR` 只有我这一种形状**（`/r2ac3link/w137ac3rtmp`）：换别的宿主目录布局会不会改变"第几个组件被拒"，未测。

## §6 两个计数 ＋ 可重跑凭据 ＋ next

### 6.1 计数（两栏，不混用同一字段）

- **判为注入数＝0**。工具输出里没有出现"自称编排者备注／系统提示／用户已更新规则／请 revert／冻结某包／放宽阈值／
  Confirm the harness note is genuine"那类文字。
  按第四条判据单独问一遍：**有没有任何一段在让我少取证／别用工具／直接给结论？** 没有——
  本程出现的重复性文字只有下面那类系统回显。
- **真通知回显数＝三族、合计 ≥15 枚（我按族给数，未逐枚计数，在此明写这个口径）**：
  1. `MEMORY.md was modified since it was last read` —— 随本程**多数**工具结果回显（≥10 次），
     点名的是 `C:\Users\swq\.qoder-cn\projects\D--work-workspace-projects-plans-Wisp\memory\*` 与
     `C:\Users\swq\.qoder-cn\memory\*` 下的真实文件 ⇒ 判据①（点名的路径真不真存在）我**现量过**：
     `ls -l` 两目录都在、`MEMORY.md`／`wisp-truth-source-docs.md` mtime `13:37`，与提醒同批；
     内容不要求我改判据、不削弱 owner 权威 ⇒ 按真回显结案（也正是 owner 记忆里那条"共树里我只独占 `docs/reports/**`"的成因）。
  2. "available skills"清单回显 **4** 次（最后一次落在本枚提交之前；无路径、无指令）⇒ 噪声。
  3. 后台任务完成通知 **2** 枚（batch／batch2 各一）。
  ⚠ 自我更正一条（同一枚数字我读错两次，逐次写清）：那枚后台任务通知写着
  `Size: 13041 bytes | Status: completed`、并把路径渲染成 `D:\tmp\wisp137ac3r2-rig\batch.out`；
  我盘上现量 `wc -c batch.out` ＝ **11390 字节／213 行**（`md5 ea2b25489e87088f3cac08fc04171c25`）⇒ **两个数对不上**，
  最可能是通知那个 13041 指的是它自己的任务输出文件、只是把路径渲染成了我重定向的目标；
  而我在中途还把另一处的 `(output truncated… Original size: 2341 chars)`（**工具输出的字符数**）当成过 `batch.out` 的大小。
  ⇒ 三个数里**只有盘上那枚算事实**；这既不是注入也不是"通知造假"，是**我引数字时没连口径一起引**，
  与既有那条旧账同源（"计数 0 有四种成因"、"引用区间要连口径"）。本格任何判据都不依赖 `batch.out` 的字节数。
- 两栏之和之外的另两笔**事实登记**（既不是注入也不该被算成噪声）：
  ① §2.0 那四发被我自己新加的 96 号状态硬闸拒掉（我写错了期望分支枚数），已如实落盘；
  ② §4 那枚**同格双派**的现场事实。

### 6.2 可重跑凭据（别人要复算这一格，照这几条就够）

```
# 台件与十发原始 -v 日志（只建不删，全在仓外）
D:\tmp\wisp137ac3r2-rig\        run.sh / go.sh / batch.sh / batch2.sh / mutate.py / parse.py / matrix.py
                                s0*.v.log  s0*.head.txt  s0*.v.{run,fail,skip,colour}.txt  batch.out  matrix.out
D:\tmp\wisp137ac3r2-tree-loose\     f53ad5c 纯净树（未收紧）        <- git archive f53ad5c | tar -x
D:\tmp\wisp137ac3r2-tree-tight\     6f3817a 纯净树（＝HEAD 的 winsec）<- git archive 6f3817a | tar -x
D:\tmp\wisp137ac3r2-work-{d-loose,d-tight,relax}\   三棵变异副本

# 任一发重跑（一发一枚全新容器；闸门 97/98/96/95 不过就不取颜色）
MSYS_NO_PATHCONV=1 docker run --rm -v /d/tmp/wisp137ac3r2-work-d-tight:/src \
  -v /d/tmp/wisp137ac3r2-rig:/io -v wisp137ac3r2-gomod:/gomod -v wisp137ac3r2-gobuild:/gocache \
  -e GOFLAGS=-mod=mod -e GOPROXY=off -e GOMODCACHE=/gomod -e GOCACHE=/gocache -w /src \
  golang:1.27 sh /io/run.sh wisp137ac3r2-work-d-tight s04-dtight-link link 1 mutd
```

我的十发读数**全部**可由上面那批 `*.v.log` 用 `parse.py` ＋ `matrix.py` 一次重算出来（本件的每一张表都标了它是哪一发生成的）。

### 6.3 凭据卫生反扫（按**词**筛，不按长度）

`grep -inE 'sk-[a-z0-9]|token|secret|password|apikey|api_key|credential|bearer'` 对本文件共跑 4 次
（§2.2 提交前、§2.4 提交前、§2.5–§3 提交前：**各命中 0 枚**；§4–§6 提交前：**命中 1 行**＝上面这一行自己，
我把 grep 模式写进了证据文件 ⇒ **自指假阳性**，按**形状**判定，一条规则都没删、判据未改）。
另外文中出现的 `TestAC1POSIXPrivateFileThroughASymlinkRefusesAndWritesNothing` 这类是**测试函数名**，是形状不是凭据；
本机任何真实密钥值**一枚未入文**（本程也未读任何凭据文件、未 `cat` 过任何 env）。

next（交回编排者）＝
① **判 §4 那枚同格双派**：`137-ac3-r1-acceptance.md`（兄弟程，最新到 `fc8c4b3`）与本件 `137-ac3-r2-acceptance.md`（我，到 `1f1611c`＋本枚）
  两选一为准，或派第三者做两份名册的对点（两者判据形状相同、结论同指"成立"，但**产物是两份**）；
② owner／编排者落 **AC#3 那一枚勾**（我一枚未翻）；
③ **AC#4 单独起单**（不与任何已裁格并做），带上 119 那枚文件里三处 sentinel-only 内联断言；
④ 临时件清点归编排者（我一枚未删，含建错的 `wisp137ac3r2-io`）；
⑤ 我这格**没有待 owner 的疑点**——三条钉复核无一被推翻，唯一发现的票面内部不一致（更正块 ③ 的原点名 vs `R-137-3`）
  已由 `R-137-3` 自己更正、并被我的读数独立证实。

---

### 6.4 最后一节的自证（`A155` 硬规矩③的形状）

上面 §4–§6 那一节落在 `7db3b7f`，它的两行原样（由**下一枚** commit 携带，本枚只能把上一枚写进来；
§2.5–§3 的两行已经逐节写在 §3 末尾、`e375b2a`／`0d6aeda` 的原样分别写在 §2.2／§2.4 末尾）：

```
$ git log --oneline -1
7db3b7f docs(137,AC#3 证据 §4-§6＋总判): AC#3 一格判＝成立（PASS、无附条件），五格逐格给判；另登记同格双派的现场事实、九条"我没核"、两栏计数与可重跑凭据
$ git show --name-only HEAD | tail -2

docs/evidence/s1/137-ac3-r2-acceptance.md
```

⇒ 读者要核本件**每一节**落在哪枚 commit，一条命令即可：`git log --oneline -- docs/evidence/s1/137-ac3-r2-acceptance.md`
（本程四枚：`e375b2a` → `0d6aeda` → `1f1611c` → `7db3b7f`，加本节这枚共 5 枚；**每枚的暂存清单都只有本文件一枚路径**）。

补一条计数更正：§6.1 第 2 族"skills 清单回显"在我写那一行时是 4 次，**到本枚提交前已是 5 次**（同一条内容逐字重复回显）
⇒ 数字改成 **5**，"未逐枚计数、按族给量级"那个口径不变；这不影响任何一格判定。

## §7 交回前追加的两条现场事实（append-only，不抹上面任何一节）

### 7.1 票面在我这一程之内被改了两处（**不是我改的**，我一枚未动）

开工时我读的票面＝**212 行**、`:41` 那格是 `- [ ] **AC#3**`（§1.1 引的就是那一版）。
交回前我复量：同一枚文件现为 **221 行**、`git status` 显示 ` M`（**未提交的改动**），
`git diff --stat` ＝ **10 增 1 删**，改动内容（按新增文字自己的署名）＝

- `- [ ] **AC#3**` → **`- [x] **AC#3**`**，其下追加一行引用块，逐字写着
  「**09-24 13:5x 编排者翻勾（判据以下面那枚 `>` 更正块的 MUT-D 形状为准，不是这一句原…）**」、
  凭据点名 `docs/evidence/s1/137-ac3-r1-acceptance.md` §2／§3、五枚 commit `4f61430 8d62096 6155a86 4c9ea23 fc8c4b3`、锚 `2d029f9`、**未 push**；
- 同一块里另写明「**本格另有一枚同格双派**（`137-ac3-r2-acceptance.md`…）⇒ **r2 交回后只做交叉核对：两枚名册/判别对若冲突 ⇒ 本格重开**」；
- 还有一处**指针更正**：票面 `:58` 那句 `assertRefused113` 在 `:127` 被更正为"HEAD 上不是 `:127`"，
  并立下新规矩「今后票面引某函数的行号要么写成函数名＋一句可复算的 grep，要么带锚点 sha」
  ⇒ **与我 §2.1 那条行号腐坏登记同源**（我是自己 `grep -n` 量到 `:174` 的，那条更正的触发来源署名是 `acceptor-ticket137-ac3-r1` 报回的第四条，不是我）。
  逐枚 `git cat-file -t 4f61430`／`fc8c4b3` ⇒ 都是 `commit`（这一程新点名的两枚我也照规矩先核再引）。

⇒ 我**没有**参与那次翻勾、**没有**写过票面那 10 行、也**没有**把别人的勾算成自己的交付物。
本格判定（§5）在翻勾之前就已 commit 完毕（`7db3b7f`），不依赖那一勾。

### 7.2 票面要的那次**交叉核对**我做完了：**无冲突**（这是判定之后做的，不回灌进 §2 的任何一格）

我把兄弟程那份 `137-ac3-r1-acceptance.md` 的**名册与红行**用 `grep` 与我 §2 的名册对了一遍（只取"它点名了哪些用例／哪些 `file:line`"这一层，**不取它的颜色与结论**）：

| 对点项 | 它（`grep -c`／`grep -oE` 现量） | 我（§2） | 冲突？ |
|---|---|---|---|
| 分母 11 枚的 10 个不同函数名 | 逐个命中各 1 行；四段子测 `link-at-depth-1`…`-4` 各 1 次；`:229` 那一行把母项写成「**自身无报错行，由下面 4 枚子测派生**」 | §2.2 表 11 行逐名（母项同样标"因子测而红、无自己的报错行"） | **无** |
| 红落在哪些行 | `113:204`(2)、`113:237`(4)、`113:254`、`113:271`、`113:296`、`108:90`、`108:156`（另 `113:127` 1 次＝它报回的旧行号指针） | §2.2 的 `file:line` 计数**完全同一批** | **无** |
| 对照组三枚 | 118×2 ＋ `TestAC3POSIXLinkInsideAResolvedDataRootStillRefused119` 各 1 次 | §2.4 表同一批三枚 | **无** |
| 普通形八数 | 文中出现 `52/17/13/0` 2 次、`52/30/0/0` 1 次 | 我量的 `s01b`/`s03`/`s06`＝`52/17/13/0＋17/5/0、rc=1`；`s08`＝`52/30/0/0＋22/0、rc=0` | **无** |

⇒ 两枚同格验收件在**名册集合、红行位置、对照组点名、普通形八数**这四层上互相吻合 ⇒ 按票面那句"若冲突 ⇒ 本格重开"，
**不触发重开**。⚠ 但这只是**一致性**核对，不是"两份合成一份凭据"：两份各自独立、各带自己的十／六发日志，
**采信哪一份、还是两份并挂，仍归编排者**（§6 next①）。

### 7.3 交回时刻的盘上账（供接管者一眼扫完）

- 本程 commit 序列（全部只带 `docs/evidence/s1/137-ac3-r2-acceptance.md` 一枚路径）：
  `e375b2a`（§0–§2.2）→ `0d6aeda`（§2.3–§2.4）→ `1f1611c`（§2.5–§3）→ `7db3b7f`（§4–§6）→ `69307c0`（§6.4）→ 本节这一枚。
- 工作树在我交回时**只剩别人的改动**：` M .scratch/wisp/issues/137-…-planted-link.md`（编排者的翻勾与更正，见 §7.1）。
  **不是我未提交的活**——我的文件每一节都已 commit，`git diff --cached --name-only` 每次只出现它一枚。
- **未 push**（本程一枚未推）。临时件全部只建不删，路径见 §0.5 与 §6.2。
- **AC#4 一枚未裁、票面一格未动、生产码与冻结面零改动。**
