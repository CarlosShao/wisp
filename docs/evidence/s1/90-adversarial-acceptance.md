...
## 11. 验收期间现场变了（本代理**没参与**，但会改变上面两句的读法，必须登记）

本文件的**一切判决都钉在 `d5564c2`**（票面指定的被验收 SHA）上——快照是从它 `git archive` 解出来的。验收期间 `dev` 上发生了两件事：

1. **票 101 落地了 `7286297`**（`feat(101): 把票 90 的档位存储接上装配根`，动 `cmd/wisp/run.go` +137 与一张新测试）：
   HEAD 上 `cmd/wisp/run.go:343` 已有 **`Modes: rt.modes`**，`internal/perm` 也终于有了 importer。
   ⇒ **第 6 节残留 (b)（`unwired.go:127` 那句用将来时写的 `consumed:`）在 HEAD 上已经不是将来时了**（该行原文一字未改，现在为真）。
   ⇒ **第 10 节②那句"今天没有任何生产进程会读这个档位"只对 `d5564c2` 成立，对 HEAD 不再成立。**
   ⚠ 但本代理**没有验收票 101**（不是我的票，也不该顺手替它盖章）：`7286297` 的读数、它是否把 `perm.Store` 的 L2 确认通道接上、
   面板只读显示那半——**归票 101 自己的验收方**。本代理只在 HEAD 上做了两行 grep 确认"事情变了"，没有跑它的测试。
2. **索引里出现了票 90 票面的改名** `.scratch/wisp/issues/90-…-modes.md → …-modes-done.md`（同批还有票 94/96），是编排者一侧的动作；
   **本代理一个字没碰票面**（两枚提交 `f3f48a2`/`16a79be` 的 `--name-only` 各只有本文件，已核）。
   ⇒ 与第 10 节①的先后关系：**本代理的判决是"能挂 `-done`，但先补 R21③ 那一句"**；若 `-done` 已先落地，那句就还欠着，
   请在下一张碰票面的会话里替票 90 补上（**成本一行**）。本文件其余结论**不依赖**这个改名。
