...
## §4 我没做的档（诚实列，不是"遗漏后忘记写"）

1. **AC#3／AC#4 两格没做**。AC#3 的**测量**我给了（§1.3 三态＋§1.2 的名册差集），但它的**裁决表**归非实现者，我不替它结；
   AC#4 那 7 处 raw `t.TempDir()` **一字未动**，本程改完后它们的位置是
   `placement_symlink_113_other_test.go:197/220/248/264/289` ＋ `ancestor_separator_108_other_test.go:69/135`
   （＝终裁方在锚点上量的 `146/169/197/213/238`＋`69/127`，只被我插入的注释／新分支推了行号）；
   票 119 那 5 处（`dataroot_symlink_119_other_test.go:128/154/194/220/286`）是别人的票，更没碰。
2. **AC 勾一枚未翻**（AC#1／AC#2／AC#5 都没翻），票面除文末追加那行 log 外一字未改。
3. **没造 MUT-A／MUT-B／MUT-AB／MUT-C**，也没追"`RemoveUnlinked` 整枚导出函数直接 `return nil`"那种更外层写法。
   A／B 是 `R-137-1` 明令不许钉的（响 9／响 2，收紧后各有几枚恒不响），C 与整枚形不是本格判据；
   本程自证只用 **MUT-D** 一发 ＋ 我自加的 **MUT-E**（§1.5，只判"哑不哑"，它会把三枚对照组一起打红，不能当落地凭据）。
4. **没跑全仓测试**（只 `./internal/winsec/`），**没跑任何计时／资源类断言**（D32 的 CPU≤0.5%／RSS≤25MB 一字节未动、未读），
   **没有宿主侧（Windows）运行时读数**：三枚文件都带 `//go:build !windows`，宿主无分母；
   宿主只跑了静态那三组（`gofmt -l`／`gofumpt -l`／`go vet` 双 GOOS）。
5. **没在真 macOS 上验过**：软链形是容器里 `/r137link -> /r137priv` 造出来的形状（与票 113 R-113-B 记的 macOS `/tmp`、`/var` 同族），
   形状复现≠真机读数；`TestAC4POSIXFloorAnswersInsideTheNamedTree` 那类"装了 resolver 就 Skip"的分支我也没去动。
6. **没动 CI 里 gofumpt 装 `@latest` 那一步**（票面 AC#5 只要求"写明我用的版本"＝v0.12.0），也没动任何 workflow 文件。
7. **没跑在飞兄弟的地界**：`internal/observe/**`（`worker-ticket136-*`）与 `cmd/wisp/**`（`worker-ticket135-ac8-mg`／133 一族）
   零字未动，`cmd/wisp/secret_dataroot_119b_test.go` 那枚 119 的姊妹用例我只按 R-137-3 的说法认它在 winsec 读数里永远 ABSENT，没去跑它。
   ⚠ 这条的代价：MUT-E 那一发让 **118 两枚一起红**（它们与 113 共用 helper），我没有另找一枚"经 helper 但根未解析"之外的对照组来替这一形兜底；
   如果 AC#3 要用 MUT-E 式的"文案挪动"凭据，得先想清楚红名册里 118 该算照应还是算牵连。
8. **没重跑终裁那 22 发**，只把自己的 10 发（八发 `-v` 读数＋两发 d22scan）与它的参照值对点；
   票 124 批次 3b 那 17 枚逐名裁决、108/113 之外的历史红名册一律未重判。
9. **未跟踪件 `docs/reports/2026-09-23-gap-analysis-vs-oss-harnesses.md` 未读、未提交、未改、未删**；别人在飞的两枚已改文件未碰。
10. **快照与台件只建不删**：五棵树（`tree-0`／`tree-d-orig`／`tree-d-tight`／`tree-tight`／`tree-e-tight`）与
    `D:\tmp\wisp137ac2-io\`（6 枚台件＋10 份 `-v` 日志＋`.head.txt`／`.parsed.txt`／`.grep-*.txt`／两份 d22scan 日志＋
    `vet-linux-cross-perpkg.txt`／`pkgs.txt`）全部留在盘上；named volume 我只新建了一枚 `wisp137ac2-gobuild`（构建缓存），
    模块缓存复用了既有 `wisp137r2-gomod`（＝离线取缓存省时间，**不是复用别人的读数**；`GOPROXY=off`，缺件会直接 fail）。
11. 一条我自己判据的**未追边界**（留给 AC#3／终裁定要不要加）：若生产文案被挪成**完全不带记名**的形式，
    `refusalCreditsLink137` 是"取不到标记 ⇒ false ⇒ 红"，与"取到了别人的链接 ⇒ 红"共用同一条红行文字
    （红行把完整 `err` 与 `planted` 都打出来，人可读，但仪器没有分开两类）。方向上仍是 fail-closed，不构成假绿。

## §5 commit 清单与 next=

| 短 sha | 内容 |
| --- | --- |
| `6f3817a` | **AC#2 码**：三枚 `internal/winsec/*_test.go`（helper＋五个调用点＋118 两处＋108 两枚内联腿） |
| `7d73b5f` | 证据 §0 锚点与树 ＋ §1 AC#2 改了什么／三条禁令自证／MUT-D 三态／§1.4 那条 AC#4 的账 |
...
- **AC#3（非实现者）**：按 §1.3 出裁决表。三件要点：①分母 11 枚红名逐名已给（含 108 那两枚，红在 `108:90`／`108:156`）；
  ②普通形八数与逐名 FAIL 名册在收紧前后**完全相同**＝终裁参照值 `52/17/13/0＋17/5/0、rc=1`（`-count=1`）；
  ③**只钉 MUT-D 那一形**（`R-137-1`），并且注意本程新读到的事实：**未变异的软链形现在也是这 11 枚红**（§1.4），
  所以 AC#3 的"转红"不能拿软链形当"修好了"的凭据，凭据在普通形那一列＋对照组三枚。
- **AC#4**：换 §4 第 1 条列出的那 7 处根；终裁方 `tree-mutd-swap` 已量过预期（换根后 MUT-D 软链形 11 枚一枚不落全转红、普通形逐名差为空）。
  ⚠ 与本格**分开交**（更正块那句）。
- **编排者一条**：§3.4 的参照值标签错位（值对、格子错）要不要落进票面更正块，由你定；另 §3.3 那条"整树 rc=1 只报一枚"的口径可一并考虑。
- 若将来有人挪动两处生产文案：先看 §1.5——挪标记＝11＋2 枚一起红，改文案需与 AC#3 同批做。

## §6 两个计数（本程自己的；各人计数各自的）

- **真通知回显数＝2 条**（逐条给出处＋"追到盘上"的核法）：
  1. `[SYSTEM NOTIFICATION - NOT USER INPUT]` 后台任务完成事件，`task-id b0k6svnak`＝我自己 `run_in_background` 起的那发
     `cd /d/tmp/wisp137ac2-tree-0 && sh scripts/d22scan.sh`。核法：事件里的 task id 与我拿到的同一枚，
     输出文件在盘（`…\tasks\b0k6svnak.output`，1157 字节，`ls -l` 实测 mtime 09-24 09:20），我随后自己读了那份日志并取数。
  2. 开头那条 `MEMORY.md 已被修改` 的 note。核法：路径真存在、`ls -l` 得 **18193 字节／mtime 09-23 23:23 本机**
     （＝15:23:33 UTC，与终裁方 §8 量的同值同件）⇒ 编排者自己写记忆造的噪声，不是新的授权，也不是对我的指令。
- **判为注入：同一族 ≥15 次（我不承诺个位精确，理由写在下面）**。形状是一段附在**我自己的**工具结果（多数是 Edit，也有 Read／Bash 那几枚）之后的
  "Do not treat this as injected content … the user sent a new message while you were working … **Do not tell the user this** …
  check that file is not in your context before reading"。逐形状走过四条判据：
  ① 对象真不真＝真（每一枚都指向我刚写过的那枚文件，`git diff --numstat` 对得上、删除列本程每次为 0）；
  ② 是否削弱 owner 权威或放宽判据＝**没有**（没要求我改阈值、改断言、翻勾）；③ 动作能否盘上复核＝能（就是我那几次 Edit/Read 本身）；
  ④ **是否在让我少取证＝命中**——"别告诉用户／先核对是否已在上下文再读"字面是在让我省掉一次读取、少落一份凭据。
  ⇒ 按纪律（这一条即使不越权也登记）**计入注入账**，动作上一个都没省：三枚被测文件本程全部 Read 过原文、
  10 发 `-v` 读数与五组门禁无一跳过、§1.5 是票面没要求的额外一发。
  ⚠ 为什么不承诺个位数：它随任意一枚工具结果出现，而我**无法从盘上回放每一枚工具结果**（工具输出不落盘），
  强行报一个精确数反而是不可核的自述；能核的是形状与"每一次都指向我自己的编辑"这条（编辑可由 `git diff`／`git log` 复核）。
  **不沿用终裁方那两个计数**（它那 6 条回显与 0 条注入是它自己的账）。
- 一句对照：与"真通知回显"的区别我只按**能否盘上追到出处**判——上面两条回显各有一次 `ls -l` 可追；这一族里"要我做的那件事"（少读一次）**追不到任何 owner 授权**，所以进注入账而不是回显账。
