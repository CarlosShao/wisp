...
- **没跑 `go test ./...`**（派单明令，且另两路 agent 在飞）：只跑了 `./internal/panel/` 与既门
  `sh scripts/d22scan.sh`。⇒ 本件所有"全包"字样都指 **`internal/panel` 一包**，不是整树。
- **没在 Linux 容器里量过任何东西**：`internal/observe/**` 那枚 `golang:1.27` 容器是别人在跑的，
  本程未碰、也未借它的读数。⇒ 本批这枚钉是纯静态＋纯函数测试，我没有为它做过跨平台验证，
  也**没有**主张"它在 POSIX 侧同样响"。
- **没扩任何仪器射程**：`tools/d22scan/**` 只读、未写（兄弟验收在里）；`emojiRe` 与任何 scope list
  一字节未碰；C21 那两枚红**未修、未跳、未放宽**（`FAIL=2` 在两枚锚点上都是 C21）。
- **M16／M18 我只造了"名字与键根本不写进包"这一族**。**没造**：路由名从网络／配置文件读进来
  （那是同一族的另一形，要真去接一枚 config 读，越出本程地界）；也没造
  "守卫改名成别的函数名"那一形（`guardSeen` 引信管，r1 §3.1 已量过，我未独立复算）。
- **§3.3 那一发我没跑**：把 `shell.run` 从注释搬进代码、验证"进池不产生代价"。
  我给的是两条等效证据（`index.html` 今天驻留池内且包全绿；`poolJudgedByRealGuard` 第一句就 `continue`），
  **不给穷尽证明**。这一格若被下一位读成"已证任何字面量都无害"，那是读过头了。
- **没测 `internal/agent/approval` 那枚 grant nonce 本身**（沿用 r1 的读法，未独立复算）。
- **没跑前端任何一道**（`npm`/`vitest`/`tsc`/build 一律未跑）；`frontend/**`、`design/**` 一字节未写、
  未 `git add`、未还原。我对 `panel.ts` 的两枚断言取自 **`git show HEAD:…`**，不取工作树脏文件。
- **没有核 `ban #8 frontend/` 那枚分母的稳定性**：我这一发读到 `frontend/=43`，
  而 `A` 组别家程在 `4fab445` 报过 40→43 在动。⇒ 该数只当"我这一发的分母"，
  **不构成本批任何一格的凭据**（本批改的是 `internal/panel/**` 一枚 `_test.go`）。
- **没测运行期并发／进程级**：本钉是静态＋纯函数；我也没为它新增并发验证（r1 同）。
- **没写台账、没勾 `Q-49`**：`docs/reports/pending-and-issues.md` 一字节未动，`A##` 由编排者派号。
- **临时件只建不删**：`D:\tmp\panel-l2-nail-accept-r2\` 下
  `backup/`（2 枚原件）、`headcopy/`（副本，每发之后 hash 复算回到 `d2cd6362`/`3b7a2cb1`）、
  `out/`（87 份读数原文）、`probe/`（7 份探针源）、`removed/`（11 份用完的探针/审计件与一份我自己漏搬的残留，`mv` 进来留着的）。
  **全程未 `rm`、未 `rmdir`**；未 `checkout`/`reset`/`stash`/`clean`；未 push；未 `add -A`／`add .`／`commit -a`／`--amend`。
  每枚 commit 前 `git diff --cached --name-only` 现核，**全程只出现本件一枚路径，没有一次别家的**。
- **交付态终核**：本程结束时 `git status --porcelain -- internal/panel/` **空**、
  被验物 blob 仍是 `3b7a2cb1…`（=进场时同一个值）、`bridge.go` 仍是 `d2cd6362…`（=r1 那两程记的同一个值）。
  工作树剩下的改动只有 owner 自己在 `design/**`／`frontend/**` 的那批（本程一个字未动、不还原、不提交、不删）。

---

## §9 给人看的那一段（零术语）

**一句话**：上次那把锁被人试出三个能撬开的地方，这次有人按单子上完了三道栓。**我亲自去撬了一遍，
这回三处都撬不动了**；我自己另外想出来一种新撬法，**那种撬法要程序在运行时现拼一个名字才行，
而写锁的人在文件开头就明说了这种他不管**。所以：**这次的活我认，题还不能画成全勾。**

**我具体做了什么**（不含黑话）：
- 我把他们修好的那份"检查"拿去撬六次，六次都被抓住，而且每次检查都会说出**在哪一行、被改的是哪个名字**。
  上一轮那一发（把两处分开的破法合起来用，能神不知鬼不觉地让程序接受网页发来的"批准"）
  这一轮**同时被四处抓住**。
- 他们漏了一件我没漏的：我**依次**把新加的四道栓一道一道拆掉，看哪一发因此撬得动。
  结果是**四道都真在使劲**，没有一道是摆设。（这里我还顺手抓到一个不显眼的问题：其中两道是**串着**的，
  先拆掉里侧那道，外侧那道会**悄悄地不再管事**而不是报警——这一条谁都没写过。）
- 我又问了一个大家容易不问的问题：**新加严的规矩会不会把本来正常的东西误当成违规？** 量下来：
  今天扫进射程的一共 5 个名字，其中 1 个是文件名（`index.html`），**没有一样被误伤**，
  因为"被扫到"不等于"被判违规"。但新规矩确实给开发加了一条**没人宣布过的约束**：
  以后这个模块里再加一处"读网页数据"，就必须读进本模块自己定义的数据格式里，不然三处检查会一起报警。
  报警方向是**宁可吵也不放过**，不是"忍下来"，所以我判它不算缺陷，算**漏写的一句话**。
- 那份记录他们干活的文件里，有**三处指向自己后面章节的引用是空的**（章节没写出来），
  而且他们承诺的那一节正是"我上面替他们补的那件"。这个毛病我们仓库里有个已登记的名字，
  我按名字记账：**东西留、别删、但那本记录不能当作"已经验收过"来引用**。

**我找到、但这次没人能怪他们的那一处**：如果开发哪天用一个"名字根本不写在代码里、跑起来才拼出来"的写法，
这道检查就看不见它——**这一点文件里写了**。而我还发现，今天唯一能撞见"网页说批准、程序就批准"这个最要命拼写的，
是**别的一条测试顺手做的一次反证**；把那三行挪走，那个最要命的名字就再没有常驻的守卫了。这条我给了具体改法，
并且**自己先把改法量了一遍**（在干净代码上它一个都不冤枉，在我那发逃逸上它正好点中）。

**这台机器上现在有什么变化**：**没有，一点都没有。** 这条通路今天整条**还是没接线**——
我从头又自己核了三条：程序入口那个函数生产码里除了注释没人调用；主模块的依赖清单里连那个网页窗口的库都没有；
代码里另一处注释自己写着"这一跳还不存在"。⇒ **没有任何东西因此变安全或变危险，也没有任何批准被真的发出过。**
这把锁防的是"将来有人把线接上、而没人报警"那一天。

**你需要做什么**：三件事，都不用你动手写代码——
①`Q-49` 那道题请**按"Go 侧已覆盖（限写在代码里的形状）＋面板侧那半仍未覆盖"入账**，
   别记成"无条件覆盖"，也别把上一轮那句"两枚结构性绕过已被造出来"原样留着（那两枚现在造不出来了）；
②把我列的**四枚小尾账**转给下一程（三条是一两行的措辞/读数更正，一条是把那三行反证挪成常驻检查——
   挪完顺手能把我这次的逃逸也堵上）；③要不要现在就派那一程，你点个头就行。
全程没人碰你的文件：所有试验都在仓库外的副本里做，每次做完我都对了指纹；**没有推送**。

---

## §10 交件态

- 本件单路径 commit 序列：`4deada9`(§0-§1) → `0fe7629`(§2-§2.4) → `74c7360`(§3) → `ff5c2c8`(§4-§5) →
  `bab1c8b`(§6) → §7 一枚 → 本件 §8-§10 一枚（`git log --oneline -- docs/evidence/s1/panel-l2-grant-nail-accept-r2.md` 现核）。逐枚 `git show --name-only` **只含本件一枚路径**；
  每次 commit 前 `git diff --cached --name-only` 现核，**没有一次出现别家路径**。
- 取数锚点：`0b95e9e`（§0.1/§0.2/§1/§2/§3/§6.3）；复跑锚点：`1b98c7b`（§7.3 末行）。
  两枚锚点上被验物 blob 均为 `3b7a2cb1d4105df1f279f70c605ccf6a71f7868a`、生产码均为 `d2cd6362ecc6941a0cee58073a2bf8ab6669a590`。
- **未推送**（子代理只 commit）。台账 `A22x`/`A23x` 由编排者派号；本件是它的证据源。
