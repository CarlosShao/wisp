...
> **【更正 · `R-133-6`（中，证据面）· 本程 `worker-ticket133-ac2-fix` 09-23 22:0x +08 插入；上面那两行原文照录、一个字未抹】**
>
> 上面那句"把本尺摘掉，这五发的形状**今天仍然没有任何一枚用例会红**"是**全称量词用假**，
> 与同一份文件 §2 逐格表里第①栏（无仪器树 `c720494`）自己的读数矛盾，验收方
> `acceptor-ticket133-r1` 在 `docs/evidence/s1/133-adversarial-acceptance.md` §6 第 1 问与 §8 `R-133-6`
> 两处都点了这一枚。按读数的正确写法是：
>
> - **摘掉本尺后仍零红的只有 X14 的两拍**（该发在 `c720494` 上 rc=0、100/53/0/0）——
>   这一形本尺是**独占目击者**，也就是本尺的存在理由；
> - **N-3／X4／X8／X12 四发摘掉本尺之后各有一枚用例会红**，红的是票 131 的门
>   `TestAC4EveryLegIsNailedOrRuled`（验收方①栏四行读数：每发 rc=1、100/52/1/0），
>   本尺在这四发上是**第二把独立目击**，不是唯一目击——这正是归属硬线（③栏：把 131 的门
>   `-skip` 掉仍红、红名是本尺）要单独复跑的原因。
>
> "删掉它哪条用例会红"这一问的正确答案不变，仍是 `TestAC1AC2DispatchHopGate133`
> （整包名册里唯一新增的一枚，`comm` 逐名已独立复现 101/54 → 100/53）。
> 越界的只有那句把"这一枚用例"加强成"五发都零红"的话——它把"本尺添了目击者"
> 说成了"除本尺外无人目击"。本程另在合并态快照上把本尺 `-skip` 掉复跑六发，
> 逐发红名见 `docs/evidence/s1/133-ac2-fix.md` §3.4（读数文件 `/d/tmp/wisp133-ac2fix-out/F-<发>-noscale.log` 六份）。
> 上面那句原文留在原地，是为了让下一位读者看得见它是**怎样被推翻**的，不是留一处无人认领的错话。
...
接续方 `worker-ticket133-ac1-r2` 只补 §4／§5／§6 三节，硬禁令"不许重做仪器、不许改 §2／§2.6／§3 任何一行读数"
生效——下面每一条都是**照实登记"本程没做到"**，不是"补一句就当做到"。

1. **AC#1／AC#2 终裁权不在本程**。§2 五发（含 X14 两拍）三态原文、§2.6 一览表、§3 覆盖面主张
   是前任 `worker-ticket133-ac1` 的**实现方自判**（票面 AC 明写"裁决表由非实现者出"），
   本接续方也是实现方同一侧，**无权终裁**；本 §4/§5 只是把前任挂着的三节"（待填）"填成读数，
   判定仍要交给**非实现者验收方**在锚定的 sha 上复算。若验收方复跑发现与 §2 任发读数不符，
   按 brief 的规矩两边都留着、登记"读数分歧"、由验收方裁，本程**不改数**（本程未复跑 §2，
   因此也**没有第二读数可登记分歧**——这是"未复跑"，不是"复跑一致"）。

2. **方法值那一支（`v.M(...)`）与跨包那一支，票面已划出本票地界**，本节**不当已覆盖**：
   - **方法值**：票面 §"第 5 发 X14" 明写"⚠ 方法值那一支（`v.M(...)`）复验方未造（本包当前没有
     可借的形状，见其 §9.2/§10），**不算已测**"；前任 §3 第 3 条形也把它登记在"清单式看不见、
     图式能看见"的名单里。本接续方 §4/§5 未新增任何覆盖。
   - **跨包**：票面 §"第 5 发 X14" 明写"⚠ 跨包那一支出 `cmd/wisp/**` 地界，**本票不接**，由编排者
     另裁"；前任 §3 第 1 条形同样登记。本接续方不声称覆盖。

3. **§2 五发的三态读数（含"门开着／门关着"两份）与 §2.6 一览表未复跑**。那是前任在
   `/tmp/wisp133c-base`（`git archive c720494`）／`/tmp/wisp133c-ins`（`git archive 3d43c3f`）
   ／`/tmp/wisp133c-ins-pristine` 三棵树上做的变异复现，硬禁令"不许重做仪器"生效。
   本接续方**独立读到的、与 §2 相互佐证的**只有 §2.0 的三行 `S0-*`——本 §4 的 101/54 与 §2.0 的
   `S0-ins-pristine RUN=101 TOPPASS=54` 逐字同调，是**同一枚事实**，不是复跑变异。

4. **§4 的名册基线 100/53 是通过 `-count=1 -v -skip '^TestAC1AC2DispatchHopGate133$'` 复现的，
   不是通过 `git archive c720494` 那棵无仪器树复现的**。两条路径**未做同形名册比对**——
   `c720494..HEAD` 之间 `cmd/wisp/` 只有 `ed18727` 一枚提交动过它（`git log --oneline c720494..HEAD -- cmd/wisp/`
   与 `git diff --stat c720494..HEAD -- cmd/wisp/` 已在 §4.5 逐条列出，`ed18727` 里 main.go/panel_assets.go/
   slo_windows.go 那 19+8+6 行都是 `WISP-LEG-COVERAGE-RULING:` 注释、不新增用例），所以两条路径
   **应该**给同一份 53 名，但本接续方**没有真的去 `c720494` 快照上再跑一遍**（那要拷 DLL、耗时、
   且前任 §1 已经给过那棵树上的四数 200/106/0/0）。若验收方要卡这一节，走的是 `-skip` 这条、
   不是 `c720494` 归档这条。

5. **AC#3、AC#4、AC#6 三格本程未动**（票面明写本接续方只补三节）：
   - **AC#3** "自证腿不许是哑的"：需要**主动把 sanity 腿破坏一次**并证明它会响——前任未做，
     本接续方亦未做（且 09-23 17:3x 编排者已裁定这条同族洞 `R-131r3-1` **归票 135**，见票面 AC#3 追加段）。
   - **AC#4** "静默缩短要能响"：票面 AC#4 追加段（Y2/Y3 两形判据、双向差集、整表恒真必须红、
     严格前缀负断言）——本仪器 `leg_dispatch_gate_133_test.go` 有 `minLegs133 = 11` 下限断言，
     但**票面 AC#4 追加段列的具体判据（`refused := reason != ""` 那族靶）不是本票的地界**，
     本接续方**未做 AC#4 追加段任何一发的变异**。
   - **AC#5** "门禁"：本 §5 交了必需四件（gofmt / go vet / GOOS=linux go vet / d22scan），
     但**票面 AC#5 里那句"gofmt/**gofumpt** 真跑"没做到**——`gofumpt` 不在本机 PATH
     （`which gofumpt` → not found，硬禁令没让装第三方工具）；且 `sh scripts/d22scan.sh` 本程只跑了一次
     **没有历史 scope 数字可比"各 scope 不降"**（前任 §1 CI 同调给了基线"RUN=100 PASS=53"是**包**级不是
     **scope** 级；d22scan 各 scope 的历史读数不在本票证据里）。
   - **AC#6** "仪器自己进 CI，说不出'上次真跑过的 run id + job id + step 名'就当它不存在"：
     本接续方**没有远程门禁读数可引**——`09a5b6e` 那枚 commit 已登记"origin 连 3 次 TLS 握手失败未推上"，
     本程按 brief **只 commit 不 push**，因此 `leg_dispatch_gate_133_test.go` 至今**没有一次真远程 CI 跑**的
     run id + job id + step 名可交，AC#6 判定**未成立**、留给验收方（或 push 之后另派一次读数）。

6. **Linux `go test ./cmd/wisp/` 未复跑**。§5.4 只补了 `go vet`（编译／分析），
   `wisp-cli-tests.sh` 头部注释登记过 ubuntu 上"19 枚真二进制 FAIL、rc=1、127.4s"——那是 **RUN 时读数**，
   归 package owner 的 finding、**不属本票**（票 133 的 CI 门在 windows leg）。本接续方未复跑，
   也**不声称覆盖**；§5.4 的 linux vet rc=0 **不能替代** linux `go test` 的读数。

7. **§3 覆盖面主张里"三条形 + 三条反形"（清单式看不见的形状 vs 图式看不见的形状）未做变异实验**。
   那是前任在 §3 里**用文字论述**的、不是**用变异读数**证明的——本接续方按禁令未动 §3 任何一行；
   若验收方认为"论述不算证"，AC#2 那一格应视为**未做变异实验、只做了主张**（前任的自判，
   本接续方不加也不减）。

8. **AC#1 五发的复现不在本接续方工作树里做**：所有变异只在 `/tmp/wisp133c-*` 三棵快照树里发生
   （前任 §2 明写），本接续方工作树 `e113b1a` → `00d3f34` → `0de835a` → 本 commit 全程**只 append-only 改证据文件**、
   未动 `cmd/wisp/**` 任何一行；`git status --porcelain cmd/wisp/` 开工与提交前都空。

9. **未跑 `go test -count=2 -v ./...` 整仓**——§4 只在 `cmd/wisp/` 一个包上做四数账。
   `internal/**` 与 `cmd/llmrecord/` 等包的账归各自票，不在本票地界。

10. **票面 Progress log 的 append 只写本程（§4/§5/§6 三节补齐 + next= 非实现者验收方）**，
    不追加"AC#1 判定"、"AC#2 判定"两行（那是验收方的话头，不是接续方的）。
