...
- **尺寸**：配置范围 44–72px（默认 56）；`Sleeping` 态为**静态玻璃体**，直径 44px（INTERIM，见下）。
  - ⚠ **INTERIM 变更（2026-09-20，owner 授权继续推进核心功能）**：原规格「`Sleeping` 缩到 12px 微点
    ...
    现改为静态玻璃体 44px，实测 `px_delta_ge8=2120`、`timers=no`、`cpu=0.000`、私有工作集 11.76MB
    （**D32 的 Sleeping 零定时器纪律未放宽**；可见性来自静态球体本身，不来自动画）。
...
| 态 | 视觉 |
|---|---|
| `Sleeping` | 静态玻璃体，直径 44px，**零动画、零定时器**（INTERIM：原「12px 微点 + opacity 0.35」经差分取证否证，见 §2 尺寸条；质感返工归票 65） |
...
## 3. 状态机权威转移表（C12/D43——20 态 40 条转移；未列出的一律非法）
