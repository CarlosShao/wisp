...
- UI 代码里出现任何 emoji。**规格与仪器的射程不是一回事，别把下面两条当同一条**：
  - **规格要求**（`PLAN.md` 的 D29／零 emoji 规范与那张"绝对禁止"清单）：范围写作 `U+2190–U+2BFF`、`U+1F300–U+1FAFF`、`U+FE0F`。
  - **仪器实际扫**（`tools/d22scan/main.go` 的 `emojiRe`，2026-09-24 票 141 交付版）：
    `U+1F000–U+1FAFF` · `U+2200–U+22FF` · `U+2600–U+27BF` · `U+2B00–U+2BFF` · `U+FE0F` · `U+1F1E6–U+1F1FF`，
    且**注释豁免、字符串不豁免**（`Q-46(c)`）。差别是双向的：**箭头 `U+2190–U+21FF` 与带圈数字 `U+2460–U+24FF` 是刻意留的空隙、
    根本不扫**（由 `TestBan8MathBandAndRemainingGaps` 正反两向钉住），而起点 `1F000` 比文档写的 `1F300` 更宽。
    ⇒ 实际后果：**`→` 可以出现在界面文案里（仪器抓不到），`✓`(U+2713) 与 `≤`(U+2264) 不行。**
    这个"规格比仪器宽"的缺口未定案，出处＝票 141 的 (b) 支（改规格文字需人工批准）＋台账 `A201②`。
    **本文件这一段属"薄索引"对仪器的过期描述，2026-09-25 06:4x 由编排者收窄为上面这两行；不改 `PLAN.md` 一字。**
