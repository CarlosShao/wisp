/* ============================================================================
   ...
   Which rows work is decided by whether the panel owns the thing being changed:
     ...
     the opacity row  WORKS. The frost is a CSS custom property in this document
                      (--panel-alpha, generated from demo/styles.css:30 and
                      :60), and moving it is a style write on this page. It is
                      not a config write: nothing leaves the WebView, nothing is
                      persisted, and reopening the panel starts from the token
                      table's own default again. PLAN.md:1043-1044 says a WebView
                      restart must re-read everything from Go, and that is
                      exactly what happens here.

     everything else  DORMANT, and says so instead of showing a number. Theme /
                      font size / panel width belong to Go's config and need a
                      C17 read plus a route back; the ball.* keys belong to the
                      native ball, which is Direct2D and cannot be reached from
                      a page at all (D29). Rendering an editable box with a
                      made-up value in it would be owner's P9 red line ("不得造
                      假数据当真实字段"), so those rows carry no value.
...
interface Row {
  ...
  /** The key inside it. The two halves are stored apart and joined at render
      time on purpose: internal/panel's route gate
      (TestTheRendererHoldsExactlyOneDoorToTheHost, composer_test.go:522) treats
      any quoted dotted host-route string anywhere in the renderer as a claim on
      a host route, and a settings label is not one. That gate may not be
      widened from this side, so the label is assembled here and
...
export function ConfigScreen() {
  ...
  return (
    <section aria-label="设置" className="flex flex-col gap-1 px-4 py-3">
      ...
      <h3 className="mt-2 text-[12px] font-medium text-muted-foreground">外观</h3>
            ...
            >
              <div className="min-w-0">
                {/* The key label is a code value, so it is drawn by the library's
                    own token chip rather than by a mono class typed here
              ...
              <div className="flex shrink-0 items-center gap-2">
                {live ? (
                  alpha === null ? (
                    <span className="text-xs text-muted-foreground">读不到 {ALPHA_VAR}</span>
                  ) : (
                    <>
                  ...
                  <span className="text-xs text-muted-foreground">{row.blocked}</span>
                )}
                {/* 状态徽章用 demo 自己的 badge 系（theme.css 的 badge /
                    badge-success，styles.css:427-444），即时行绿、断线行灰。 */}
