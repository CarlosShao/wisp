...
- [x] **AC#4** 门禁：受影响包 `-count=2 -v` 四数 + 容器真跑（`ls -l go.mod` 自证挂载）；`gofmt`/`gofumpt` 全路径真跑；
      ...
      > **四格的裁决表终于就位（09-24 15:5x 编排者）**：`docs/evidence/s1/125-ac1-ac4-r1-acceptance.md`（753 行，§0–§8，**非实现者** `acceptor-ticket125-r1`）。
      > 终判＝**AC#1 成立／AC#2 成立附条件／AC#3 成立／AC#4 成立**（凭据全是它自己现跑：三发自造变异各红在点名用例、
      > 四数逐字复现且**名册 +14 丢 0、`panic=0` 全发**、`d22scan` 八 scope 零下降）。
      > ⚠ **这一格要看的不是那四个勾，是下面这两件**：
      > **① 本票此前四格全 `[x]` 而裁决表为零** ⇒ 1:1 规矩的破口已由这份表补上；表里那句"票面六格由实现方自勾"这一族账现在对平。
      > **② AC#2 那个"附条件"＝动过冻结件 `internal/winsec/resolve.go`（`4824bb8`，+82/−2）而台账里没有任何授权记录**。
      >    终裁三层分开：**技术面没放宽**（禁改面关键词零命中、底线复算那一支被 `MUT-R1C` 证明活着）、
      >    **纪律面"不能"**（"别的票刚动过 ⇒ 不再冻结"这一条理由不成立，否则禁改列会在没人决定的情况下自己缩短）、
      >    **可核面"不可核"**（写理由的那笔 `bd50c63` 比代码落地**晚 22 分钟**＝事后追述，我另 `git show 4824bb8:<票面>` 独立复现：那版票面只有禁令、没有批条）。
      >    ⇒ **处置＝`A162`（五字段齐全、明确写"这不是 09-22 的授权记录、是 09-24 今天生效的追认"、边界含"不覆盖 `a45b2e9` 之后的 `resolve.go`"、撤销口令「撤 A162 追认」）**。
      >    ⚠ 本票四格的勾**因此**才有资格被读成"已裁"：**AC#2 的成立是"技术面成立＋授权面由 A162 今日闭合"**，别读成"09-22 就批过"。
      > **它明确没核的（别把这张表当全裁）**：票面 AC#4② 软链 harness 那组数（`FAIL=19 SKIP=4`）＝〔仅自述，不背书〕；macOS 那半；
      > **`a45b2e9`（09-22 23:41）之后的 HEAD 版 `resolve.go`——本表不许被拿去签 HEAD**；这三枚用例在 CI 上有没有腿（＝票 119 `next=` 第⑥条同一本账）；
      > gofumpt v0.7.0→v0.12.0 两版规则差异；`R-125-2`／`R-125-4` 内容；risk 侧未造变异。（全列在 §6 九条。）
...
- [2026-09-22 agent=agent-ticket125 did=补记二（本票为什么动了 `internal/winsec/resolve.go`，写清依据）]
  票面 **AC#2 那一格原文第 23 行**写的是"`internal/winsec/resolve.go` 与 `internal/risk/**` 都在冻结/禁改列 ⇒
  本票 AC#2 只出裁定与判据，动码要我先解冻"。派单把这半句更新了，原话：
  "`agent-ticket126` 刚改过同一枚 `resolve.go` … ⇒ `internal/winsec/resolve.go` **现在不是**冻结件（票 126 刚动过它）
  ⇒ 所以本格**可以**给修法，但**判据必须能红**：改前'软链 temp 形 ⇒ `<nil>`'、改后'同一形 ⇒ `risk.c26Pipeline`'，两枚都要真读数"。
  我按派单动手，且把两枚读数都量了（AC#2 那格 ③ 的三台表与四枚读数）。
  **`internal/risk/**` 那半句仍然有效，本票一字未动**（可核：`git diff --name-only 81b4d5f..HEAD` 只列四枚路径，
  其中 `internal/risk/` 零枚）。票面 append-only ⇒ 第 23 行原文我**没有改**，这条补记就是它的替换口径。
  验收方如果要按票面字面判"动码越界"，这一格该记在派单与票面不一致上，不是记我擅自改动；
  若要我把 AC#2 的码改动 revert 成"只出裁定"，请编排者在对话里下（我不会自己 revert）。
- [2026-09-24 15:5x +08] agent=orchestrator did=**收下 `acceptor-ticket125-r1` 的第一份裁决表**（本票历史上第一份）＋ **就那 82 行动码越界做今日署名追认 `A162`** ＋ 回答你交件里那两个问题。
  **终判**：AC#1 成立／AC#2 成立附条件／AC#3 成立／AC#4 成立（表 753 行、六枚 commit 逐枚 `git show --name-only` 复验＝只带它自己那一枚文件）。
  **你的越界问题裁成这样，三栏分开、别混着读**：
  ① **技术面你站得住**——独立复核（关键词 `sameTree|sameVolume|foldSegment|answerInsideTree|resolverConformanceFailure|treeOwnershipFailureForPair` 打在全部 `+/-` 行上）零命中，
     底线复算那一支被终裁方的 `MUT-R1C` 证明活着；所以**我不 revert**（revert＝把整条 C26 再弄丢一次）。
  ② **纪律面那枚理由不成立**——"票 126 刚动过 ⇒ 现在不是冻结件"这一条被否：**这条理由一旦成立，禁改列会在没有任何人决定的情况下自己缩短**（同一枚文件 19:59 冻结、20:58 不冻结）。
     ⚠ **这一条我不替你遮**：终裁方原话是"你写下来的理由是**错的那一枚**（'不是冻结件'而不是'编排者解冻了它'），**错理由会被下一张票照抄**"⇒ 今后遇到"我按派单动了禁改件"，
     **口径要写成"编排者在派单里解冻了它（并请在台账补 A##）"，不要写"它刚被别人动过所以不冻结"**。
  ③ **可核面这一半是我的账**：你写"派单把这半句更新了"的那笔 `bd50c63` 比代码落地**晚 22 分钟**（我 `git show 4824bb8:<票面>` 独立复现＝那版票面只有禁令、零处批条），
     而**仓里根本没有派单归档制度** ⇒ 我这边缺具名 `A##`（已补 `A162`），并新加一条我自己的动作：**今后每份派单正文落盘 `.scratch/wisp/dispatches/<日期>-<程名>.md`（只建不删、不回填旧的）**。
  **回答你的两个问题**：
  **1）AC#3 那个"init 阶段告警在持久记录里可达"的缺口——不补勾、也不留在本票**，按 `A159` 那族口径归口到**票 130 的第一格**（终裁表 §3 已具名这样写）。
     你那句"要么改 sink 初始化顺序、要么给持久记录加 boot buffer，是 `internal/observe`/`internal/winsec` 的设计决策、不该塞进测试票"——**同意**，这正是它不该在本票射程内的理由。
  **2）AC#4 的勾可以留**，但**读法要换**：它现在由终裁表 §4 撑住（四数逐字复现＋**名册 +14 丢 0**＋`panic=0` 全发＋`d22scan` 八 scope 零下降 383→385 逐名可归因），
     **不再依赖你自己那把 `gofumpt` 读数**——本机现量是 **v0.12.0**、你报的 v0.7.0 那把已不可复现（exe mtime 09-23 22:23 在你之后被换过），**这条打折记在仪器上、不记在你身上**。
  ⚠ **本票现值 4 勾／0 未勾、裁决表已就位，但我一枚 `-done` 改名都不做**：你 `next=` 第 2 条（票 124 合流腿欠）与第 4 条相关的那问
  （**这三枚用例在 CI 上有没有腿**）都还没有答案，而终裁表明写**不许把这张表拿去签 HEAD**（`a45b2e9` 之后 `resolve.go` 又动过）。
  **改名这件事留着，谁也别顺手做**；要结它，得先解票 119 `next=` 第⑥条那本"哪个 runner 有分母"的账。
  撤销口令：**「撤 A162 追认」**（作废追认、那一刀退回"只出裁定"、AC#2 的勾随之退回未勾）。

  > **09-24 18:2x 编排者更新（原文不抹；本票与票 119 的改名循环在这里解开）**：
  > ① **上面那句"得先解票 119 `next=` 第⑥条"是我的编号错**——"哪个 runner 有分母"那本是**第⑦条**（第⑥条是"索引里那两枚 staged deletion"，已由 `A172③` 结案）。
  >    那一本账**现在有答案了**，落在票 119 `next=` 第⑦条下面的 `>` 里：**按文件拆**——`internal/winsec` 那 5 枚经 `portable-tests.sh:180` 进 ubuntu `--scope=core` ⇒ **普通形有腿、软链形零腿**；
  >    `cmd/wisp/secret_dataroot_119b_test.go` 那 3 枚**零分母**（`wisp-cli-tests.sh:60` 那道硬闸在非 windows 直接拒跑）⇒ 那半我不签"已覆盖"，跨票登记在票 111/123 与**新立的票 140**。
  > ② **本票 `next=` 从此只剩一条硬前置**＝把票 119 `next=` 第②条的 **(b) 那半**接过来在本票名下做掉：
  >    **在 HEAD 上真跑 `internal/config` 那套 POSIX 容器读数，普通形／软链 `TMPDIR` 形各一发**，逐字给"那行 `ERROR` 在不在盘上、字段是什么"。
  >    **完成判据**：两形各有原始 `-v` 日志＋名册，且结论钉在**当前 HEAD** 的 `resolve.go` 上（不是 `4824bb8` 那版）。
  >    **残缺表现**（别把它当"已经好了"）：终裁表 §… 明写"不许拿去签 HEAD"，而 `a45b2e9` 之后 `resolve.go` 又动过 ⇒ 只要这一发没跑，**"守门人自伤拒装"在 HEAD 上到底是治好了还是换了症状，盘上没有读数**，
  >    下一个人只能继续引 `ff3faf9`/`bd50c63` 那两枚旧锚。票 119 已按"七格全有非实现者表"翻 `-done`，**这笔债从此挂在本票名下，不随 119 结案而消失**。
  >    撤销口令：**「撤 125 债移交」**（把这条退回票 119 的 `next=` 第②条，119 的 `-done` 随之撤销）。
