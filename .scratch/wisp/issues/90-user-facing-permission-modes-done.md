...
**Status:** **accepted-done**（2026-09-21 17:3x 独立对抗验收：**六框无一 FAIL**；AC#4 那 5 轮"补作业"被验收代理 6 刀重跑**全部复现**，裁决表 `docs/evidence/s1/90-adversarial-acceptance.md`。它只把 `-done` 卡在一行文面上，现按 **R21③** 补上：**权限模式不改变 OS 能力**——"降权运行工具"属票 100（RESERVED），把"全自动"实现成任何形式的 OS 级降权/放宽都是假承诺。⚠ 验收同时判：**不存在让"会话授权"跨重启的路径**（这就是 R20/M3 那条边界的实测背书）。装配缺口（`internal/perm` 生产零 importer）另案**票 101**，已交付待验收。）
