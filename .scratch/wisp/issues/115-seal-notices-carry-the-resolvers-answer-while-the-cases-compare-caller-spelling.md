...
- 2026-09-21 21:5x（`agent-ticket115b`，**远程坐实那一格**）：编排者在这期间把 `527d303` push 了出去 => 我不必再等下一次 push，直接取到了含我那 6 行的 step4 读数。上面 ⑤.next=1 已被实测兑现。

  - run **35608530583**（`run_number=183`，head `a13c6dc`；`compare/527d303...a13c6dc` = `status=ahead ahead_by=6 behind_by=0` ⇒ **含 `527d303`**），job `106361546809`，**step 4 = failure**。正向形状四判据逐字：`HTTP/1.1 200 OK`；日志 403969 字节 / 3006 行；首行 `2026-09-21T13:53:48.8126112Z Current runner version: '2.337.0'`；第 187 行 `2026-09-21T13:54:36.1989540Z ##[group]Run bash scripts/winsec-tests.sh`。
  - 四数逐字（第 1464 行）：`winsec-tests.sh: four numbers (all from -v output): === RUN=82  --- PASS=40  --- FAIL=2  --- SKIP=0` ⇒ **FAIL 由 6 降到 2**。降掉的 4 枚逐字为 `--- PASS`：`TestSealReportsThePrincipalsItCleared`、`TestAC1SealFileReportsTheInheritedGrantItCleared`、`TestAC2InheritedNoticeHasANoiseBound`（连同 leg 1/leg 2/leg 3 三枚子项全 PASS）、`TestSealNarrowsAndNamesThePrincipalItRemovedBySID` ⇒ **停手条件② 在 runner 上兑现**（112 那条红转绿，且票 104 的 leg 2"整树传播只 1 条"上界不变）。
  - 剩下 2 枚 FAIL 正是 ③ 登记的 `R-115-2`：`TestNoticeAttributionSurvivesAn83AliasOfItsOwnTree`（红点仍 `notice_attribution_115_windows_test.go:185`，四种拼写各一次，`path="C:\Users\runneradmin\..."` vs `long spelling="C:\Users\RUNNER~1\..."`）与 `TestNoticeAttributionKeepsTwoTreesApart`（红点仍 `:231` 的裸 `sameTree`）。`the instrument planted nothing` 与 `not attributable through spelling` 全步仍各 **0 次**。
  - 第 204 行本轮又回到 resolver 那句：`2026-09-21T13:55:32.1440552Z 2026-09-21 13:55:24 INFO winsec: sealing path resolver installed resolver=risk.c26Pipeline probes_passed=2` ⇒ C26 仍装得上；180 那轮的"第 205 行"是脚本 ledger 说明的行数漂移，不是回退。
