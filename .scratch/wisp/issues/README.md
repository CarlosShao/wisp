...
8. **临时件（`/tmp` 下的快照、变异副本、日志）在任务运行期间一律"只建不删"**（2026-09-23 14:4x owner 批准，起因见 `A119`）：
   删除类命令（`rm` / `rmdir` / `Remove-Item`）每发一次就要弹一次授权窗给 owner 看，**而"少弹一次窗"和"少一次误删机会"是同一件事**。
   ⇒ 收尾时**不要清理自己的工作目录**，把清单（路径 + 体积 + 有没有被证据文件引用）写进交件即可；
   批量清理由编排者**一次做完**（一次授权，而不是一天几十次）。
   ⚠ **例外**：证据文件里被当作"可重跑凭据"引用的快照目录**不许任何人事先删**——
