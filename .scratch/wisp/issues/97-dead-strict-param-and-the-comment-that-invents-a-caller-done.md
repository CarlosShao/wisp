...
`docs/evidence/s1/97-adversarial-acceptance.md`（`000a2d2`，票面追加删除列 0）。五格全第一档：
`strict` 与 `resolveLocked` 各 0 命中、唯一调用点 `:345`/`:395` 它自己 grep；**M4 它重跑**（build rc=0 ⇒ `:85 SECURITY` 红、矩阵 5 条仍绿）；
"有没有第三条路让别名买到批准"逐处读码（`AnswerAllow` 产生点唯一 `queue.go:360`、`deliver` 不认名字、`len(set)==1` 在拒绝侧、
panel 无 Allow、`revokeGrants` 只读精确键、`Replay` 非答案）⇒ **无路，但靠审查不靠类型**。
⚠ 它把我票上的账纠了三条：**R-97-1** 注释里"only reader of `q.alias`"被 `:197`/`:210` 反证；
**R-97-2** 我 commit 标题写的"编译不过"被 M4 证伪（别名读注入是能编译的）；
**R-97-3** 票内"58+38+2=98"是**跨层相加**，正确分层是 60+38=98。三者都在裁决表，改注释该有自己的票。）
