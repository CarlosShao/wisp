...
- [x] Hard-coded read-only fields reject writes with explicit errors.
- [x] Round-trip: load → marshal → load yields identical structs (GUI double-source guard).
...
- note: AC#5 **PASS** — 见 docs/evidence/s1/05-adversarial-acceptance.md §"Addendum 裁决（2026-09-20，AC
      补裁）" AC#5 行：validate.go:68/78/82/92 四处只读拦截 + validate_test.go:129 双断言（必报错 + 点名
      key path），`go test ./internal/config/ -run TestValidateHardcodedReadOnly -count=2 -v` 2×PASS
- note: AC#6 **PASS** — 见同上报告 AC#6 行：boundary_test.go:188 TestRoundTripLoadMarshalLoad 对 17 段
      全非默认夹具做两次 DeepEqual（辅以 :216 TestRoundTripBytes），
      `go test ./internal/config/ -run 'TestRoundTripLoadMarshalLoad|TestRoundTripBytes' -count=2 -v` 4×PASS
...
- [2026-09-20T11:10Z] agent=ac-addendum-auditor did=补裁 AC#5/AC#6（两框原为空白裁决）：实跑 TestValidateHardcodedReadOnly + TestRoundTripLoadMarshalLoad/TestRoundTripBytes 均 -count=2 全绿，并读 validate.go:68-94 与 loader.go:62 确认"写入文件"路径确被 validate 拦截、错误串点名 key path；两框改勾。裁决写入 docs/evidence/s1/05-adversarial-acceptance.md §Addendum next=none
