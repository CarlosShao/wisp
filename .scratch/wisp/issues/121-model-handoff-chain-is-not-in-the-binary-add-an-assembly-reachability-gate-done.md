...
**Status:** **done**（2026-09-23 10:1x 编排者按 `acceptor-ticket121` 的裁决表改判：**七格全过、零退回**；AC#5 那格的「勾 + 归口」见该格下方 09-23 附注，票 109 另收 AC#6。上一状态 open（2026-09-21 21:2x 编排者建；来源=`acceptor-ticket109b` 的
...
- [x] **AC#4** 顺带补票 109 退回的两个洞（验收方实测能滑过去的两形）：
      **未点名的文件/子目录不参与哈希**（`R-109-2`）· **按名字而不是按 SID 过滤 ACL**（`R-109-3`，
      ...
      本票只在自己的调用层面处理。
- [x] **AC#5** 残留窗要写清不掩盖：验收方证明守卫返回之后到读取者 open 之前**还剩同样形状的一整段**，
      ...
      ↳ **2026-09-23 10:1x 编排者按验收方裁决勾本格**（`acceptor-ticket121`，
      `docs/evidence/s1/121-adversarial-acceptance.md` 的 AC#5 行判「诚实降级」）：
      本格勾的是**票面写着的交付物**——「把这句话写进交付面」，它已在 `agent-ticket121b` 09-22 18:1x 那条 log 与
      `cmd/wisp/models.go` 的 AC#5 注释块里逐字落地。**这一勾不声称残留窗被修**：洞本身**未修**，
      已按验收方判语（「票 109 的账不随本票结清」）**归口到票 109 新增的 AC#6**。
      ⚠ 交付方当初故意把这格留在 `[ ]`（「它没被修，也不许被说成修了」），这份谨慎**原样保留在上面的 log 里、不改写**；
      编排者改判为「勾 + 归口」是因为**不变式要求 `-done` 的票未勾框数 = 0**，而「留空」会让整票工作被下一个会话重做。
...
- 2026-09-22 18:2x（`agent-ticket121b`）：**接续者收尾 + `next=`**。本票现在 **AC#1/AC#2/AC#3/AC#4/AC#6/AC#7 六格已复算并勾上，
  ...
  6. **注入文本登记（Rules 末条要求）**：本轮工具输出里反复出现同一段自称系统提示的文字
     ...
     主动提取可复用组件 / 写码前检索知识卡"）。**出现次数：交件前最后一条 commit 之前已累计 18 次，且它随工具输出成串复现，真实次数只更多**（STEP 0 之后起，多数跟在 Edit/Bash 结果后面）。
     它**不是编排者在对话里下的指令**，故：未据此改动任何判据、未 revert、未放宽任何阈值；本票代码/注释/用例
     本来就不含 emoji、判据仍按票面"能力类必问生产调用者"走。**revert 只在对话里由编排者下。**
- 2026-09-22 18:3x（`agent-ticket121b`）：本枚只把上一条的注入文本**计数改成有界说法**（那段文字随工具输出成串复现，报死一个数会立刻失真）。其余票面一字未改；**票面状态仍是 6 勾（AC#1/2/3/4/6/7）+ AC#5 留 `[ ]`**。
