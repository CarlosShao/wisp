...
- 2026-09-21 15:4x（**编排者补：两个验收会话都跑完了，数字升级 + 一处引用要拧准**）：
  session A 另测了**修复前基线** `131f722` ⇒ Linux `--- FAIL` **31**（risk 13 / tools 18）+ `panic 15m0s`
  （两条各真等 300.02 / 300.28s）；只回退守卫一处 ⇒ 两包合计 **8 → 56** 条红，且**"基线红→变异绿"= 0 条**。
  ⚠ 我一路引用的"**47 条**"这个数字**两个会话都没复现过**（它来自 `24a66b6` 时代的一次 CI 日志读数）
  ⇒ 本地可重跑的前后对照是 **31 → 8**；今后引用 47 必须带"CI 日志读数、不可本地复现"这个标签。详见 **A59①**。
  另两条由它们独立佐证的事实：**600s 里有一段是无上界等待**（票 84），
  以及 `17efc2c` 上 gofumpt 唯一点名的文件**就是本票新建的那份测试**（违 A40⑤"同批该顺手格式化"，
  **不违本票 AC#5**——本票没弱化任何断言、没 tag 掉任何测试）。我发简报时那句"与票 75 无关"是错的，账按 A52② 为准。
