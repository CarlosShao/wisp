...
## Acceptance criteria
- [x] 改 `[hotkey]` 后热键真的重注册（端到端：改配置→按新键→球响应；旧键不再响应）。
      —— **2026-09-21 00:39 由编排者勾上（读测试体之后，不是看名字）**：
      `internal/ball/hotkey_live_test.go TestLiveHotkeyRebindEndToEnd` 五步都是断言而非日志——
      `:100` 改文件成新键、`:101-109` 轮询 poll tick 后断 `len(live)==4 && live[hkSummon].VK=='R'`、
      `:110-112` 断 `Rebinds()==1`（**一次编辑只能重绑一次**，这条顺手钉住了"不重复注册"）、
      `:126` 投递 `WM_HOTKEY` 到活 id 断回调计数、`:132` **物理注入新键** `Ctrl+Alt+R` 必须唤起、
      `:139-146` **物理注入旧键 `Ctrl+Alt+Q` 后计数必须仍是 2**，即"旧键不再响应"是**真断言**不是注释。
      ⚠ 那两条 `t.Skipf("SKIP-LOUD…")` 只在 `keybd_event` 注入进不了输入流时才走；
      本次两遍 winlive（08:29 代理跑 + 08:33 我跑）日志里字符串 `SKIP` **出现 0 次**
      ⇒ 注入真的发生了，新/旧两键那两步都是**执行过**的。旁证：A16 变异（对调 `hkNames` 前两项）
      把这条测试**打红**（`docs/evidence/s1/64-winline-retest-2026-09-21/a16-mutation-red.log`），
      说明它确实在跑真实 Win32 注册表而不是自证。逐跑记录同目录 `RESULTS.md`。
...
- [x] 默认唤起键为 `Ctrl+Alt+Q`，且 `Ctrl+Alt+Space` 作为可配置备选写进文档。
      —— **2026-09-21 00:42 落文档半**：新建非冻结说明文档 **`docs/HOTKEYS.md`**
      （四个默认键表 + 为什么不是 `Ctrl+Alt+W`（R10：84 组合实扫，`W` 是 owner 机上唯一被占用者）+
      `Ctrl+Alt+Space` 受支持但**故意不当默认**（搜狗/微信式输入法会抢在应用前吃掉它）+
      改绑定的热加载行为 + **五态**失败语义）。
      ⚠ 写这份文档时我自己纠了一处：**初稿只列了四态，漏了 `HotkeyError`**
      （"尝试过但 Win32 因其它原因拒绝"）——回代码数了 `:225-236` 才补齐。
      契约层（`PLAN.md`/`SPEC-03 §4`）**一字未动**，本文件只做说明并声明"以代码为准"。
      GUI 侧的可配置性仍按 MINOR-3 的裁定走**票 39 双轨**，不在本框范围内。
...
- [2026-09-20T14:26Z] agent=orchestrator did=**自我更正 A16：上一条里"全仓无一用例钉住 hkNames 配对"这句撤回**。
  我只凭 `grep hkNames --include=*_test.go` 零命中就下结论，**没读测试体**。逐条读完 `hotkey_status_test.go`
  的六个两两对调后：涉及 mute/panel 的换序被 `:98` `IsLive` 与 `:103`/`:106`/`:111` 状态断言抓住
  （四槽状态恰好各不相同 Live/Taken/Live/Unparsable）；summon↔mute 被 `:120-126`（只配 Summon 却断言其余三槽全不 live）抓住；
  **summon↔cancel 这条唯一"同为 Live"的边被 `:225`/`:241` 的 `Binding(hkSummon).Acc.VK=='R'/'Q'` 抓住**——
  那就是配对断言本身。⇒ A16 严重度从"静默错位的潜在缺陷"**下调为「保护是副作用而非意图」**，
...
- [2026-09-21T00:42Z] agent=orchestrator did=**A16 变异检验已做（结果 6 红，见 registry A16 的 ✅ 追加块）+ 勾 `:44` + 落 `:48` 的文档半（新建 `docs/HOTKEYS.md`）**。
  ⚠ **纠我自己一条数字**：上一条 log 的 commit message 里我写"本票现在 6 勾 / 2 未勾"，
  实测是 **8 勾 / 1 未勾（共 9 框）**——我少数了一框。今天这类"顺手估的计数"已经错过好几次，
  所以这次的数字来自 `grep -c "^- \[x\]"` 与 `grep -c "^- \[ \]"` 两条命令，不是我心算。
  **唯一剩下的未勾框是 `:62` 多显示器真拖**：本机没有第二块屏，按票面规则保持未勾并写明所需硬件
  ⇒ 本票**不能**置 done，`Status` 继续留 `review`，等一次有副屏的桌面批次（与票 68 AC#3、票 07 `:60` 同批最省）。
