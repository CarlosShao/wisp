...
- [x] **AC#2** 裁 ①/②/③ 并写理由；若选 ② ⇒ 顺带答一句"`WISP_ENV=test` 的数据根该不该由调用方解析成实路径"，
      ...
      > **AC#1／AC#2 复判回执（09-24 17:2x 编排者翻勾，撤销口令「撤 119 AC#1 AC#2 复判」）**：
      > 第一轮的表 `119-adversarial-acceptance.md` 判的是 **AC#2 退回／AC#1 附条件**，而**票面这两个 `[x]` 是实现方自己勾的** ⇒ 上面那句"勾"从头到尾没有裁决表背书，
      > 这一句就是把这段账拆开写清楚：**"第二程复判＝成立"，不是"表与勾一直一致"**。
      > **AC#1** 只挂一枚表：`docs/evidence/s1/119-ac1-ac2-r2-acceptance.md`（527 行，八枚 `6e04d1a 9b29951 fc48fc0 97a4e41 0dec286 53acca9 85a035f 3f56822`，§4 判"成立（无附条件）"）＝**单证**。
      > **AC#2** 主表同上；第二见证 `docs/evidence/s1/119-ac2-r2b-acceptance.md`（529 行，七枚 `20f8c22 fc71fa6 4757198 024ee87 85c1393 4e5d717 32bfb99`，§4 判"成立（无附条件）"）——
      > ⚠ **见证不抵账、也不叠加成"更可信"**；两枚表各裁各的、时间交错但互不相干（我 16:3x 曾把它们并成"同一程第二次交件"，`A170③` 已就地更正）。
      > 两枚硬条件（② 只做了一半／注释把一半写成全部）都由复判方**自造读数**闭合 ⇒ 按"造没造出来"的分界走，不给"附条件"章。
...
- [x] **AC#6** 门禁：容器内 `-count=2 -v` 相关包四数逐条点名（`-count=2` 不缓存；`=== RUN` 行数 == 不同测试名 × 2；
      ...
      > **⚠ 这一格里"本机 v0.7.0"已过期（09-24 17:2x 编排者现量，原文不抹）**：盘上是 **`v0.12.0 (go1.27.1)`**，
      > 路径 `D:\work\base\gopath\bin\gofumpt.exe`，文件 mtime **09-23 22:23**（`--version` rc=0）。
      > ⇒ 引这一格做门禁的程**以盘上为准**；这一枚影响的是"格式门是不是同一把尺"，不影响已交的读数（`gofumpt -l` 两版都跑过、都空）。账在 `A174`。
      >
      > **⚠ 上面那句括号"两版都跑过、都空"是我没验的断言，现在作废**（09-24 18:1x 编排者自纠，原文不抹）：
      > 今天**只有 `v0.12.0` 那把尺真跑过**（实现方一次、终裁程一次，都空输出）；**`v0.7.0` 今天没有任何人跑过**（容器里那把它自己写"跑不了"并引了错误原文）。
      > ⇒ 终裁程 §5.4#3 还给我补了一条我没想到的：票面 `:199`、`:382` 那两轮的格式读数是 **`v0.7.0` 那把尺量的** ⇒ **"历史门禁与今天同尺"这句不成立**，别拿它当"这把尺一直没变过"；
      > 反过来**本格这一格与终裁程是同一把尺（`v0.12.0`）**，这一句成立。**要哪一版的历史读数重算，谁下次真跑谁补，不许靠推断盖过去。**

- [x] **AC#7**（09-24 15:5x 编排者追加，来源＝`worker-ticket137-ac4` 交件里点名要我另开一格的那条 finding，严重度＝中；**09-24 18:1x 依非实现者终裁表翻勾，回执见本格下方 `>` 块**）
      本票文件 `internal/winsec/dataroot_symlink_119_other_test.go` 里 **`:203` 与 `:308` 两处内联断言只判 sentinel、不判记名**
      （`if err == nil` ＋ `errors.Is(err, winsec.ErrUnresolvedPath)`）⇒ **两枚母项在软链形里零区分力**：
      实现方现量（八发 `-count=1 -v`，容器 `go1.27.1 linux/amd64`，见 `docs/evidence/s1/137-ac4-impl.md` §3.2/§3.3）＝
      **普通·MUT-D 那两枚 FAIL、软链·MUT-D 那两枚 PASS 且都在 `RUN` 名册里**，而 `r4`/`r8` 的日志里它们自己把拒因打印成**宿主的链接**。
      ⚠ **为什么现在才成为一笔债**（这条是 AC#4 的代价账落到我头上）：票 137 AC#4 把 108/113 那 7 处根换成已解析形之后，
      **"未解析那一形"只剩这两枚持有**，而它们**能打印、不能区分** ⇒ 这一格不接住，那条形就从"有腿"退化成"有日志"。
      **结案判据（可重算，四条一条不许省）**：
      ① **两味药必须一起下**：按票 137 AC#2 的形状给这两枚接**记名断言**（同包 helper `refusalCreditsLink137`，**零新增依赖**），
         **并且同时**把这两枚的 `base` 换成已解析根 ⇒ **只收紧不换根会在软链形恒红**（那正是 137 的 `r2` 处境，别复活它）；
      ② **改前读数必须先量再说**：今天这两枚在软链形 PASS（＝零区分力）这一发要自己复现出来当起点，不许只引我上面那句话；
      ③ **改后的判别对**：同一棵树、同形状，**未变异 ⇒ 两枚都绿**；**MUT-D ⇒ 两枚都红且红句点到"我自己种的那枚链接"**（红句原文入库），
         两形（普通／软链）都要给，且四数之外给**名册差集**与 `panic` 计数；
      ④ **反半边不许被这一格弄绿**（本票 AC#3 那条"穿过链接把密封带到另一棵树 ⇒ 仍然拒"）**：AC#3 那枚用例
         `TestAC3POSIXLinkInsideAResolvedDataRootStillRefused119` 两形颜色逐名不许变；
         ⚠ **`:251` 那一处不并入本格**（它两形都响、在 137 AC#4 里被当对照组用，动它＝拆别人的对照）。
      **禁改**：`internal/winsec` 生产码（要动先报回等人批）、任何阈值／golden；**不许把 FAIL 改成 SKIP**、不许放宽断言换绿。
      **裁决表**＝`docs/evidence/s1/119-ac7-*.md`，**由非实现者出**（本票已有 `119-adversarial-acceptance.md` 那张 r1 表，
      它判 **AC#2 退回／AC#1 附条件**、那两格的复判是另一程的活，**本格不复用那张表、也不给它翻勾**）。
      **撤销口令**：回「撤 119 AC#7」⇒ 我把这一格退回成 137 证据 §3 里的一条报名（票面这一格删掉，报名与出处不动）。

      > **本格现状（09-24 17:2x 编排者记，勾仍不打）**：实现侧已交——`ddc1583`＋`c94927d`（合计 **62 增 7 删**，只动 `internal/winsec/dataroot_symlink_119_other_test.go`）、
      > 实现侧证据 `docs/evidence/s1/119-ac7-impl.md`（469 行）、票面进度 `2956897`（53/0）。它**没有自勾、也没有改本票名**（做对了）。
      > ⚠ **上面判据里那几个行号是"改前号"**：盘上现量记名断言在 **`:243`／`:363`**、两处换根在 **`:225`／`:337`**、`:251` 那处现在在 **`:291`**；
      > 而实现方报告写的是 `:241`／`:361`，**与盘上差 2 行** ⇒ 这一枚交给终裁程自己判"它引的是中间态还是数错了"（我复量的是盘上，不是它的表）。
      > 我自己独立复算到的一条：`AC#3` 那枚函数体在 `a9c8b6e` 与 HEAD **两版逐字节相同**（45 行／1934 字节／md5 前缀 `831a5c08`），且 `:251` 那行不在本次任何 hunk 内 ⇒ **判据④"反半边不许被弄绿"在码面这一层成立**（颜色那一层等终裁程自己跑）。
      > **还差**：`docs/evidence/s1/119-ac7-r1-acceptance.md`（非实现者，含它自造的四发单点回退进攻）——那一格判语回来我才翻勾；`-done` 仍不动（本票另有未结的 `next=`）。
      >
      > **终裁回执（09-24 18:1x 编排者翻勾，撤销口令「撤 119 AC#7 勾」）**：判 **成立（无附条件）**，出自非实现者
      > `docs/evidence/s1/119-ac7-r1-acceptance.md`（**748 行／§0–§8.5／八枚 `27a6f90 62088ce d0f97d3 67704c7 33af710 fa4ee80 4e1e26b 91be4ae`**，
      > 逐枚 `--name-only` 只带那一枚文件；我另核 `wc -l`＝748 与逐枚 numstat）。四条判据它**全部自己重跑**（改前八格它用**另一套链接名** `/acwlink` 量出同一组数＝不是抄实现方日志）；
      > 判据④它把 `AC#3` 函数体量到**四版同 md5 `831a5c08`**（我此前独立复算过其中两版）；**翻转并集**它按实数报 **17 对里 8 对有翻转、并集恰好那两枚母项、集合闭合无第三枚**。
      > ⚠ **它推翻了我简报里两处前提，两处都成立、都记在我身上**（上面 `:96` 那句错判原文不抹）：
      > **(a)** "实现方 `:241`／`:361` 与盘上差 2 行"**是我读错了**——那两个号标的本来就是**哨兵那一支**（它原文括号写着"原 `:203` 那一处，条件未动"），记名那一支它写的是 `:243`／`:363`，**七个号与盘上一字不差**；
      > **(b)** 我写的"单点回退 B 预期 MUT-D 打不红"**实测不成立**（摘掉记名那一支后 MUT-D 在两形照旧把两枚母项打红、名册差集 0 行）⇒ **若我照自己的预期硬判，就会错退实现方那一格**。
      > ⇒ 根因是同一个：**我把别人在某一版树上量的行号，拿去和 HEAD 比**——而"引别程读数要带它是在哪一版量的"这条规矩本来就写在我自己的记忆里，这次中在我自己手上（账在 `A176`）。
      > **那一问的答案**（本格最值钱的一条）：**"两发变异各自能响一支"＝承重，成立**；承重的可操作定义是它给的这句——**"摘掉任意一味，都存在一发变异从此打不红"**（换根对应 MUT-D＋软链形恒红被正面排除、记名对应 MUT-D2），
      > 而"要求同一发里记名支先响"等于要求一支在**它自己的前提被变异抹掉时**仍然响，只能靠把 `else if` 拆成两条 `if` 造出来＝**措辞工程**，不是加固。
...
- 2026-09-22 17:2x（agent-ticket119b，**返修第三、四件 + 门禁复算：`R-119-2`/`R-119-10` 注释收窄、`R-119-3` 裁定、票 113 那 16 枚复算**）：
  ...
  **`git diff --cached --name-only` 里从头到尾都带着两枚不是我下的件**：
  ...
  2. `R-119-5`（POSIX 上"C26 解析器在不在位"零用例 + 软链 temp 下守门人自伤拒装）**今天仍在**：
     ...
     > **编排者裁定（09-24 17:2x，账在 `A172`，原文不抹）：这一条"仍在"是一句问错了版本的描述，现在按两半分账——(a) 已结、(b) 半开。**
     > **(a) "POSIX 上解析器在不在位零用例"这一半已经不在**：钉子 `internal/config/c26_seam_posix_125_test.go` 在 HEAD、
     > 与票 125 被验版 `4824bb8` **逐字节相同**（`git diff --numstat 4824bb8..HEAD -- <该文件>` 为空，我自己复量），终裁表判它 AC#1 成立。
     > **(b) "守门人自伤拒装"这一半我不签"已结"**：药确实在 HEAD 码面上（`resolve.go` 里探针根先过 `resolveProbeRoot(os.TempDir())`、失败方向＝退回原拼写），
     > 但"软链 temp 那一形不再打 ERROR"的**最强读数锚在 `ff3faf9`/`bd50c63`（码面＝`4824bb8`）**，而票 125 那张终裁表**具名写着"不许拿去签 HEAD"** ⇒ **HEAD 级那一发今天没有任何人量过**。
     > **可复算触发条件**：下一枚在 **HEAD** 上真跑 `internal/config` 那套 POSIX 容器读数的程（候选＝票 125 续程或全树终判据复算那一格），
     > **普通形／软链 temp 形各一发**，两形都要交出"盘上那行 JSONL/ERROR 在不在、字段是什么"的原文 ⇒ 在那之前这一半记**半开**，本票**不因它翻 `-done`**。
  ...
  5. `R-119-4`（② 换出来的落点归属面）已由本轮写进 `winsec_other.go` 的第二条成本；**要不要并案到票 120 / `R-108-2` 那本账，请编排者裁**。
     > **编排者裁定（09-24 17:2x，账在 `A172`）：两支都不并——就地自立一条可检索的账。**
     > 不并 `R-108-2`：那条（`A84②`，台账 `:1218`）我当初裁的是**"不改"**（底线不管这棵树归谁），而 `R-119-4` 是 `AC#2` 选 ② 换根之后**在那条边界内侧新买回的后果**（落点归环境所有者）；
     > 两者是同一枚事实的正反面，**并案会让"后果"这一面从账上消失**（当时台账里只有两处转述、没有独立行——`grep -n R-119-4` 现量即证）。
     > 不并票 120：那是 check-then-act 竞态，另一枚东西，且它今天 open／五格零勾／零裁决表 ⇒ 挂上去＝挂给没人开工的票。
     > `r2b` 表 `:330` 那句"未裁前别记成已并案"由本条结掉：**裁了，结论是不并**；完成判据与残缺表现都写在那条自立账里。
  6. 索引里那两枚不是我下的 staged deletion，请编排者裁是谁的字（见上）。
     > **编排者裁定（09-24 17:2x，账在 `A172③`）：字是我的，而且 09-22 就已自首收编——这一条是一个已经被答完的问题。**
     > 形状：09-22 我用 `git mv` 把 105／116 两枚票面改成 `-done`，`git mv` 把改名记成"一加一删"两枚索引条目，而我 commit 时**只列了新名**
     > ⇒ 那两枚删除停在索引里、HEAD 里**新旧两名并存**（而 `-done` 后缀是本仓防重领的唯一键）；发现者按规矩停手没替我提交、只回报"请裁是谁的字"。
     > 收编在 `fbbecaa`（09-22 17:25），其 message 正文与这一条逐字对得上。**教训我记在台账**：这类自纠当时就该落一行账，否则它两天后还会被当成未决问题问一遍。
  7. 本轮三枚新用例在 CI 上**没有腿**（ubuntu 跑 `--scope=core`，`./cmd/wisp/` 在 `--scope=cli`，CLI 那一步只有 windows；
     ...
     > **编排者裁定（09-24 17:2x，账在 `A174`）：按文件拆开裁，两句都要说清"哪半有门、哪半没有"。**
     > **`internal/winsec` 那 5 枚**（`:127/:153/:193/:219/:285`，我先前被票面那句带成"三枚"）经 `scripts/portable-tests.sh:180` 进 ubuntu 的 `--scope=core` 名单
     > ⇒ **普通形有 CI 腿、软链形零腿**（那台 ubuntu 的 `TMPDIR` 本身就是真目录，造不出这个形状）⇒ 这一半我**认"容器读数即本轮判据"，但只在软链形这一半认**，普通形以 CI 为准。
     > **`cmd/wisp/secret_dataroot_119b_test.go` 那 3 枚零分母**（只有 `scripts/wisp-cli-tests.sh:113` 跑，而 `:60` 那道硬闸在非 windows 直接拒跑；决定性一发：`GOOS=windows go list` 里 `119b` 命中 0、linux 命中 1）
     > ⇒ 这一半**我不签"已覆盖"**：给 ubuntu 加一条 cli-scope 腿是票 111/123 的地界、且要先解 `R-119-7` 那 19 枚 DPAPI 红 ⇒ 它作为**跨票依赖**登记在那里，本票**不许拿"容器跑过了"替它背书**。
     > ⚠ 这一条同时纠正我 `A165` 那句过粗的表述（我写成"那三枚用例有普通形分母"，真值是**分文件**两样）；**本票 `-done` 的前置**是 `AC#7` 那张非实现者表结掉＋上面两半各自记账，不是"这条没门"。
