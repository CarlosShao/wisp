...
**Status:** **rejected-needs-fix**（2026-09-21 20:5x 编排者按 `acceptor-ticket105b` 的裁决表改判：
              AC#1/AC#2/AC#4/AC#5 通过，**AC#3 不通过** ⇒ 按票 108 先例不写"通过附条件"，退回单＝**票 116**（只加测试，禁改判定本体）。
              被退回的只有 AC#3 那一格：实现方交件里那句"只删 `ares.Actable()` 在真机上触发不了"被验收方用反例驳回，
              且变异读数显示**票面交的两条用例 2/2 仍然 PASS**（那条腿被删掉不会红）。
              AC#2 的接线本体维持通过：装配根 `run.go:265→335/337/353 → agent/loop.go:727 → bridge.go:243→:271→:756/:770 book()→:864`，生产调用者非零。
              一条如实收窄：`rt.auditf` 其实是 `run.go:428 fmt.Fprintf(rt.stderr,…)` ⇒ 生产里这本账**只到 stderr**，
              "盘上"是测试自己的 sink ⇒ 登 `R-105-1`，与票面"MODE-READ 同族"一致故不判 FAIL，但我不能对 owner 说"改写账已经落盘"。
              —— 原 `ready-for-review`（2026-09-21 19:5x agent-ticket105 交件，date 读数 19:55 CST；建票原文保留：
