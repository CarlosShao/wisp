...
**Status:** **done**（2026-09-23 18:2x 编排者按 `acceptor-ticket134-r2` 的总判结案：**六格 AC 全 PASS、无退回项**；
              该程残留六条 `R-134-3`/`-4`/`-5`/`-6`/`-7`/`-8` **全部挪到票 136**（其中 `-6`＝零样本 fail-closed 无钉，立成硬格），
              `R-134-9` 是**口径登记不是缺陷**，留在证据 §8.2；撤销口令「slo-full 恢复判红」只回退前半、三枚新鲜度钉不许跟着撤。
              ⚠ 一桩自纠也入档：`0a6554d` 把票 124 的两枚在飞暂存件带进了本票验收方的 commit（内容无损、双方各自登记过），
              由此把规则收紧成"**commit 必须带 pathspec ＋ 暂存清单里有别人的路径就停手报回**"。
              ── 以下为原状态原文，一字不抹 ──
              ready-for-review（**2026-09-23 10:5x：`agent-ticket134` 五格 AC 全落，AC#5 的 `shellcheck` 一格除外；
