...
## AC#4 那一格正式翻格（编排者 09-23 21:4x，依据＝票 133 的独立验收方，不是实现方自述）

**条件满足**：非实现者 `acceptor-ticket133-r1` 在锚定 `c5f140c` 的**纯净树**上（`git archive`，测量后与归档逐字节同）
**独立复现**了我 17:3x 定下的三件事——五发（N-3／X4／X8／X12／X14）**逐发转红**、
红名点到的是票 133 那把尺本人（③栏机检：131 的门名提及 **0**、`--- SKIP` **0**、本尺 `=== RUN` 恰 1 且 `--- FAIL` 恰 1、无 build failed）、
**X14 那一形在无仪器的树上确实零红**（`rc=0 100/53/0/0`）＝这把尺独占目击，且真二进制两拍都落得出 `logs/wisp-<day>-<seq>.jsonl`。
裁决表：`docs/evidence/s1/133-adversarial-acceptance.md`（400 行／20 节）。

⚠ **交接时同时登记的两条，不许被这一格勾盖过去**：
① **接手看守的那把尺比它要取代的这扇门更松**——验收方实测（**造出来的，不是读码猜的**）：
   往 `_test.go` 里种一枚 **Go 永不运行的方法** `func (phantomRecv133) TestSfx131LegIsDriven(){ _ = cmdSfx131(nil) }` ＋一行 usage，
   133 的尺就把第五发第二拍**判绿**（账本印 `covered=test TestSfx131LegIsDriven`），而那枚"用例"整包里既没有 `=== RUN` 也没有 `--- PASS`；
   同一发在 131 这扇门上过不去（`:617` 明查 `fd.Recv == nil && HasPrefix("Test")`，钉表存的是**函数值**）。登记为 `R-133-1`（高），
   **修它的格子在票 133 的 AC#2**（那一格已被同一位验收方判**退回**），不回头阻塞本格。
② 本票门头自陈的残窗三支里，**"跨包那一支"至今无证人**（验收方未造，出 `cmd/wisp/**` 地界）、**"方法值那一支"仍未测**——
   这两条**不因本格翻勾而作废**。
