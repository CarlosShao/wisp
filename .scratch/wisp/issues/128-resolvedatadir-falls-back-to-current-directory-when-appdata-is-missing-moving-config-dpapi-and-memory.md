...
**Status:** in progress（2026-09-22 16:41 编排者建；来源 `acceptor-ticket117` 的 `R-117-B`。AC#1 量毕；**AC#2 落地 + AC#3 变异自证已完成**（`agent=T128-ac23`，报告 `docs/evidence/s1/128-ac2-refusal-and-ac3-mutation.md`）；**AC#4 门禁未做**：`sh scripts/d22scan.sh` 纯净快照、台账各 scope 不降、票 123 CLI 用例未被放宽三格一律未量）
