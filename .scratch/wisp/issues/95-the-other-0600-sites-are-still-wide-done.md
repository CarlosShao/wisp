...
→ **accepted-done**（2026-09-21 18:5x 由 `acceptor-ticket95` 判：**AC#2–#5 通过、AC#1 通过但有条件**，
裁决表 `docs/evidence/s1/95-adversarial-acceptance.md`（`c2d2111`，票面追加删除列 0）。
它**没有照抄**我的读数：`icacls` 前后自己量（`0o600` 落的是六条含 `Users:(I)(RX)` + 外来 `(I)(M,DC)`；
`PrivateFile` 后只剩 SYSTEM/Admin/当前用户 `(F)`、无继承），并把我提的"封了会不会挡住自己读"**三条路全实测为读得到**
（另进程 `type` / `SealFile` + 描述符写 + `rename` 重开）⇒ **我给 owner 的那句"封日志你自己就 tail 不到了"被证伪**（台账 A80②）。
它还**补做了一枚我漏要的变异**（M2 单独退回 `parse.go` 那条腿 ⇒ `SaveFile` 也红）⇒ 两条腿各有锚，
"另一条腿仍绿"是**分层正确**而非"从没覆盖"。models 那组"计数不一致"裁为**算术误读**（52+4=56 自洽，真坑是 top-level 与含子用例两种 grep 口径）。
⚠ 三条**新账转票 109**（`AC95-R1` 交还后的跨账户写窗 / `R3` 宽 temp+rename 无 seam 守卫 / `R4` 安装目录无钉子）；
`⑤日志` 交回 owner 裁（代价已换成实测值）、`⑥球位置`/`⑦doctor-SLO` 转各包主人。**本票未推翻票 95 的七类判定**。）
...
- [x] **AC#1** 上面七类**逐类裁定**"算不算私有数据"，每类给：里面**实际会出现什么**（去读一处真实写入的格式，
