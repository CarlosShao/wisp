...
**Status:** **accepted-done**（2026-09-21 15:3x 独立对抗验收判 **五框全 PASS**，裁决表
`docs/evidence/s1/87-adversarial-acceptance.md`；AC#1-AC#5 数字由 `acceptor-ticket87` 在仓外快照
`git archive 1068eb9` 里逐条复跑。⚠ **本票交付的是"库层可用"，不是"用户已经能提前拒"**：
`Gate.Veto` 在全仓**非测试代码里调用命中 0 处** ⇒ **用户可见效果由票 37（审批 UI/L2 卡）与票 35
（PanelBridge：correlationId 路由）接**，三条残留（R-1 死参 `strict` / R-2 宽松解析的影响面是整个拒绝族 /
R-3 入键撞车会拒到别人的卡）已落成 **票 97**，别在本票里找它们。
⚠ **更正验收代理的一处引用**：它写的"票 37/41"里 **41 是误引**——票 41 是 **KWS 唤醒词 + 语音 veto 词**
（同名不同物），与本票的审批 `Veto` 无关。正确的接线归口是 **37 + 35**。）
