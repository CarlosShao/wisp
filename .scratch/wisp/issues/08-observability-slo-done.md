...
- [2026-09-20T15:17Z] agent=agent-ticket66b did=retroactive-instrument-note（票 66 AC#5 的追溯；**只补事实，不改本票任何 AC 勾选与 Status 字段**——那是编排者的权）：本票归档的 `build/slo/slo-report.json`（`generated_at 2026-09-19T23:35:49Z`，subset=smoke）逐字段复核 = `Sleeping exit_code=2 / report=null`、`Warm exit_code=2 / report=null`、`settle exit_code=1 / pass=false`、`all_pass=false` ⇒ **那次「state 口径」的九项门一门都没有产出样本窗**：红的原因不是资源超预算，而是判据仪器自身坏了（registry **A14**：`internal/proc` 的 `parseSystemProcesses` 丢系统快照链末项 → 采样进程在快照里看不见自己 → 60 次重试后 fail-closed `system snapshot does not contain the sampling process`；同批另立 **A15**：树内口径的 CPU 门被观测者自身 `ReadTree` 成本顶穿，判「仪器未定义」）。修复归票 66：`86e868d`（解析末项 + 回归用例）+ `00bbb76`（CPU 行改**树外**读取，D32 的 0.5% 阈值一字未动）+ 本次 AC#4 挖出的 `scripts/slo-check.ps1` 聚合崩溃（**全过路径**上 `.Count` 在 Set-StrictMode 2.0 下抛 PropertyNotFoundException，报告文件根本没写出来 ⇒ CI 连 artifact 都没有；A14 修好后才暴露）。重跑数字与逐字命令见 `docs/SLO.md` 附录 C，原始 JSON 归档 `docs/evidence/s1/66/`。next=none
