...
> **⚠ 编排者更正（2026-09-21 21:0x，来源=`acceptor-ticket104` 的 `R-104-2`；票 104 的 AC#5 就卡在这一句）**
> 上面那句**今天已经是对行为的错描述**，原文一字不删（已推送的历史与已结案票面都不改写）：
> `4d43447`（票 104）之后，`SealFile` 清掉**继承来的**带外授权时**也会出声**——通知面从只看显式 ACE 扩成
> `Principals` / `Inherited` 两桶，日志带 `kind=inherited|explicit|explicit+inherited`。
> ⇒ 那句的**判断前提**（全报＝把本包存在的理由也报一遍）仍然成立，但**实现事实已经不成立**：
> 继承桶报的仍然只是**带外主体**被清除，白名单（本机 SID 集合）一个字都没开口 ⇒ 噪声并没有变成「全报」。
> 验收方实测的噪声上界支持这一点：**每对象 ≤1 条、重复封同一对象 0 条**
> （18 次重复密封＝3 条不涨；12 个孩子＝12 条；`PrivateDirAll` 三档 1/0/0）。
> ⚠ 一句仍然有效的射程：`beforeErr` 取不到与 NULL DACL 那两格**票 89 就已存在**，票 104 没有改变它们（`R-104-4`），
> 结案语要把这一格写成**射程**而不是洞，免得它变成第三次「残留又没人管」。
