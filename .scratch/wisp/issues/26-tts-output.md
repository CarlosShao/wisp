...
- Half-duplex (D16): during playback, mic capture closed AND KWS inference suspended (except
  veto-word channel rule in 41); playback end → Speaking→Warm transition (mic stays closed,
  default policy).
...
## Out of scope
- Result routing tiers (30); Conversation mode (28); punctuation (27); barge-in AEC (deferred).

## Acceptance criteria
- [ ] Speak path: short result → TTS audible, ball Speaking with success breathing; interrupt
      mid-playback → instant stop + Listening.
- [ ] Half-duplex: playback injects no ASR transcripts (self-echo fixture); capture gate closed
      during playback (13 counter asserts zero frames).
